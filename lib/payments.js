...
async function handleWebhook(gatewayId, payload) {
  ...
  let led = await admin().from('credit_ledger').insert({ user_id: pay.user_id, delta: pay.credits, reason: 'purchase', payment_id: pay.id });
  if (led.error) led = await admin().from('credit_ledger').insert({ user_id: pay.user_id, delta: pay.credits, reason: 'purchase' });
  if (led.error) await admin().from('credit_ledger').insert({ user_id: pay.user_id, delta: pay.credits });
