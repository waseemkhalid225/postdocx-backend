...
function evaluate(opp, facts) {
  ...
  // ---- verdict ----
  let pct = stated ? Math.round(100 * met / stated) : null;
  ...
  let status;
  if (overqualified) { status = 'below_your_level'; pct = null; }
  ...
  return { status, pct, reasons, stated, met, overqualified, fieldMismatch };
}
