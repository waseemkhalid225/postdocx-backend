...
function evaluate(opp, facts, wantedLevels) {
  ...
  if (myLevel && oppLevel && LEVEL_RANK[oppLevel] < LEVEL_RANK[myLevel] && String(opp.kind || '') !== 'work') overqualified = true;   /* Phase 4 BUG-008: work postings never count as 'below your level' */
  ...
  { const reqYrsCol = opp.req_experience_years != null ? Number(opp.req_experience_years) : null; const mTxt = String(opp.requirements || opp.description || '').match(/(\d{1,2})\s*\+?\s*(?:years?|yrs?)(?:'|s)?\s+(?:of\s+)?(?:relevant\s+|post-?qualification\s+|clinical\s+)?(?:work\s+)?experience/i); const reqYrs = reqYrsCol != null && isFinite(reqYrsCol) ? reqYrsCol : (mTxt ? Number(mTxt[1]) : null);
    if (reqYrs != null && reqYrs > 0) { stated++; const mine = Number(facts._profile.total_experience_years != null ? facts._profile.total_experience_years : (facts._profile.experience_years != null ? facts._profile.experience_years : ((facts.years_experience || {}).value))); if (isFinite(mine) && mine > 0) { if (mine >= reqYrs) { met++; reasons.push({ ok: 'yes', text: 'Requires ' + reqYrs + '+ years; you have ' + mine }); } else reasons.push({ ok: 'no', text: 'Requires ' + reqYrs + '+ years experience; your profile shows ' + mine }); } else reasons.push({ ok: 'unknown', text: reqYrs + '+ years experience required; add dated experience to your profile' }); } }
  // ---- language test score stated in the posting vs the score on file (Phase 4) ----
  { const lt = String(opp.requirements || opp.description || '').match(/\b(IELTS|PTE|OET|TOEFL)\b[^0-9]{0,30}(\d{1,3}(?:\.\d)?)/i); if (lt) { const test = lt[1].toUpperCase(); const need = parseFloat(lt[2]); const mineT = (facts._profile.language_tests || []).find(t => String(t.test || '').toUpperCase().includes(test)); const score = mineT ? parseFloat(mineT.score) : NaN; if (!isNaN(need)) { stated++; if (isFinite(score)) { if (score >= need) { met++; reasons.push({ ok: 'yes', text: test + ' ' + need + ' asked; you hold ' + score }); } else reasons.push({ ok: 'no', text: 'Requires ' + test + ' ' + need + '; your score on file is ' + score }); } else reasons.push({ ok: 'unknown', text: test + ' ' + need + ' asked; no ' + test + ' result on your profile' }); } } }
  ...
  let status;
  const allUnknown = stated > 0 && met === 0 && unknowns === stated;
  if (wrongTarget) { status = 'wrong_target_level'; pct = null; }
  else if (allUnknown) { status = 'needs_profile'; pct = null; }
  ...
  /* PARTNER PRIORITY (rule): only when the applicant meets the criteria (status eligible) does a partner institution rank first;
     a partner never becomes eligible by being a partner, and the reason is written on the card. */
  let pctOut = pct, reasonsOut = reasons; if (opp && opp.is_partner && status === 'eligible') { pctOut = Math.min(100, pct + 6); reasonsOut = ['Partner institution: shown first among the options you qualify for'].concat(reasons || []); }
  return { status, pct: pctOut, dims, reasons: reasonsOut, stated, met, overqualified, fieldMismatch, wrongTarget, levelUnknown, adjacentRole, adjacentEvidence, partner: !!(opp && opp.is_partner) };
}
