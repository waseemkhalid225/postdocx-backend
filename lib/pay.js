...
/* One place that turns any currency amount into rupees, so every screen agrees. */
function toPKR(amount, currency, usdToPkr) {
  const a = Number(amount);
  if (!isFinite(a) || a <= 0) return null;
  const per = PER_USD[String(currency || 'USD').toUpperCase()];
  if (!per) return null;
  const rate = Number(usdToPkr) > 0 ? Number(usdToPkr) : 278;
  return Math.round((a / per) * rate);
}
/* Rupees the way Pakistanis actually say them. */
function pkrWords(pkr) {
  const n = Number(pkr);
...
module.exports = { parsePay, annualise, summarise, summariseLive, liveRates, perUsd, toPKR, pkrWords, PER_USD };
