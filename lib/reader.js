...
function applyExtractedFacts(v, p) { const patch = {}; const filled = [];
  ...
  try { const yrs = (v.experience || []).reduce((a, e) => { const s0 = parseInt(String(e.start || '').slice(0, 4)), e0 = /present|current|now/i.test(String(e.end || '')) ? new Date().getFullYear() : parseInt(String(e.end || '').slice(0, 4)); if (s0 && e0 && e0 >= s0) { const sm = /^\d{4}-\d{2}/.test(String(e.start || '')) ? parseInt(String(e.start).slice(5, 7)) : 1; const em = /^\d{4}-\d{2}/.test(String(e.end || '')) ? parseInt(String(e.end).slice(5, 7)) : (/present|current|now/i.test(String(e.end || '')) ? new Date().getMonth() + 1 : 12); return a + (e0 - s0) + (em - sm) / 12; } const y = parseFloat(e.years); return a + (isFinite(y) ? y : 0); }, 0); const total = Math.round(Math.max(yrs, Number(v.total_experience_years) || 0) * 10) / 10; if (total > 0 && !(Number(p.experience_years) > 0)) { patch.experience_years = total; patch.total_experience_years = total; filled.push('experience_years'); } } catch (e) {}
