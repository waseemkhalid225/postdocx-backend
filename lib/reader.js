...
/* THE FULL PROFILE FROM THE DOCUMENTS: every fact the extractor found, written where it belongs; totals computed, never trusted. Shared by both extraction pipelines. */
const PROFILE_COLS = new Set(['full_name','phone','whatsapp','city','address','country_code','nationality','date_of_birth','national_id','passport_number','headline','field','profession','degree','degree_level','education','experience','experience_years','total_experience_years','cgpa','last_institution','publications','licenses','linkedin','methods','languages','target_countries','current_salary','summary','highest_degree','highest_degree_year','skills','certifications','language_tests','awards','memberships','projects','volunteering','gender','marital_status','current_employer','notice_period','driving_licence','origin_country','arrival_date','links']);
function applyExtractedFacts(v, p) { const patch = {}; const filled = [];
  for (const k of ['full_name', 'phone', 'whatsapp', 'headline', 'profession', 'field', 'methods', 'nationality', 'city', 'address', 'linkedin', 'cgpa', 'last_institution', 'degree_level', 'national_id', 'passport_number', 'gender', 'marital_status', 'current_employer', 'notice_period', 'driving_licence', 'current_salary', 'summary', 'highest_degree']) {
  ...
  for (const k of ['education', 'publications', 'experience', 'licenses', 'skills', 'languages', 'certifications', 'language_tests', 'awards', 'memberships', 'projects', 'volunteering', 'target_countries']) {
  ...
  /* Phase 3: total experience is the UNION of employment intervals (overlapping jobs are not double-counted); month precision when given. */
  try { const now = new Date(); const iv = []; for (const e of (v.experience || [])) { const s0 = parseInt(String(e.start || '').slice(0, 4)); if (!s0) continue; const sm = /^\d{4}-\d{2}/.test(String(e.start || '')) ? parseInt(String(e.start).slice(5, 7)) : 1; let e0, em; if (/present|current|now/i.test(String(e.end || '')) || !e.end) { e0 = now.getFullYear(); em = now.getMonth() + 1; } else { e0 = parseInt(String(e.end || '').slice(0, 4)); em = /^\d{4}-\d{2}/.test(String(e.end || '')) ? parseInt(String(e.end).slice(5, 7)) : 12; } if (!e0 || e0 < s0) continue; iv.push([s0 * 12 + sm, e0 * 12 + em + (/present|current|now/i.test(String(e.end || '')) || !e.end ? 0 : 1)]); }   /* inclusive end month: Jan–Dec = 12 months */ iv.sort((a, b) => a[0] - b[0]); let months = 0, cur = null; for (const [a, b] of iv) { if (!cur || a > cur[1]) { if (cur) months += cur[1] - cur[0]; cur = [a, b]; } else if (b > cur[1]) cur[1] = b; } if (cur) months += cur[1] - cur[0]; const yrs = months / 12; if (yrs > 0 && (p.total_experience_years == null || p.total_experience_years === '')) { patch.total_experience_years = Math.round(yrs * 10) / 10; filled.push('total_experience_years'); } if (yrs > 0 && (p.experience_years == null || p.experience_years === '')) { patch.experience_years = Math.round(yrs * 10) / 10; filled.push('experience_years'); } } catch (e) {}
  try { const order = ['postdoc', 'phd', 'mphil', 'masters', 'bachelors', 'diploma', 'intermediate', 'matric']; const lvl = d => { const t = String(d.degree || '').toLowerCase(); return /post-?doc/.test(t) ? 'postdoc' : /ph\.?d|doctor/.test(t) ? 'phd' : /m\.?phil/.test(t) ? 'mphil' : /diploma|dae|associate|certificate/.test(t) ? 'diploma' : /master|\bmsc\b|\bma\b|\bmba\b|\bm\.?s\b|pharm-?d|\bmd\b|mbbs/.test(t) ? 'masters' : /bachelor|\bbsc\b|\bba\b|\bbs\b|\bb\.?e\b|b\.?tech|\bllb\b|\bbds\b/.test(t) ? 'bachelors' : /intermediate|hssc|a-?level|fsc|fa\b/.test(t) ? 'intermediate' : /matric|ssc|o-?level/.test(t) ? 'matric' : ''; }; const ed = (v.education || []).map(d => ({ d, l: lvl(d) })).filter(x => x.l).sort((a, b) => order.indexOf(a.l) - order.indexOf(b.l)); if (ed.length) { const top = ed[0]; if (!p.degree_level || !order.includes(String(p.degree_level))) { patch.degree_level = top.l; filled.push('degree_level'); } if (!p.degree && top.d.degree) { patch.degree = String(top.d.degree).slice(0, 120); filled.push('degree'); } if (top.d.year && !p.highest_degree_year) { patch.highest_degree_year = String(top.d.year).slice(0, 4); filled.push('highest_degree_year'); } } } catch (e) {}
  for (const k of Object.keys(patch)) if (!PROFILE_COLS.has(k)) delete patch[k];
  return { patch, filled: filled.filter(k => PROFILE_COLS.has(k)) }; }
...
Only facts literally present in the documents. Empty string/array/0 where absent. Do not invent years. Never use the email address in the CV for anything; ignore it.` });
  ...
  { const r = applyExtractedFacts(v, p); Object.assign(patch, r.patch); for (const k of r.filled) filled.push(k); }
...
module.exports = { applyExtractedFacts, PROFILE_COLS, extractProfileFromDocs, ensureBucket };
