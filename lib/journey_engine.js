...
async function compute(userId) {
  ...
  else if ((mob.missing_for_match || []).length) { stage = 'qualify'; action = A('Complete your profile: ' + mob.missing_for_match.slice(0, 3).map(k => ({ highest_level: 'your highest degree', lane: 'study or work', target_countries: 'where you want to go', field: 'your field', profession: 'your profession', experience_years: 'years of experience', languages: 'language tests', cgpa: 'your grades', origin_country: 'your country' })[k] || String(k).replace(/_/g, ' ')).join(', '), 'you', null, 'profile'); }
