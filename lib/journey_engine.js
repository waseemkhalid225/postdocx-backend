...
async function compute(userId) {
  ...
  else if (apps.some(a => ['applied', 'submitted', 'submitted_email', 'sent'].includes(a.status) || ['submitted_email', 'submitted', 'sent', 'applied'].includes(a.stage))) { stage = 'reply'; action = A('Waiting for replies; they land in your Mail tab and are read for you', 'them', null, 'mail'); }
  else if (apps.some(a => ['prepared', 'ready', 'draft'].includes(a.status))) { stage = 'prepare'; action = A('Review your application', 'you', null, 'apps', 'Your case is prepared; check it and press Send'); }
  else if (apps.length) { stage = 'prepare'; action = A('We are preparing your case', 'us', null, 'apps', 'Nothing to do; it appears here when ready'); }
  else if (!hasCV) { stage = 'discover'; action = A('Upload your CV', 'you', null, 'profile', 'It fills your profile by itself; then we can match you'); }
  else if ((mob.missing_for_match || []).length) { stage = 'qualify'; const H = { highest_level: 'your highest degree', degree_level: 'your highest degree', lane: 'study or work', target_countries: 'where you want to go', field: 'your field', profession: 'your profession', origin_country: 'your country', budget: 'your budget', budget_pkr_per_year: 'your budget', funding_source: 'how you will fund it', years_experience: 'your years of experience', test_name: 'your language test', overall_score: 'your test score', earliest_start: 'when you can start', nationality: 'your nationality', date_of_birth: 'your date of birth', phone: 'your phone', full_name: 'your name' }; const human = k => H[k] || String(k).replace(/_/g, ' '); action = A('Complete your profile', 'you', null, 'profile', 'Add ' + mob.missing_for_match.slice(0, 3).map(human).join(', ') + ' to unlock matched opportunities'); }
  else { stage = 'match'; action = A('Find your matches', 'you', null, 'explore', 'Your profile is complete; open Opportunities to see what fits you'); }
  const flags = [];
