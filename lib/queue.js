...
async function enqueue(kind, payload, opts = {}) {
  /* Phase 9: an idempotency key (explicit, or derived for per-user/per-document jobs) means the same work is never queued twice while a copy is queued or running. */
  const idem = opts.idemKey || (payload && (payload.docId || payload.messageId || payload.applicationId || payload.userId) ? kind + ':' + (payload.docId || payload.messageId || payload.applicationId || payload.userId) : null);
  if (idem) { try { const { data: ex } = await admin().from('job_queue').select('id').eq('idem_key', idem).in('status', ['queued', 'running']).limit(1); if (ex && ex.length) return ex[0].id; } catch (e) {} }
  const { data, error } = await admin().from('job_queue').insert({ idem_key: idem, status: 'queued', attempts: 0,
    kind, payload: payload || {}, org_id: opts.orgId || null, user_id: opts.userId || null,
    max_attempts: opts.maxAttempts || 3, run_after: opts.runAfter || new Date().toISOString(), timeout_ms: opts.timeoutMs || null
  }).select('id').single();
...
async function claimOne() {
  ...
  const { data: rows } = await admin().from('job_queue').select('id,kind,payload,attempts,max_attempts,org_id,user_id,timeout_ms')
...
async function runOne() {
  ...
  try {
    ...
    /* Phase 9: a job cannot hang a worker for ever; after its timeout it is treated as a failure (retry or dead). */
    const ms = Number(job.timeout_ms) || 10 * 60000; let timer = null; const result = await Promise.race([fn(job.payload || {}, job), new Promise((_, rej) => { timer = setTimeout(() => rej(new Error('job timeout after ' + ms + ' ms')), ms); })]).finally(() => clearTimeout(timer));
...
/* Restart recovery: a job left 'running' by a worker that died goes back to the queue (Phase 9: callable, tested). */
async function reapStale() { try { await admin().from('job_queue').update({ status: 'queued', locked_at: null, locked_by: null }).eq('status', 'running').lt('locked_at', new Date(Date.now() - 20 * 60000).toISOString()); } catch (e) {} }
function start(intervalMs = 5000, concurrency = 2) {
  ...
  setInterval(async () => { await reapStale();
  }, 5 * 60000);
}
...
module.exports = { reapStale, laneStatus, laneResume, laneKey, register, enqueue, runOne, start, status, WORKER };
