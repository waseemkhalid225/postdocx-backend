...
async function sendFromApplyEmail(userId, { applicationId, to, subject, body, attachDocIds }) {
  const M = require('./mailer');   /* Phase 5: a missing or failing provider no longer throws; the message is queued durably (DISC-006). */
  ...
  /* The case message row is written FIRST (queued), then the durable send; the outbox flush marks it sent. Idempotency key = user + application + recipient + subject + body. */
  const crypto = require('crypto'); const idem = crypto.createHash('sha256').update([userId, applicationId || '', String(to).toLowerCase(), String(subject || ''), String(bodyOut || '').slice(0, 4000)].join('|')).digest('hex');
  try { const { data: dupMsg } = await admin().from('case_messages').select('id,send_status').eq('user_id', userId).eq('direction', 'out').eq('idempotency_key', idem).limit(1); if (dupMsg && dupMsg[0]) return { sent: dupMsg[0].send_status === 'sent', queued: dupMsg[0].send_status === 'queued', deduped: true, id: dupMsg[0].id }; } catch (e) {}
  const { data: cm } = await admin().from('case_messages').insert({ application_id: applicationId || null, user_id: userId, direction: 'out', channel: 'email', from_addr: p.apply_email, to_addr: String(to).slice(0, 200), subject: String(subject || '').slice(0, 300), body: String(body || '').slice(0, 20000), assigned_by: 'sent', read_at: new Date().toISOString(), triage: 'application', send_status: 'queued', idempotency_key: idem }).select('id').single().then(x => x, () => ({ data: null }));
  const r = await M.sendDurable({ idempotencyKey: idem, userId, applicationId, kind: 'case', caseMessageId: cm && cm.id, from: (p.full_name ? p.full_name + ' <' + p.apply_email + '>' : p.apply_email), to, subject, html, text: bodyOut, replyTo: p.apply_email, cc: undefined, attachments, attachDocIds: (attachDocIds || []).slice(0, 8), headers: { 'X-Case-Channel': channel.kind === 'agency' ? 'c:' + String(channel.org_id).slice(0, 8) : 'd' } });
  ...
  if (r.sent) { try { if (cm && cm.id) await admin().from('case_messages').update({ send_status: 'sent', provider_message_id: r.id || null }).eq('id', cm.id); } catch (e) {} }
  else if (r.queued) { try { if (cm && cm.id) await admin().from('case_messages').update({ send_status: 'queued', outbox_id: r.outbox_id || null }).eq('id', cm.id); } catch (e) {} }
  else { try { if (cm && cm.id) await admin().from('case_messages').update({ send_status: 'failed' }).eq('id', cm.id); } catch (e) {} throw new Error('Could not send: ' + (r.reason || 'mail error')); }
  ...
  return { sent: !!r.sent, queued: !!r.queued, id: r.id || (cm && cm.id), note: r.queued ? 'The mail service is not reachable right now. Your message is queued and will go out automatically; nothing to do.' : null };
}
