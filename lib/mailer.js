...
async function sendMail({ to, subject, text, html, fromName, attachments, creds, replyTo, bcc, gmail }) {
  // ONLY sender: the user's own Gmail via the Gmail API. Nothing third party, ever.
  const uref = gmail && gmail.refresh ? gmail.refresh : undefined;
  const uaddr = gmail && gmail.addr ? gmail.addr : undefined;
  if (!gmailApi.isConfigured(uref)) {
    throw new Error(uref === undefined ? 'Gmail is not connected. Connect your Gmail in Profile and settings, or set the GOOGLE_OAUTH variables.' : 'Your Gmail token looks empty, reconnect it in Profile and settings.');
  }
  try {
    const r = await gmailApi.sendViaGmailApi({ to, subject, text, fromName, replyTo, bcc, attachments, authRefresh: uref, senderAddr: uaddr });
    return r.id;
  } catch (e) {
    const msg = String(e.message || '');
    if (/insufficient.*scope|ACCESS_TOKEN_SCOPE_INSUFFICIENT|Request had insufficient authentication/i.test(msg)) {
      throw new Error('Gmail refused: the token has the WRONG SCOPE. Redo the OAuth Playground with EXACTLY https://www.googleapis.com/auth/gmail.send then reconnect it (Profile and settings for members, Railway variables for the owner).');
    }
    if (/invalid_grant/i.test(msg)) {
      throw new Error('Gmail refused: the refresh token is expired or revoked. Generate a fresh one at the OAuth Playground and reconnect (Profile and settings for members, Railway for the owner).');
    }
