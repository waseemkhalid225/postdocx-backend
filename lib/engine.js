...
${p.headline || ''}; field ${p.field || ''}; skills ${String(p.methods || '').slice(0, 200)}. ${budgetLine} ${licenseLine} ${countryLine} ${remoteLine} ${levelLine} ${langLine} ${licenseLine2} ${fieldLine} ${intakeLine} ${jobLine} ${uniLine}
...
CRITICAL: fill criteria ONLY from requirements literally stated on the official page. Leave any unstated requirement as empty; never guess a CGPA, language score, or restriction.`;
  ...
  if (!added && lastPrompt) {
    try {
      const txt2 = await callAI('search_verify', searchStrategy() + lastPrompt, { search: true, urls: true, maxTokens: 1800, userId });
