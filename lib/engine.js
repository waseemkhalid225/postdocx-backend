...
async function discoverForUser(userId, kind, prefs) {
  ...
  const licFitLine = (prefs.licenses && prefs.licenses.length)
    ? 'CREDENTIAL FIT CHECK: confirm each selected credential actually belongs to this applicant profession. If a selected exam does not apply to their profession, ignore that exam and say so plainly in the results summary rather than returning unsuitable roles.'
    : '';
  ...
  const eligLine = 'ELIGIBILITY RULE, ABSOLUTE: only return an opportunity if its OWN stated requirements would accept this applicant\'s exact degree and profession. If the advert names required disciplines and the applicant\'s discipline (or a clearly equivalent one) is not among them, DISCARD it, however attractive it looks. State, for every result, the exact eligibility line from the official page that admits this applicant.';
...
async function ingestOpps(items, kind, userId) {
  ...
  for (const it of items) {
    ...
    const lvl = ['bachelors', 'masters', 'phd', 'postdoc', 'diploma', 'short_course', 'fellowship', 'observership', 'licensing_exam'].includes(String(it.level || '').toLowerCase())
...
async function prepareApplication(appId) {
  ...
  try {
    ...
    const ex = (pv.licenses || []).concat(pv.licenseExam ? [pv.licenseExam] : []);
    if (ex.length || pv.licenseStatus) {
      licCtx = `\nLICENCE CONTEXT: exams/registrations ${ex.join(', ') || 'not specified'}; status ${pv.licenseStatus || 'not specified'}.`;
    }
  } catch (e) {}
  const profileBlock =
...
DEPTH REQUIREMENT: analyse the position deeply and mirror its requirements point by point. For research positions (MS, MPhil, PhD, postdoc) read the applicant's own publications and research papers above and weave their actual findings, methods and titles into the research proposal and motivation, so the documents read as written by this exact researcher. Write at the length and polish of a highly qualified applicant: substantial, specific, warm and human.${licCtx}`;
  ...
  const MINLEN = { cv: 2600, cover: 2200, sop: 3000, research_proposal: 4000, scholarship_statement: 2000, checklist: 1200, licensing_plan: 2600, visa_summary: 1400, funder_outreach: 900 }
const MAXLEN = { licensing_plan: 6000, visa_summary: 4000, checklist: 6000, cover: 6000, cv: 20000, sop: 8000, research_proposal: 10000 };
...
{"required_now":["needed to SUBMIT this application"],"required_later":["needed after acceptance, visa stage"],"optional":["strengthens the case"],"missing_urgent":["required_now items the applicant has NOT uploaded"]}`,
  ...
  const DOC_CATALOG = {
    licensing_plan: ['Your Licensing Action Plan', 'Adapt to the applicant LICENCE STATUS given in the profile block: if they are PREPARING, lead with eligibility and examination strategy; if they have PASSED or are REGISTERED, lead with activation, employer linkage and job-readiness. Include a BOOKING AND SLOTS section (how and where to book, realistic waiting time for centres in Pakistan, and what identification must match), an INDICATIVE COST section (examination fee, verification fee, attestation and courier, and travel where the exam or OSCE must be taken abroad, clearly marked as indicative and to be confirmed on the official page), and a WHAT MAY HAVE CHANGED note reminding the applicant to confirm current rules on the regulator page because requirements are updated periodically. Produce a precise, personal LICENSING ACTION PLAN for this applicant and this position\'s country and profession. Sections: REGULATOR AND ROUTE (name the exact authority and the licence route that applies to this applicant\'s degree and profession). YOUR ELIGIBILITY (state plainly, from the applicant profile, which requirements they already meet). STEP-BY-STEP SEQUENCE (numbered, with the realistic time each step takes and which steps can run in parallel). DOCUMENTS TO PREPARE (exact documents and any attestation, naming the issuing bodies). VERIFICATION (DataFlow, EPIC/ECFMG or CGFNS as applicable, with typical duration and the single most common cause of delay). EXAMINATION (what is tested, how to book, and typical waiting time for slots). AFTER YOU PASS (activation, employer linkage, validity windows to watch). Write it as a confident professional adviser, specific, never generic.'],
  ...
  let _isLicenceCase = false;
  try {
    const { data: pfRow } = await admin().from('app_settings').select('value').eq('key', 'prefs:' + a.user_id).single();
    const pv = (pfRow && pfRow.value) || {};
    _isLicenceCase = !!((pv.licenses || []).length || pv.licenseExam ||
      /licen[cs]|registration|prometric|dataflow/i.test(String(opp.title || '') + ' ' + String(opp.req_license || '')));
  } catch (e) {}
  try {
    ...
    if (!allowed.includes('licensing_plan')) allowed.push('licensing_plan');
    if (!allowed.includes('visa_summary')) allowed.push('visa_summary');
    ...
    if (opp.kind === 'work') { plan.push('checklist'); plan.push('visa_summary'); }
    if (_isLicenceCase) plan.push('licensing_plan');
    ...
    // PORTAL-ONLY EFFICIENCY: when the position is submitted through an online portal
    // (no email recipient), we do NOT waste minutes writing an email or cover letter the
    // portal never uses. We prepare the tailored CV (the one real attachment) and the
    // requirements checklist so the applicant knows exactly what to ready and upload, and
    // the browser assistant fills the portal fields from the profile. Email-route cases
    // keep the full set. This is decided per case from the verified apply route.
    const portalOnly = (opp.apply_via === 'portal') && !((opp.contact_emails || []).length); _portalOnly = portalOnly;
    if (portalOnly) {
      plan = plan.filter(k => k === 'cv' || k === 'checklist' || k === 'funder_outreach' || k === 'licensing_plan');
      if (!plan.includes('cv')) plan.unshift('cv');
      if (!plan.includes('checklist')) plan.push('checklist');
    }
  } catch (e) {}
  ...
  if (_portalOnly) {
    // No email is drafted for a portal submission; the case is ready to attach + fill.
    await setProg(appId, ['profile','cv','checklist'], 'done');
    try { await admin().from('applications').update({ stage: 'portal_apply', next_action: 'Open the position\'s official portal, let the assistant fill your details, attach your prepared CV, and complete any in-person steps yourself.' }).eq('id', appId); } catch (e) {}
    return;
  }
