...
[{"title":"","institution":"","country_code":"ISO2","city":"","url":"official page","deadline":"YYYY-MM-DD or empty","funding":"","funding_type":"fully|partial|self","level":"bachelors|masters|phd|postdoc","stipend":"","duration":"","contact_emails":["seen on official pages only"],"apply_via":"email|portal"}]`;
  ...
  for (const it of items) {
    ...
    // If the new columns aren't migrated yet, retry without them so discovery never breaks.
    if (error && /funding_type|level|column/.test(error.message || '')) {
      delete row.funding_type; delete row.level;
      ({ error } = await admin().from('opportunities').insert(row));
    }
    if (!error) added++;
  }
