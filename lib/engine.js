...
const noSmell = t => { let x = String(t || '')
  .replace(/\s*[—–]\s*/g, ', ').replace(/,\s*,/g, ',').replace(/,\s*\./g, '.')
  .replace(/\[(?:insert|your|add|name|date|university|position)[^\]]*\]/gi, ''); try { x = require('./partnerships').humanize(x); } catch (e) {} return x; };

/* Eligibility flags read from the posting text: citizenship-only, security clearance, local-candidates-only. Flagged
   postings stay visible with the reason, ranked lower, and never counted as 'eligible' for a foreign applicant. */
function eligibilityFlag(text) { const t = String(text || '').toLowerCase(); if (/must be a (us|u\.s\.|uk|british|canadian|australian|eu) citizen|citizens only|citizenship (is )?required|only (uk|us|eu|canadian|australian) citizens/.test(t)) return 'citizens_only'; if (/security clearance|sc cleared|dv cleared|top secret|clearance required/.test(t)) return 'clearance'; if (/local candidates only|must (already )?(be|reside|live) in (the )?(uk|us|canada|australia|germany|eu)|no (visa )?sponsorship|we (do not|don't|cannot) sponsor|not able to sponsor|without sponsorship/.test(t)) return 'local_only'; return null; }
...
/* ---------- DISCOVERY: profile-driven, evidence-only, verified at birth ----------
   Vocabulary: prefs._synonyms (from ESCO) is appended to the search brief as 'Also search these titles: ...'. */
...
/* Profession synonyms from the ESCO store widen the search vocabulary (e.g. 'pharmacist' → 'hospital pharmacist',
   'clinical pharmacist', 'dispensing chemist') so structured feeds and grounded search both catch the real titles. */
async function synonymsFor(term) { try { const t = String(term || '').trim(); if (!t) return []; const { data } = await admin().from('professions').select('title,alt_labels').ilike('title', '%' + t.replace(/[%,]/g, ' ') + '%').limit(5); const out = new Set(); for (const r of (data || [])) { out.add(r.title); for (const a of (r.alt_labels || []).slice(0, 6)) out.add(a); } return [...out].filter(x => x && x.toLowerCase() !== t.toLowerCase()).slice(0, 8); } catch (e) { return []; } }
async function discoverForUser(userId, kind, prefs) {
  try { const base = (prefs && (prefs.profession || prefs.field || prefs.text)) || ''; if (base && !prefs._synonyms) { prefs._synonyms = await synonymsFor(base); } } catch (e) {}
  prefs = prefs || {};
  ...
  /* LIVE NARRATION. The screen used to run a timed animation with invented stage names
     and a randomly incrementing "opportunities checked" counter, which meant the user was
     watching a cartoon of a search rather than the search. Every real phase now writes
     what it is genuinely doing, and the client reads it back. */
  let _found = 0;
  const prog = async (stage) => {
    if (!prefs.progressKey) return;
    try {
      await admin().from('app_settings').upsert({ key: prefs.progressKey, value: {
        status: 'running', startedAt: prefs.startedAt || new Date().toISOString(),
        kind, target, found: _found, stage: String(stage || '').slice(0, 120), prefsHash: prefs.prefsHash || null } });
    } catch (e) {}
  };
  ...
  /* COVERAGE WHEN THE APPLICANT NAMES NO COUNTRY. This used to be a single pass with no
     country line at all, which handed the choice to the model - and a model reaches for
     the destinations it has read most about. The result was the same five countries every
     time, while Hungary, Malaysia, China, Turkiye, Taiwan and the Gulf, several of which
     are markedly easier for a Pakistani applicant, were never searched. The three passes
     now sweep every supported destination in accessibility order. An applicant who DID
     name countries is unaffected: their choice leads, exactly as before. */
  const ACCESS = require('./access');
  /* Access intelligence applies to a CHOSEN set too, not only to "any country". It adds
     and removes nothing - the applicant's list is exactly the applicant's list - it only
     decides which of their countries is searched first, so a run that runs short of time
     has spent it on the destinations that pay best and are most reachable. */
  /* A CHOSEN COUNTRY IS THE WHOLE SCOPE. The second and third passes used to widen to
     "comparable nearby destinations" and then worldwide, and the finish test counted
     those rows towards the target. The search reported fifteen found while the applicant's
     own country held four, and the rest were never shown to them because the results
     endpoint correctly filters to what they chose. When countries are named, every pass
     stays inside them (each pass attacks from a different source family, so coverage
     still compounds), and anything the model returns from elsewhere is dropped before it
     is even verified. The remote lane is the one exception: the work is done from
     Pakistan, so geography is not a wall there. */
  const strictScope = priority.length > 0 && !prefs.remote;
  const passes = priority.length
    ? (strictScope
        ? [ACCESS.orderChosen(priority), ACCESS.orderChosen(priority), ACCESS.orderChosen(priority)]
        : [ACCESS.orderChosen(priority), ACCESS.orderChosen(expandCountries(priority)), null])
    : ACCESS.tiers();
  // Free tier: one pass, cost bounded. Copy before trimming so shared tier arrays are never mutated.
  const passList = Number(prefs.maxPasses) > 0 ? passes.slice(0, Number(prefs.maxPasses)) : passes;
  const inScope = it => !strictScope || priority.includes(String((it && it.country_code) || '').slice(0, 2).toUpperCase());
  const ingestScoped = async (items) => {
    const list = Array.isArray(items) ? items : [];
    const kept = strictScope ? list.filter(inScope) : list;
    if (strictScope && kept.length < list.length) {
      try { await admin().from('audit_log').insert({ actor: userId, event: 'DISCOVER_OUT_OF_SCOPE_DROPPED', detail: (list.length - kept.length) + ' of ' + list.length + ' outside ' + priority.join(',') }); } catch (e) {}
    }
    return ingestOpps(kept, kind, userId);
  };
  ...
  await prog('Reading your profile and eligibility');
  for (let pi = 0; pi < passList.length; pi++) {
    await prog(pi === 0
      ? ('Searching official sources' + (priority.length ? ' in ' + priority.slice(0, 3).join(', ') : ''))
      : ('Widening the search, round ' + (pi + 1) + ' of ' + passList.length));
  const scope = passList[pi];
  /* Why these destinations, in the applicant's terms. Structural facts only - language of
     instruction, tuition, funding norms, mission in Pakistan, work rights - never invented
     visa statistics. */
  const stipendLine = (!priority.length)
    ? 'NO COUNTRY WAS CHOSEN, so the applicant is asking for the best position they can realistically reach, anywhere. Rank what you return by the VALUE of the offer: the size of the stipend or salary against the local cost of living first, then whether tuition is charged at all, then the strength of the funding. State the actual figure with its currency every time the page gives one, and never return a position with unstated money ahead of one that states it.'
    : '';
  const accessLine = (!priority.length && scope && scope.length)
    ? 'PAKISTAN ACCESS PRIORITY for this pass. The applicant named no country, so search these destinations, which are ordered by how realistically a Pakistani applicant can reach them: '
      + scope.slice(0, 18).map(cc => cc + ' (' + (ACCESS.accessNote(cc) || '').slice(0, 90) + ')').join('; ')
      + '. Judge each position on its merits; this ordering decides where to look, never what to accept.'
    : '';
  const countryLine = prefs.remote
    ...
    : scope
    ? ((pi === 0 || strictScope)
      ? 'STRICT COUNTRY SCOPE, only: ' + scope.join(', ') + '. Every position you return MUST be located in one of these countries; a position anywhere else will be discarded unread.'
  ...
  /* THE AGENT NOW READS THE CV. It used to receive one sentence - profession, years,
     five certifications - and search on the applicant's current job title. It now gets
     the CV evidence itself and the role families a recruiter would derive from it,
     whatever the field: engineering, computing, law, finance, teaching, agriculture,
     design, health, the sciences or business. */
  const TGT = require('./targeting');
  let tgt = null, digest = '';
  try {
    digest = TGT.cvDigest(px, p);
    tgt = await TGT.getTargeting(userId);
    if (!tgt || !(tgt.direct_roles || []).length) tgt = await TGT.buildTargeting(userId, px, p);
  } catch (e) {}
  const digestLine = digest ? 'CANDIDATE CV EVIDENCE, read all of it before deciding what to search for:\n' + digest + '\n' : '';
  const roleLine = tgt ? [
    (tgt.direct_roles || []).length ? 'SEARCH FOR THESE ROLE TITLES DIRECTLY, one search each, not one combined search: ' + tgt.direct_roles.join('; ') + '.' : '',
    (tgt.skill_roles || []).length ? 'ROLES THAT FOLLOW FROM THEIR SPECIFIC TECHNICAL SKILLS, search these too: ' + tgt.skill_roles.join('; ') + '.' : '',
    (tgt.adjacent_roles || []).length ? 'ADJACENT ROLES their stated skills genuinely cover. Include them, and set extra.track to "adjacent" on each so the applicant is told plainly that it is a step sideways: ' + tgt.adjacent_roles.join('; ') + '.' : '',
    (tgt.employer_types || []).length ? 'EMPLOYER TYPES that hire this profile - search each kind: ' + tgt.employer_types.join('; ') + '.' : '',
    (tgt.blocked_roles || []).length ? 'NEVER RETURN THESE, the applicant lacks a mandatory licence or degree for them: ' + tgt.blocked_roles.join('; ') + '.' : '',
    tgt.seniority ? 'SENIORITY, judged from real years and responsibility in the CV: ' + tgt.seniority + '. Target roles at this level, not at the level of their job title.' : ''
  ].filter(Boolean).join(' ') : '';

  /* Schemes, not vacancies. These fund large numbers of Pakistani applicants across every
     discipline every year, and are searchable objects in their own right. */
  const SCHEME_ATLAS = kind === 'work' ? '' : 'FUNDING SCHEMES OPEN TO PAKISTANI NATIONALS IN ANY DISCIPLINE, search these as objects in their own right, not only university pages: '
    + 'China CSC; DAAD (Germany); Stipendium Hungaricum; Turkiye Burslari; MEXT (Japan); Korean GKS; Taiwan MOE and NSTC; '
    + 'Erasmus Mundus Joint Masters; Marie Sklodowska-Curie Actions and COFUND; Swiss Government Excellence; Holland Scholarship; '
    + 'Vanier and Trudeau (Canada); Commonwealth Scholarships and Chevening (UK); Fulbright and USEFP (USA); Australia Awards and RTP; '
    + 'Swedish Institute; Norwegian and NORHED; Finnish Government; Eiffel (France); Italian Government and Invest Your Talent; '
    + 'KAUST and King Abdullah; Qatar Foundation; UAE university fellowships. State the scheme name, its own deadline, and whether it is applied for separately from the institution.';

  /* Postdocs and most European doctorates are won by writing to a person, in every
     discipline - a materials group, an economics centre, a law faculty, a lab. */
  const SUPERVISOR_LINE = (kind === 'work') ? '' :
    'SUPERVISOR-FIRST SEARCHING. For postdoc, PhD and fellowship results the real route is a named academic with funding, not a portal. Search for GROUPS AND PEOPLE whose recent work matches this applicant methods and themes'
    + ((tgt && (tgt.research_themes || []).length) ? ' (' + tgt.research_themes.join('; ') + ')' : '')
    + ': department staff pages, group and centre pages, recent papers, ERC and national grant award announcements, faculty news. '
    + 'When a position belongs to a named group, put the supervisor in contact_name and ONE line of genuine group-fit evidence in extra.group_fit, naming a real recent paper or project of theirs that matches this applicant own work. Never invent a paper or a name.';

  const PROGRAMME_LINE = (kind === 'work') ? '' :
    'PROGRAMME LEVEL, NOT INSTITUTION LEVEL. Never return a university as an opportunity. Every result must be one named programme, position or scheme with its own page and its own deadline.';
  ...
  /* The job-board atlas was health-only AND gated behind a "is this a medical field"
     test, so a software engineer, a lawyer, an accountant, a teacher or an agronomist
     received no source atlas at all and the agent had to guess where their vacancies
     live. It is now chosen from the applicant own professional families. */
  const DOMAINS = require('./domains');
  /* Two sources, both honoured. The profession the applicant PICKED in the finder is an
     instruction and comes first; what their CV says about them is added to it, because a
     civil engineer who selects "Project Management" is both. */
  const pickedFam = DOMAINS.fieldSlugToFamily(prefs.field);
  const myFams = [...new Set((pickedFam ? [pickedFam] : []).concat(DOMAINS.familiesFor([
    String(p.field || ''), String(prefs.field || '').replace(/-/g, ' '), String((px && px.profession) || ''),
    ...((px && px.professions) || []), String(p.headline || ''), String(p.methods || '')
  ])))];
  const PRO_BOARDS = DOMAINS.sourceAtlas(myFams);
  const CRED_ATLAS = kind === 'work' ? DOMAINS.credentialAtlas(myFams) : '';
  /* Regulated is not the same as clinical. Engineers, architects, lawyers, accountants
     and teachers all face registration or a skills assessment, and deserve the same
     warning a nurse gets. Fields with no licence correctly produce nothing here. */
  const LICENCE_REALITY = (kind === 'work' && CRED_ATLAS)
    ? 'REGISTRATION AND VERIFICATION REALITY. ' + CRED_ATLAS
      + ' Put the named body, the stated processing time and the official fee into extra.licence_reality whenever the advert or the regulator states them. If this field needs no licence, say so plainly rather than leaving the applicant wondering.'
    : '';
  const licFitLine = '';
  const licDirective = '';
...
${digestLine}${roleLine}\n${SCHEME_ATLAS}\n${SUPERVISOR_LINE}\n${PROGRAMME_LINE}\n${LICENCE_REALITY}\n${budgetLine} ${licenseLine} ${countryLine}\n${accessLine}\n${stipendLine} ${remoteLine} ${levelLine} ${langLine} ${licenseLine2} ${fieldLine} ${eligLine} ${intakeLine} ${jobLine} ${ladderLine} ${pxLine} ${progLine} ${sectorLine} ${laneLine} ${jobAggr} ${hardRules} ${licExamLine} ${licStatusLine} ${licFitLine} ${licDirective} ${kind === 'work' ? PRO_BOARDS : ''} ${uniLine} ${ANGLES[pi % ANGLES.length]} ${knownLine}
...
CRITICAL: fill criteria ONLY from requirements literally stated on the official page. Leave any unstated requirement as empty; never guess a CGPA, language score, or restriction.`;
  ...
  try {
    const synLine = (prefs._synonyms && prefs._synonyms.length) ? '\nALSO SEARCH THESE TITLES (ESCO synonyms): ' + prefs._synonyms.join(', ') + '\n' : '';
    const labourLine = (() => { try { const L = require('./labour'); const prof = String(prefs.profession || prefs.field || prefs.text || ''); if (prefs.category === 'labour' || L.classify({ title: prof }) === 'labour' || L.classify({ title: prof }) === 'care') { const ccs = (prefs.countries || prefs.priority || []).slice(0, 3); const qs = ccs.length ? ccs.flatMap(c => L.queriesFor(c, prof)) : L.queriesFor('', prof); return '\nLABOUR / TRADES / CARE LANE: search employer and government scheme pages for these roles: ' + [...new Set(qs)].join(', ') + '. Include official schemes (Korea EPS, Japan SSW, UK Health and Care, Gulf employers via licensed recruiters, Canada TFWP, US H-2A/H-2B, Italy Decreto Flussi, Poland/Romania/Croatia work permits) and licensed recruiters\' job orders with the employer named. Reject any posting that asks the worker to pay a fee.\n'; } } catch (e) {} return ''; })();
    const txt = await callAI('search_verify', searchStrategy() + synLine + labourLine + prompt, { search: true, urls: true, maxTokens: 3200, userId });
    const items = parseJSON(txt) || [];
    await prog('Verifying ' + (items.length || 0) + ' listings on their official pages');
    added += await ingestScoped(items);
    _found = added;
    await prog(added ? (added + ' verified so far, still looking') : 'Checking deadlines and eligibility');
  } catch (passErr) {
  ...
  /* The finish line is counted INSIDE the chosen countries on every pass, so a pass can
     never declare the target met with rows the applicant will never be shown. */
  const scopeForCount = priority.length ? priority : null;
  ...
  if (!added && lastPrompt) {
    try {
      ...
      added += await ingestScoped(parseJSON(txt2) || []);
    } catch (e) {}
  }
  /* GAP 5: REFLECTION. Three fixed passes ran and the engine never once looked at what
     came back. A person searching would notice that four of their eight target role
     families returned nothing and go looking specifically for those. One extra pass, only
     when it is warranted, aimed exactly at the gap. */
  try {
    if (tgt && (tgt.direct_roles || []).length && added < target && lastPrompt) {
      const { data: got } = await admin().from('opportunities').select('title')
        .eq('status', 'verified').gte('created_at', since).limit(120);
      const gotBlob = (got || []).map(x => String(x.title || '').toLowerCase()).join(' | ');
      const missing = (tgt.direct_roles || []).concat(tgt.skill_roles || [])
        .filter(r => {
          const head = String(r).toLowerCase().split(/[ ,(]/).filter(w => w.length > 4)[0];
          return head && gotBlob.indexOf(head) < 0;
        }).slice(0, 6);
      if (missing.length) {
        await prog('Looking specifically for ' + missing.slice(0, 2).join(' and '));
        try {
          const gapTxt = await callAI('search_verify',
            lastPrompt + '\n\nGAP PASS. The searches so far returned NOTHING for these role families, which this applicant is genuinely qualified for: '
            + missing.join('; ') + '. Search for these specifically now, by exact title, on employer, regulator and institution pages. '
            + 'Return only positions in these families. If a family truly has nothing open, skip it silently rather than substituting something else.',
            { search: true, urls: true, maxTokens: 6000, userId });
          added += await ingestScoped(parseJSON(gapTxt) || []);
          _found = added;
        } catch (e) {}
      }
    }
...
async function ingestOpps(items, kind, userId) {
  ...
  for (const it of items) {
    ...
    /* THE LINK MUST ACTUALLY RESOLVE. Until now the only test of a URL was that it looked
       like one. A model that invents a plausible address passes a regex effortlessly, and
       the applicant discovers the fabrication only after paying, which is the single worst
       moment to discover it. We fetch the page. A hard 404 or 410 means the position does
       not exist at that address and the row is thrown away. Timeouts, 403s and rate limits
       are NOT treated as failures - many university sites refuse robots, and rejecting a
       real position because its server was slow would be its own kind of dishonesty. */
    try {
      const ac = new AbortController();
      const timer = setTimeout(() => ac.abort(), 6000);
      let resp = null;
      try {
        resp = await fetch(it.url, { method: 'HEAD', redirect: 'follow', signal: ac.signal,
          headers: { 'user-agent': 'Mozilla/5.0 (compatible; ForiForeignBot/1.0)' } });
        if (resp && (resp.status === 405 || resp.status === 501)) {
          resp = await fetch(it.url, { method: 'GET', redirect: 'follow', signal: ac.signal,
            headers: { 'user-agent': 'Mozilla/5.0 (compatible; ForiForeignBot/1.0)' } });
        }
      } finally { clearTimeout(timer); }
      if (resp && (resp.status === 404 || resp.status === 410)) {
        await reject('official url returned ' + resp.status + ': ' + String(it.url).slice(0, 120));
        continue;
      }
      it._reachable = !!(resp && resp.status < 400);
    } catch (e) { it._reachable = false; /* unreachable right now is not the same as fake, but it is not VERIFIED either (Phase 4) */ }
    // Spec 17: opportunity fingerprint = normalized identity; dedup by fingerprint OR url.
    /* Phase 4 (DISC-002): identity = canonical URL OR fingerprint (institution + title + country [+ level]); no deadline in the key, no or-filter built from raw input. */
    const K = require('./oppkey'); const urlKey = K.canonicalUrl(it.url); const fp = K.fingerprintOf(Object.assign({}, it, { kind }));
    let dup = null; try { const r1 = await admin().from('opportunities').select('id').eq('url_key', urlKey).limit(1); dup = r1.data && r1.data[0] ? r1.data : null; if (!dup) { const r2 = await admin().from('opportunities').select('id').eq('fingerprint', fp).limit(1); dup = r2.data && r2.data[0] ? r2.data : null; } if (!dup) { const r3 = await admin().from('opportunities').select('id').eq('url', it.url).limit(1); dup = r3.data && r3.data[0] ? r3.data : null; } } catch (e) {}
    ...
    const row = {
      ...
      eligibility_flag: eligibilityFlag((it.description || '') + ' ' + (it.requirements_text || '') + ' ' + (it.title || '')),
      category: require('./labour').classify({ title: it.title, description: it.description, kind: inferKind(it) }),
      is_partner: await (async () => { try { if (!it.institution) return false; const { data } = await admin().from('institutions').select('id').eq('partner_tier', 'mou').ilike('name', String(it.institution).slice(0, 120)).limit(1); return !!(data && data.length); } catch (e) { return false; } })(),
      ...
      status: it._reachable === false ? 'pending' : 'verified', verified_at: it._reachable === false ? null : new Date().toISOString(), verify_note: it._reachable === false ? 'official page not reachable at discovery; re-checked nightly, not shown until verified' : null, url_key: urlKey, source: 'agent',
      fingerprint: fp,
    ...
    if (it.extra && typeof it.extra === 'object') {
      /* This whitelist is the reason an extracted field can vanish without trace: any key
         not named here is silently dropped at ingest, however carefully the agent found
         it. The four new evidence fields are named explicitly. */
      const ex = {}; for (const k of ['track','group_fit','licence_reality','scheme_name','acceptance_hint','annual_living_cost','housing_support','intake_terms','application_process_steps','interview_required','scholarship_stack','work_rights','pr_pathway_note','ranking_or_reputation']) if (it.extra[k]) ex[k] = String(it.extra[k]).slice(0, 240);
    ...
    /* Provenance (Phase 0): every fact knows where it came from and when. Discovery reads
       official pages through grounded search, so the class is permitted_web; partner and
       government feeds will set their own class when they arrive. */
    row.source_class = 'permitted_web';
    row.source_fetched_at = new Date().toISOString();
    try { row.provenance = { method: 'grounded_search+official_page', url: row.url, host: new URL(String(row.url)).hostname, by: 'discovery', at: row.source_fetched_at }; } catch (e) {}
    ...
    if (error && /funding_type|level|req_|column|source_class|provenance/.test(error.message || '')) {
      const stripped = { ...row }; delete stripped.source_class; delete stripped.source_fetched_at; delete stripped.provenance;
...
/* The screen must describe what actually happens. We stopped rewriting the applicant's
   CV - their original is attached as-is - so "Tailoring your CV" was a step that no
   longer existed, and "Writing your cover letter" hid the six other documents being
   written behind it, which is why the bar appeared frozen. */
const PREP_STEPS=[['profile','Profile prepared'],['recipient','Researching the right contact'],['cv','Attaching your original documents'],['cover','Writing your documents'],['email','Preparing your email'],['final','Final checks']];
...
DEPTH REQUIREMENT: analyse the position deeply and mirror its requirements point by point. For research positions (MS, MPhil, PhD, postdoc) read the applicant's own publications and research papers above and weave their actual findings, methods and titles into the research proposal and motivation, so the documents read as written by this exact researcher. Write at the length and polish of a highly qualified applicant: substantial, specific, warm and human.${licCtx}`;
  ...
  /* Length floors, in characters. These were set for a one-page letter. A cover letter
     that decides whether the rest of the file is read, a motivation letter and a research
     concept note are not one-page documents, and a short one reads as a form letter. */
  const MINLEN = { cv: 2600, cover: 4200, sop: 4500, research_proposal: 7000, scholarship_statement: 2600, checklist: 1200, visa_summary: 1400, motivation: 6500, publications: 800, funder_outreach: 900 }
const MAXLEN = { visa_summary: 4000, motivation: 9000, checklist: 6000, cover: 6000, cv: 20000, sop: 8000, research_proposal: 10000 };
  ...
  const mk = async (kind, title, instruction) => {
    ...
    /* THERE IS NO STUB ANY MORE. When generation failed this wrote a five-line
       placeholder - "I am writing to apply for the position, my documents are attached" -
       stored it as a finished document and showed it to the applicant as our work. That
       is how a case ended up displaying one short, poor document. A failed document is
       now retried once with a plain prompt, and if it still fails it is simply not
       stored: the step reports honestly and Retry regenerates it. Nothing weak is ever
       presented as prepared. */
    if (!content || content.length < Math.min(need, 900)) {
      try {
        content = noSmell(await callAI('case_writing',
          `${STYLE}\n\n${profileBlock}\n\nPOSITION: ${opp.title} at ${opp.institution}. ${instruction}\nWrite the COMPLETE document now, in full, at least ${Math.round(need / 6)} words. Every section written out. No placeholders, no notes to the reader, no markdown symbols.`,
          { maxTokens: Math.max(2600, Math.round(need / 2)), userId: a.user_id, applicationId: appId }));
      } catch (e) {}
    }
    if (!content || content.length < Math.min(need, 900)) {
      try { await admin().from('audit_log').insert({ actor: a.user_id, event: 'DOC_FAILED', detail: kind + ' not stored, too short or generation failed' }); } catch (e) {}
      throw new Error('Could not prepare ' + title + ' this time');
    }
    ...
    /* Documents are written three at a time now, so this read-modify-write races with
       its own siblings and quietly loses entries. Re-reading immediately before the write
       and merging keeps the list complete; a lost entry here makes a finished document
       look unprepared on the case screen. */
    try {
  ...
  try {
    /* Match on the file NAME as well as the kind. A transcript uploaded as "kind:
       document, name: BS_transcript.pdf" was invisible to the comparison below, so the
       applicant was asked again for a file they had already given us. */
    const { data: mydocs } = await admin().from('documents').select('kind,name').eq('user_id', a.user_id);
    const have = (mydocs || []).map(d => String(d.kind || '') + ' ' + String(d.name || ''));
...
{"required_now":["needed to SUBMIT this application"],"required_later":["needed after acceptance, visa stage"],"optional":["strengthens the case"],"missing_urgent":["required_now items the applicant has NOT uploaded"]}`,
    ...
    let reqs = parseJSON(txt) || { required_now: [], required_later: [], optional: [], missing_urgent: [] };
    /* BASELINE REQUIREMENTS, NOT GUESSWORK. The classifier was the only source of this
       list, so when it returned nothing - a bad parse, a thin advert - the applicant was
       asked for no documents at all and discovered at submission time that they needed
       transcripts. Every application of a given type has a floor of documents that is
       true regardless of what the advert bothered to print, so that floor is stated
       deterministically and the classifier only ADDS to it. */
    const LVL = String(opp.level || '').toLowerCase();
    const base = [];
    if (opp.kind === 'work') {
      base.push('Experience certificates from every employer', 'Degree certificate', 'Academic transcript',
        'Professional licence or registration certificate', 'Passport copy', 'Language test certificate if the employer requires one');
    } else {
      base.push('Passport copy', 'Language test certificate (IELTS/TOEFL/OET) if required');
      if (['postdoc', 'fellowship'].includes(LVL)) base.push('PhD degree certificate', 'PhD transcript', 'Masters degree certificate and transcript', 'Bachelors degree certificate and transcript', 'Two or three reference letters', 'Publication list');
      else if (LVL === 'phd') base.push('Masters degree certificate and transcript', 'Bachelors degree certificate and transcript', 'Two or three reference letters');
      else if (LVL === 'masters') base.push('Bachelors degree certificate and transcript', 'Two reference letters');
      else base.push('Secondary school certificates and transcripts', 'One or two reference letters');
    }
    const uniq = a => [...new Set((a || []).map(x => String(x).trim()).filter(Boolean))];
    reqs.required_now = uniq(base.concat(reqs.required_now || []));
    reqs.required_later = uniq(reqs.required_later);
    reqs.optional = uniq(reqs.optional);
    /* Missing is computed from the vault, not asserted by the model. */
    try {
      const haveBlob = (have || []).join(' ').toLowerCase();
      reqs.missing_urgent = reqs.required_now.filter(r => {
        const key = String(r).toLowerCase().split(/[ ,(]/)[0];
        if (key.length < 4) return false;
        return !haveBlob.includes(key.slice(0, 6));
      }).slice(0, 12);
    } catch (e) {}
    if (reqs) {
  ...
  const DOC_CATALOG = {
    ...
    publications: ['Publication List', 'Produce a formal PUBLICATION AND RESEARCH OUTPUT LIST from the applicant real record only. Full citations in a consistent style: all authors in order, exact title, journal or venue, year, volume, pages, DOI where known. Group as PEER-REVIEWED JOURNAL ARTICLES, then CONFERENCE PRESENTATIONS AND ABSTRACTS, then THESES, then MANUSCRIPTS UNDER REVIEW if the profile states any. Add a one-line summary of total counts at the top. Never invent a paper, never pad the list, and never alter an author order.'],
  ...
  /* The default was ['cv','cover']. Everything that removes the CV lives inside the try
     block below, so ANY exception in there - a null opportunity, a bad criteria field -
     fell back to this line and generated the machine-written CV again. The default no
     longer contains it, and a hard filter after the block makes it unreachable by any
     path. The applicant's own uploaded CV is the one that travels. */
  let plan = ['cover'];
  ...
  try {
    ...
    const needs = k => {
      ...
      /* A research application without a publication list is incomplete, and the reviewer
         reads it before the cover letter. Only produced when the applicant actually has
         published work on file - an empty list is worse than none. */
      if (k === 'publications') return (['phd', 'postdoc', 'fellowship'].includes(opp.level)
        || /research|postdoc|fellow|scientist|lecturer|faculty|phd/.test(blob))
        && ((p.publications || []).length > 0 || ((px2 && px2.research_papers) || []).length > 0);
    ...
    /* This line used to read `if (!plan.includes('cv')) plan.unshift('cv')`, which undid
       the filter three lines above and re-added the generated CV to every single case.
       That is why prepared cases kept showing a machine-written summary of the CV
       instead of the applicant's own uploaded file. The original is attached by the
       package builder; we never write a CV. */
    plan = plan.filter(k => k !== 'cv');
    if (!plan.includes('cover')) plan.unshift('cover');
    /* The statement of purpose and the letter of motivation are the same document
       under two names. Writing both doubled the wait and produced two overlapping
       letters the applicant then had to reconcile. Motivation wins; SOP is dropped. */
    if (plan.includes('motivation')) plan = plan.filter(k => k !== 'sop');
    // A visa summary belongs to a job move, not to a research fellowship application.
    if (opp.kind !== 'work') plan = plan.filter(k => k !== 'visa_summary');
    // A scholarship statement is for a scholarship, not for every funded position.
    if (opp.kind !== 'scholarship' && !/scholarship|bursary|financial need/.test(String(opp.title || '').toLowerCase())) plan = plan.filter(k => k !== 'scholarship_statement');
    ...
    /* PORTAL MEANS PORTAL. This used to require the opportunity to have NO contact email
       before it counted as portal-only, so any portal advert that happened to print a
       departmental address was pushed down the email route: we drafted a message nobody
       should send, and the applicant was left wondering which of the two applied. The
       advert's own stated route decides, and nothing else. */
    const portalOnly = (opp.apply_via === 'portal'); _portalOnly = portalOnly;
    if (portalOnly && !plan.includes('checklist')) plan.push('checklist');
  } catch (e) {}
  /* Final, unconditional guarantee: whatever happened above, we do not write a CV. */
  plan = plan.filter(k => k !== 'cv');
  if (!plan.includes('cover')) plan.unshift('cover');
  await setProg(appId, ['profile','recipient'], 'cv');
  /* SPEED. Documents were written strictly one after another, each one a long call with
     a quality re-run behind it. Eight documents in series is fifteen minutes, past the
     nine-minute stall guard, which is exactly why a healthy case reported that a step
     had hit a problem. They do not depend on each other, so they are written three at a
     time. A failure inside one batch no longer takes the rest down with it. */
  const BATCH = 3;
  for (let i = 0; i < plan.length; i += BATCH) {
    const slice = plan.slice(i, i + BATCH);
    await Promise.all(slice.map(k => mk(k, DOC_CATALOG[k][0], DOC_CATALOG[k][1]).catch(async err => {
      try { await require('./oblog').errlog('prepare:' + k, err, { userId: a.user_id, applicationId: appId }); } catch (e) {}
    })));
    const done = (i + BATCH) >= plan.length;
    await setProg(appId, done ? ['profile','recipient','cv','cover'] : ['profile','recipient','cv'], done ? 'email' : 'cover');
  }
  if (!plan.length) await setProg(appId, ['profile','recipient','cv','cover'], 'email');
  ...
  if (_portalOnly) {
    ...
    /* Every step is genuinely finished on a portal case. The old call marked a
       non-existent 'checklist' step done and left 'email' pending forever, so a
       completed portal case sat at 67% looking broken. There is no email to write. */
    await setProg(appId, PREP_STEPS.map(x => x[0]), null);
    try { await admin().from('applications').update({
      stage: 'portal_apply',
      next_action: 'This position is applied for on its official portal, so there is no email to send. Open the portal, keep your documents ready on your device, and upload them yourself; the assistant fills in your details for you.'
    }).eq('id', appId); } catch (e) {}
    return;
  }
...
async function draftMessage(appId) {
  ...
  if (!body) {
    ...
    L.push('');
    L.push('Kind regards,');
    L.push(p.full_name + ((p.apply_email || p.email) ? '\n' + (p.apply_email || p.email) : '') + (p.phone ? '\n' + p.phone : ''));
...
async function portalPack(appId) {
  ...
  const prof = { full_name: p.full_name, email: p.apply_email || p.email, phone: p.phone, country: p.origin_country || 'Pakistan', headline: p.headline, field: p.field, degree: p.degree_level, cgpa: p.cgpa, university: p.last_institution, experience_years: p.experience_years, language: p.language_scores };
...
module.exports = { synonymsFor, eligibilityFlag, discoverForUser, prepareApplication, draftMessage, cleanEmails, noSmell, parseJSON, ingestOpps, seedDiscovery, portalPack };
