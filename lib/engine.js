...
function searchStrategy() {
  ...
  return 'SEARCH STRATEGY, multi-source: (1) official university/employer admissions and careers pages on their own domains; (2) major funding and vacancy databases as discovery leads: DAAD, Campus France, Fulbright, Chevening, Commonwealth Scholarships, Erasmus Mundus catalogue, MEXT, China CSC, Korea GKS, Euraxess, ScholarshipPortal, FindAPhD, FindAMasters, jobs.ac.uk, Academic Positions, LinkedIn Jobs, Indeed; (3) public posts by professors, labs or departments on LinkedIn or X announcing openings, used ONLY as leads. EVERY fact must then be verified on the official institutional page before reporting, and the url you report must be that official page, openable by anyone, so the user can personally verify it. Never cite a portal or a social post as the source. FRESHNESS, today is ' + today + ': the deadline must be ' + today + ' or later, or the official page must literally state applications are open or rolling; reject expired, closed or unverifiable listings. Read pages with URL context, never rely on snippets. Report ONLY facts literally present on official pages, leave everything else empty.\n\n';
}
...
async function discoverForUser(userId, kind, prefs) {
  ...
  // Fresh inventory already covers the target within the requested scope: serve from DB, save the search cost.
  if (await freshCount(priority.length ? priority : null) >= target) return 0;
...
${p.headline || ''}; field ${p.field || ''}; skills ${String(p.methods || '').slice(0, 200)}. ${budgetLine} ${licenseLine} ${countryLine} ${remoteLine} ${uniLine}
...
CRITICAL: fill criteria ONLY from requirements literally stated on the official page. Leave any unstated requirement as empty; never guess a CGPA, language score, or restriction.`;
  const txt = await callAI('search_verify', searchStrategy() + prompt, { search: true, urls: true, maxTokens: 1800, userId });
  ...
  try { if (prefs.progressKey) await admin().from('app_settings').upsert({ key: prefs.progressKey, value: { status: 'running', startedAt: prefs.startedAt || new Date().toISOString(), kind, target, found: added } }); } catch (e) {}
...
async function seedDiscovery(kind, query, userId) {
  kind = kind === 'work' ? 'work' : (kind || 'masters');
  const prompt =
