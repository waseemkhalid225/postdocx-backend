...
async function discoverForUser(userId, kind) {
  try {
    ...
    const q = admin().from('opportunities').select('id', { count: 'exact', head: true }).eq('status', 'verified').gte('created_at', since);
    const { count } = await (kind ? q.eq('kind', kind) : q);
    if ((count || 0) >= 10) return 0; // fresh inventory exists — serve from DB, save the search cost
  } catch (e) {}
...
CRITICAL: fill criteria ONLY from requirements literally stated on the official page. Leave any unstated requirement as empty; never guess a CGPA, language score, or restriction.`;
  const txt = await callAI('search_verify', 'SEARCH STRATEGY: (1) official university/company domain first, (2) the department or careers page, (3) the scholarship/funding page, (4) cross-check the deadline on the official page itself. Read pages with URL context, never rely on snippets. Report ONLY facts literally present on official pages, leave everything else empty.\n\n' + prompt, { search: true, urls: true, maxTokens: 1800, userId });
  const items = parseJSON(txt) || [];
  return await ingestOpps(items, kind);
}

/* Shared ingest: validate, dedupe, and insert discovered opportunities. Used by
   per-user discovery AND admin corridor seeding. */
async function ingestOpps(items, kind) {
  let added = 0;
...
async function draftMessage(appId) {
  ...
  if (!a) return;
  const { data: p } = await admin().from('profiles').select('*').eq('id', a.user_id).single();
  try { await admin().from('applications').update({ prep_started_at: new Date().toISOString() }).eq('id', appId); } catch (e) {}
  await setProg(appId, ['profile'], 'recipient');
  const opp = a.opportunities;
...
CRITICAL: fill fields ONLY from facts literally stated on the official page; leave everything else empty.`;
  const txt = await callAI('search_verify', 'SEARCH STRATEGY: official domains first, read pages with URL context, cross-check deadlines. Never rely on snippets.\n\n' + prompt, { search: true, urls: true, maxTokens: 1800, userId });
  return await ingestOpps(parseJSON(txt) || [], kind);
}
