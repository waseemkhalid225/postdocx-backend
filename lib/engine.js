...
async function draftMessage(appId) {
  ...
  try {
    const txt = await callAI('high_value',
      `${STYLE}\n\nWrite a first-contact application email (150-200 words, MUST be under 1600 characters total) from ${p.full_name} (${p.headline || ''}, ${p.field || ''}) for: ${opp.title} at ${opp.institution}. Specific, warm, respectful; documents stated as attached; courteous close. Respond ONLY with JSON {"subject":"","body":""}`,
