...
async function backupSheetFallback() {
  // Quota-proof backup: dump every tab as JSON into the document storage backend.
  const db = require('./sheets');
  const storage = require('./storage');
  const dump = {};
  for (const tab of Object.keys(db.SCHEMA)) {
    try { dump[tab] = (await db.all(tab)).map(({ _row, ...o }) => o); } catch (e) { dump[tab] = []; }
  }
  const name = 'PostDocX-Backup-' + new Date().toISOString().slice(0, 10) + '.json';
  const f = await storage.put(name, 'application/json', Buffer.from(JSON.stringify(dump)));
  return name + ' (JSON, stored in document storage)';
}
...
async function backupSheet() {
  ...
  try {
    ...
    // Drive copy can fail on personal accounts (service-account quota policy): fall back to JSON dump
    try { return await backupSheetFallback(); } catch (e2) { throw new Error(explain(e) + ' | JSON fallback also failed: ' + e2.message); }
  }
}
