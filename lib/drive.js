...
function drive() {
  ...
  authClient = new JWT({ email, key, scopes: ['https://www.googleapis.com/auth/drive'] });
...
async function probe() {
  ...
  try {
    drive(); // init clients
    await authClient.authorize(); // token fetch only: isolates credential problems from API problems
    out.token = { ok: true, note: 'OAuth token obtained, credentials are valid' };
