...
async function vaultFor(userId) {
  const { data } = await admin().from('documents').select('id,name,kind,doc_type,doc_status,expiry_date,issue_date,issues,confidence,extracted,sensitive,attestation_status,compressed,created_at,read_at').eq('user_id', userId).eq('generated', false).order('created_at', { ascending: false });
...
async function checklist(userId, purpose, extraRequired) {
  ...
  const row = t => {
    ...
    if (d.doc_status === 'uploaded' || d.doc_status === 'reading') return { type: t, label: LABEL[t] || t, state: 'reading', document_id: d.id };
    return { type: t, label: LABEL[t] || t, state: 'ok', document_id: d.id, expiry_date: d.expiry_date };
  };
  const req = required.map(row), rec = (base.recommended || []).map(row);
...
module.exports = { DOC_TYPES, LABEL, CHECKLISTS, SENSITIVE, readDocument, vaultFor, checklist, mapKind };
