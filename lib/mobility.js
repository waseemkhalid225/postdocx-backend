...
/* PROGRESSIVE PROFILING (R12000): nothing is required to start matching; the CV is the source of truth and everything else is asked when a case needs it. */
const REQUIRED_FOR_MATCH = [];
...
async function get(userId) {
  const { data: p } = await admin().from('profiles').select('lane_pref,target_countries,mobility,mobility_provenance,mobility_updated_at,consent_vault_sensitive,full_name,nationality,field,total_experience_years,date_of_birth,city,passport_number,national_id,license_number,license_authority,profession,mobility_enc').eq('id', userId).maybeSingle();
  ...
  /* DATA-002/003 (Phase 3): the profile columns are canonical for lane and target countries; the mobility view mirrors them when it has no value. */
  try { if (p) { if (!m.lane && p.lane_pref) { m.lane = p.lane_pref; prov.lane = prov.lane || { source: 'profile' }; } if ((!Array.isArray(m.target_countries) || !m.target_countries.length) && Array.isArray(p.target_countries) && p.target_countries.length) { m.target_countries = p.target_countries.slice(); prov.target_countries = prov.target_countries || { source: 'profile' }; } } } catch (e) {}
...
async function update(userId, patch, source) {
  ...
  /* DATA-002/003: a mobility save writes the canonical columns too. */
  if (m.lane === 'work' || m.lane === 'study') upd.lane_pref = m.lane; if (Array.isArray(m.target_countries)) upd.target_countries = m.target_countries.map(x => String(x).toUpperCase()).slice(0, 10);
