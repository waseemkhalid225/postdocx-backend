...
const { sendAsUser } = require('./gmail');
...
/* ---------- SendAgent: dispatch authorized messages from each user's OWN Gmail ---------- */
async function sendAgent(transport) {
  const { data: msgs } = await admin().from('messages').select('*').eq('status', 'approved').eq('direction', 'outbound').limit(10);
  let sent = 0;
  for (const m of (msgs || [])) {
    if (!m.to_emails || !m.to_emails.length) continue;
    try {
      const { data: docs } = await admin().from('application_documents').select('*').eq('application_id', m.application_id);
      const { data: p } = await admin().from('profiles').select('full_name').eq('id', m.user_id).single();
      const attachments = [];
      for (const d of (docs || [])) {
        const nice = (d.kind === 'cv' ? 'CV - ' : d.kind === 'cover' ? 'Cover Letter - ' : d.title + ' - ') + ((p || {}).full_name || 'Applicant');
        attachments.push({ filename: nice + '.pdf', content: await textToPdf(d.title, d.content), contentType: 'application/pdf' });
      }
      const providerId = await sendAsUser(m.user_id, { to: m.to_emails, subject: m.subject, text: m.body, attachments }, transport);
      await admin().from('messages').update({ status: 'sent', sent_at: new Date().toISOString(), provider_id: String(providerId || '') }).eq('id', m.id);
      await admin().from('applications').update({ stage: 'submitted_email', next_action: 'Sent from your Gmail. Replies land in your inbox; follow-up handled automatically.', updated_at: new Date().toISOString() }).eq('id', m.application_id);
      await log('SEND_AGENT', 'sent ' + m.id + ' to ' + m.to_emails.join(','));
      sent++;
    } catch (e) {
      if (String(e.message) === 'GMAIL_NOT_CONNECTED') {
        await admin().from('applications').update({ next_action: 'Authorized and ready. Connect your Gmail in Profile so it sends from YOUR address.' }).eq('id', m.application_id);
      } else {
        await admin().from('messages').update({ status: 'failed' }).eq('id', m.id);
        await log('SEND_AGENT_FAIL', m.id + ': ' + e.message);
      }
    }
  }
  return sent;
