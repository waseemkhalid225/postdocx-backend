...
async function assess(userId, cc, routeKey) {
  ...
  const STATE_LABEL = { missing: 'missing', reading: 'awaiting verification', review: 'needs review', expired: 'expired', ok: 'verified' };
  const flagsFrom = (txt, val) => { const t = String(txt || '').toLowerCase(); const v = val || {}; return { translation: v.translation != null ? !!v.translation : /translat/.test(t), attestation: v.attestation != null ? !!v.attestation : /attest|legali[sz]/.test(t), apostille: v.apostille != null ? !!v.apostille : /apostille/.test(t), notarisation: v.notarisation != null ? !!v.notarisation : /notar/.test(t), certified_copy: v.certified_copy != null ? !!v.certified_copy : /certified (true )?cop/.test(t), original_required: v.original != null ? !!v.original : /original/.test(t) }; };
  const attestRules = rules.filter(r => r.rule_type === 'attestation');
  const required = docTypes.map(t => { const base = byType[t] || { type: t, label: VAULT.LABEL[t] || t, state: 'missing' }; const rule = rules.find(r => r.rule_type === 'document' && r.value && r.value.doc_type === t) || {}; const v = rule.value || {}; const cond = v.condition || (/^(if|only if|where applicable|when|unless)\b|\bif you (are|have|were|hold)\b|\bwhere applicable\b|\bonly (if|when)\b/i.test(String(rule.text || '')) ? String(rule.text).slice(0, 160) : null); const lf = flagsFrom(rule.text, v); const anyLeg = Object.values(lf).some(Boolean); const att = attestRules.length ? attestRules.map(a => ({ text: a.text, source_url: a.source_url })) : null;
    return Object.assign(base, { requirement: cond ? 'conditional' : (v.recommended ? 'recommended' : 'required'), condition: cond, state_label: STATE_LABEL[base.state] || base.state, rule_text: rule.text || null, source_url: rule.source_url || null, source_title: rule.source_title || null, rule_verified: rule.status === 'verified', legalisation: anyLeg ? lf : null, legalisation_note: anyLeg ? null : 'Translation, attestation, apostille or notarisation are not specified in the configured rule for this document; confirm on the official page before assuming.', origin_attestation: att, validity: v.validity || v.validity_note || null, issuing_authority: v.issuer || null, format: v.format || null }); });
  const flags = [];
  const p = mob.profile || {};
