...
async function createClient(orgId, ownerUserId, body) {
  ...
  // Link to an existing login when the email matches an account (consent stays with the client).
  if (row.email) {
    try { const { data: p } = await admin().from('profiles').select('id').ilike('email', row.email).maybeSingle(); if (p) row.user_id = p.id; } catch (e) {}
  }
...
async function listClients(orgId, { stage, q, limit, scope } = {}) {
  let query = admin().from('clients').select('id,full_name,email,phone,whatsapp,nationality,lane,stage,status,branch,owner_user_id,user_id,created_at,updated_at').eq('org_id', orgId).order('updated_at', { ascending: false }).limit(Math.min(200, limit || 100));
...
async function updateClient(orgId, clientId, patch) {
  const allowed = {};
  for (const k of ['full_name', 'email', 'phone', 'whatsapp', 'nationality', 'lane', 'stage', 'status', 'profile', 'owner_user_id', 'branch']) if (patch[k] !== undefined) allowed[k] = patch[k];
