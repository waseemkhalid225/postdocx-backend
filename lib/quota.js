...
const GRACE_DAYS = 3;
/* An active plan counts until period_end, then for GRACE_DAYS more (the consultancy keeps working while it renews); the state tells the app which. */
async function activeSub(orgId) { const now = new Date(); const { data } = await admin().from('org_subscriptions').select('*').eq('org_id', orgId).eq('status', 'active').gt('period_end', new Date(now.getTime() - GRACE_DAYS * 86400000).toISOString()).order('period_end', { ascending: false }).limit(1); return data && data[0] || null; }
...
const TRIAL_DAYS = 14;
/* TWO-WEEK TRIAL: a consultancy created less than 14 days ago and without a paid plan runs on the Starter limits. The app shows the
   end date in the workspace; after that, Billing. White label and the API need a paid Growth or Scale plan. */
async function trialSub(orgId) { try { const { data: o } = await admin().from('organisations').select('created_at,kind').eq('id', orgId).maybeSingle(); if (!o || !['agency', 'institution', 'employer', 'partner'].includes(o.kind)) return null; const end = new Date(new Date(o.created_at).getTime() + TRIAL_DAYS * 86400000); if (end < new Date()) return null; const cfg = await require('./settings').getConfig(); const t = ((cfg.agency || {}).tiers || [])[0] || { key: 'starter', name: 'Starter 100', cases_month: 100, searches_day: 100, searches_month: 2000, seats: 5 }; return { org_id: orgId, tier_key: t.key, tier_name: t.name + ' (trial)', cases_month: t.cases_month, searches_day: t.searches_day, searches_month: t.searches_month, seats: t.seats, status: 'trial', period_start: o.created_at, period_end: end.toISOString(), cases_used: 0, trial: true, trial_ends: end.toISOString().slice(0, 10), white_label: false, api: false }; } catch (e) { return null; } }
async function limitsFor(orgId, member) {
  let sub = await activeSub(orgId); if (sub && new Date(sub.period_end) < new Date()) { sub = Object.assign({}, sub, { in_grace: true, grace_until: new Date(new Date(sub.period_end).getTime() + GRACE_DAYS * 86400000).toISOString(), days_left: 0 }); } else if (sub) { sub = Object.assign({}, sub, { days_left: Math.ceil((new Date(sub.period_end) - new Date()) / 86400000) }); } if (!sub) sub = await trialSub(orgId); if (!sub) return { ok: false, reason: 'Your two-week trial has ended. Choose a plan under Billing to continue.', code: 'NO_PLAN' };
...
module.exports = { trialSub, TRIAL_DAYS, GRACE_DAYS, activeSub, allocations, limitsFor, check, consume, usage };
