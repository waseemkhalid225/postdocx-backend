...
async function migrateNaming() {
  try {
    ...
    for (const d of docs) {
      const n = d.name || '';
      const nn = n.replace(/^([A-Za-z]+_)+[A-Za-z]+_\d{8}\.pdf$/, '').trim();
      // old machine-named files keep working; only titles above matter for display
      if (false && nn !== n) { d._row.set('name', nn); await d._row.save(); }
    }
