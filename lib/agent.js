...
async function backfillPreparedCases(limit) {
  ...
  for (const o of opps) {
    ...
    if (!hasDraft.has(o.id)) {
      try { await draftEmailForCase(o.id); done++; } catch (e) { await db.log('BACKFILL_FAIL', (o.institution || '') + ': ' + e.message); }
    }
  }
  ...
  if (!opp.contact) {
    try {
      ...
      const cleanE = arr => [...new Set((arr || [])
        .map(e => String(e || '').replace(/^mailto:/i, '').trim().toLowerCase())
