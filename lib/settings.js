...
const DEFAULTS = {
  ...
  /* FF-CRM · fixed platform plans (R14000, BYOC): the consultancy pays the platform; searches and applications are not capped by FF; the
     consultancy connects its own AI account and pays its provider directly. Higher plans buy staff, branches and workflow, not AI. */
  agency: {
    tiers: [
      { key: 'starter', name: 'Starter', usd_month: 79, usd_year: 790, seats: 5, branches: 1, sub_agents: 5, white_label: false, api: false, cases_month: null, searches_day: null, searches_month: null, features: ['Clients, pipeline, documents, visa desk', 'Client mailbox and replies', 'Finance and P&L'] },
      { key: 'professional', name: 'Professional', usd_month: 179, usd_year: 1790, seats: 15, branches: 3, sub_agents: 30, white_label: true, api: false, cases_month: null, searches_day: null, searches_month: null, featured: true, features: ['Everything in Starter', 'Work queues, follow-up sequences, broadcasts', 'Branch reporting, cohorts, white label'] },
      { key: 'business', name: 'Business', usd_month: 399, usd_year: 3990, seats: 40, branches: 10, sub_agents: 200, white_label: true, api: true, cases_month: null, searches_day: null, searches_month: null, features: ['Everything in Professional', 'Multi-branch controls and permissions', 'API keys, webhooks, integrations'] }
    ],
    overage_usd_per_case: 0,
  ...
  prospecting: { auto: false, daily_cap: 40, trial_days: 15, signer: 'Partnerships Office, ForiForeign (Private) Limited', targets: [{ kind: 'university', country_code: 'GB' }, { kind: 'agency', country_code: 'PK', city: 'Lahore' }, { kind: 'agency', country_code: 'IN', city: 'Delhi' }] },
  ...
  /* PLUS: after an offer, one purchase covers the rest of the journey (offer pack, visa desk, interview pack, arrival pack, residence year). No step-by-step add-ons. */
  plus: { tiers: [{ key: 'o2s', name: 'O₂S', long: 'Offer letter to settle and get PR abroad', usd: 100, from: 'any' }], includes: ['offer_pack', 'visa_desk', 'interview_pack', 'arrival_pack', 'residence_year'] },
  /* PATHWAY MEMBERSHIP: free discovery and readiness for everyone; a low monthly or yearly membership for the personal manager. Upsell only after value appears. */
  pathway: { month_usd: 9, year_usd: 79, free_days_before_upsell: 7, upsell_min_score: 40 },
  /* per-user daily caps on paid capabilities, enforced by overCap() */
  security: { totp_required: false, admin_ip_allowlist: [] },
  tax: { default_pct: 0, by_country: {} },   /* invoices: VAT/GST per country code, e.g. { GB: 20, AE: 5 } */
  caps: {},   /* 30-day caps per capability; empty = code defaults (CAPS) */
  ...
  payment: {
    bank_name: '', account_title: 'ForiForeign (Private) Limited', account_number: '', iban: '',
  ...
  ai: { pii_to_model: true, pii_note: 'Documents go to the model provider for extraction under its data-processing terms; national IDs, card and account numbers are masked in text; PDFs/images are sent as-is.', usd_to_pkr: 278, pkr_override_per_case: '' },
