...
async function openaiPlain(purpose, prompt, opts = {}) {
  ...
  const r = await fetch('https://api.openai.com/v1/responses', {
    method: 'POST', headers: { 'content-type': 'application/json', authorization: 'Bearer ' + require('./ai_keys').keyFor('openai') },
  ...
  try { await admin().from('ai_cost_ledger').insert({ org_id: (opts.orgId || require('./ai_keys').tenant() || null), user_id: opts.userId || null, application_id: opts.applicationId || null, provider: 'openai', model: bm, purpose, input_tokens: (d.usage || {}).input_tokens || 0, output_tokens: (d.usage || {}).output_tokens || 0 }); } catch (e) {}
  return t2;
}
async function callAI(purpose, prompt, opts = {}) {
  /* BYOC (per-consultancy AI account): resolve the tenant once; inside runAs() the provider modules see only that tenant's keys. */
  const K = require('./ai_keys');
  if (!K.tenant() && !opts._tenantResolved) { const orgId = opts.orgId || (opts.userId ? await K.tenantOfUser(opts.userId) : null); if (orgId) { const keys = await K.loadKeys(orgId); const need = (REGISTRY[purpose] && REGISTRY[purpose].provider === 'anthropic' && !opts.search && !opts.urls) ? 'anthropic' : 'gemini'; if (!keys[need] && !keys.gemini) { await K.recordHealth(orgId, 'not_connected', 'no ' + need + ' key'); throw new K.NotConnected(orgId, need); } try { const out = await K.runAs(orgId, () => callAI(purpose, prompt, Object.assign({}, opts, { _tenantResolved: true, orgId }))); K.recordHealth(orgId, 'healthy', purpose).catch(() => {}); return out; } catch (e) { const h = K.classify(e); if (h) await K.recordHealth(orgId, h, e.message); throw e; } } }
  ...
  if (r.provider === 'anthropic' && require('./ai_keys').keyFor('anthropic') && !opts.search && !opts.urls) {
    try {
      const { anthropicCall } = require('./anthropic');
      const a = await anthropicCall(prompt, { maxTokens: opts.maxTokens || 3000, model: r.model });
      try {
        await admin().from('ai_cost_ledger').insert({ org_id: (opts.orgId || require('./ai_keys').tenant() || null),
      ...
      if (require('./ai_keys').keyFor('openai')) {
  ...
  try {
    ...
    if (!opts.search && !opts.urls && require('./ai_keys').keyFor('openai')) {
