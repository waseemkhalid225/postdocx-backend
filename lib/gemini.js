...
async function geminiCall(prompt, { maxTokens = 1200, thinking = 'medium', search = false, urls = false, json = false } = {}) {
  ...
  // Reliability: adaptive timeout per attempt, 3 attempts total with backoff on 429/5xx/network.
  // Grounded calls (Google Search / URL Context) legitimately take 60-90s+; plain calls stay at 45s.
  const tmoMs = (search || urls) ? 120000 : 45000;
  let res, d, lastErr;
  for (let attempt = 0; attempt < 3; attempt++) {
    const useModel = attempt === 2 ? FALLBACK() : MODEL(); // final attempt: stable fallback model
    try {
