...
async function list(prefix) {
  if (BACKEND === 'supabase') {
    ...
    return items.filter(x => !prefix || x.name.includes(prefix)).map(x => ({ id: 'sb:' + x.name, name: x.name }));
  }
  try {
    await ensureDir();
    const files = await fsp.readdir(DIR);
    return files.filter(f => !prefix || f.includes(prefix)).map(f => ({ id: 'fs:' + f, name: f }));
  } catch (e) { return []; }
}

module.exports = { put, get, remove, isStorageId, probe, list };
