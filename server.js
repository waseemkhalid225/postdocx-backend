...
const FF_BUILD = '2026-09-05-R10300';
...
app.post('/api/addons/checkout', auth, async (req, res) => {
  try { const key = String((req.body || {}).addon || ''); const cfg = await siteSettings.getConfig(); const price = (cfg.addons || {})[key + '_usd']; if (/_plus$/.test(key)) { const t = ((cfg.plus || {}).tiers || []).find(x => x.key === key); price = t ? t.usd : 0; } if (!price) return res.status(400).json({ error: 'Unknown add-on' });
...
app.get('/api/me/offers-for-me', auth, async (req, res) => {
  try { const cfg = await siteSettings.getConfig(); const A = cfg.addons || {}; const { count: paid } = await admin().from('payments').select('id', { count: 'exact', head: true }).eq('user_id', req.userId).eq('status', 'confirmed'); const bal = await balance(req.userId); const { data: pn } = await admin().from('profiles').select('journey_stage,free_searches_used').eq('id', req.userId).maybeSingle(); const fu = cfg.fair_use || {};
    ...
    let plusItem = null; try { const { count: offers } = await admin().from('offers').select('id', { count: 'exact', head: true }).eq('user_id', req.userId); const { data: owned } = await admin().from('user_addons').select('bundle,addon_key').eq('user_id', req.userId); const hasPlus = (owned || []).some(x => x.bundle); if (((offers || 0) > 0 || ['offer', 'visa', 'travel', 'arrive', 'settle', 'pr'].includes(stage)) && !hasPlus && paid) { const { data: lastPay } = await admin().from('payments').select('credits').eq('user_id', req.userId).eq('status', 'confirmed').gt('credits', 0).order('created_at', { ascending: false }).limit(1); const tier = lastPay && lastPay[0] ? (((cfg.packages || {}).tiers || []).find(t => Number(t.credits) === Number(lastPay[0].credits)) || {}).key : 'basic'; const pt = (((cfg.plus || {}).tiers) || []).find(t => t.from === tier) || (((cfg.plus || {}).tiers) || [])[0]; if (pt) plusItem = { key: pt.key, title: pt.name + ' · everything after the offer', why: 'One payment. Offer pack, visa desk (files, pre-fill, check-ins, decision), interview pack, arrival pack and a year of residence support. Nothing else to buy on this journey.', price: '$' + pt.usd, plus: true }; } } catch (e) {}
...
function visitorCountry(req) { const h = req.headers; const cc = String(h['cf-ipcountry'] || h['x-vercel-ip-country'] || h['x-country-code'] || h['x-appengine-country'] || '').toUpperCase(); if (cc && cc.length === 2 && cc !== 'XX') return cc; const q = String(req.query.cc || '').toUpperCase(); if (/^[A-Z]{2}$/.test(q)) return q; const al = String(h['accept-language'] || ''); const m = al.match(/-([A-Z]{2})/); if (m) return m[1]; return 'PK'; }
app.get('/api/local-price', async (req, res) => { try { const usd = Number(req.query.usd) || 0; const cc = visitorCountry(req); const fx = await require('./lib/pay').liveRates().catch(() => null); const W = require('./lib/world'); const lp = W.localPrice(usd, cc, fx && fx.rates); res.set('Cache-Control', 'private, max-age=600'); res.json({ cc, local: lp, as_of: fx && fx.at ? new Date(fx.at).toISOString().slice(0, 10) : null }); } catch (e) { res.status(400).json({ error: e.message }); } });
app.get('/api/visitor', (req, res) => { res.set('Cache-Control', 'private, max-age=600'); res.json({ cc: visitorCountry(req) }); });
