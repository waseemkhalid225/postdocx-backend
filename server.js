...
const app = express();
app.use((req, res, next) => { res.set('Referrer-Policy', 'strict-origin-when-cross-origin'); res.set('Permissions-Policy', 'camera=(self), microphone=(), geolocation=(), payment=(self)');
  /* SEC-002 (Phase 1): enforced CSP. Inline scripts are unavoidable in the single-file app, so script-src keeps 'unsafe-inline'; everything else is strict: no plugins, no framing except our own pages, no base tag hijack, forms only to us, connections only to us, Supabase and the payment providers. */
  const csp = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: blob: https:; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://api.lemonsqueezy.com https://*.paddle.com https://ipapi.co; frame-src 'self' https://*.lemonsqueezy.com https://*.paddle.com; frame-ancestors 'self'; object-src 'none'; base-uri 'self'; form-action 'self' https://*.lemonsqueezy.com https://*.paddle.com; upgrade-insecure-requests";
  res.set('Content-Security-Policy', csp); next(); });
...
const FF_BUILD = '2026-09-06-R14100';
...
/* (security headers moved to the head of the chain in Phase 1: see below the app creation) */
...
app.post('/api/pay/lemon/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try { const L = require('./lib/gateway_lemon'); const raw = req.body instanceof Buffer ? req.body.toString('utf8') : String(req.body || '');
    ...
    /* Phase 7 BUG-014: consultancy plan purchases arrived here as 'order_created' too and were settled as credit purchases (1 credit to the owner, plan never activated, no invoice). Route by the same metadata rule as the Stripe path. */
    if (name === 'order_created') { const so = L.sessionFromEvent(evt); const md = so.metadata || {}; return res.json((md.org_id && md.credits === '0') ? await settleAgencySubscription(so) : await settleCardPayment(so, 'lemon')); }
...
const UPLOAD_OK = /\.(pdf|doc|docx|jpg|jpeg|png|webp|heic|txt|rtf)$/i; const UPLOAD_MIME = /^(application\/pdf|application\/msword|application\/vnd\.openxmlformats-officedocument\.wordprocessingml\.document|image\/(jpeg|png|webp|heic|heif)|text\/plain|text\/rtf|application\/rtf|application\/octet-stream)$/i;
const up = multer({ storage: multer.memoryStorage(), limits: { fileSize: 15 * 1024 * 1024, files: 20 }, fileFilter: (req, file, cb) => { if (UPLOAD_OK.test(file.originalname || '') && UPLOAD_MIME.test(file.mimetype || '')) return cb(null, true); const e = new Error('Only PDF, Word, text and image files are accepted (' + String(file.originalname || '').slice(0, 60) + ')'); e.code = 'UNSUPPORTED_TYPE'; cb(e); } });
...
(() => { try { const C = require('./lib/crypto'); if (!process.env.FF_DATA_KEY) console.error('[SECURITY] FF_DATA_KEY is not set: portal credentials cannot be stored (secrets refused, not stored in plain text).'); else if (!C.enabled || !C.enabled()) console.error('[SECURITY] FF_DATA_KEY is set but not a 32-byte hex/base64 key.'); } catch (e) {} })();
(() => { const prodish = process.env.NODE_ENV === 'production' || /foriforeign\.com/i.test(String(process.env.PUBLIC_URL || '')); if (prodish && (process.env.FF_MEMDB === '1' || process.env.FF_FAKE_MAIL || process.env.FF_FAKE_AI_FILE)) { console.error('[SECURITY] Test-mode flags (FF_MEMDB / FF_FAKE_MAIL / FF_FAKE_AI_FILE) are set on a production configuration. Refusing to start.'); process.exit(1); } })();
...
async function auth(req, res, next) {
  ...
  /* SEC-006 (Phase 1): the role is part of the request identity. It was only set by perm()/staffOnly, so every plain-auth route that
     branches on req.userRole (staff case view, fair-use bypass, checkout exception, full checklists) saw undefined. Loaded once per
     request from a short cache; a role change invalidates it through role_changed_at below. */
  try { const cached = _roleCache.get(u.id); if (cached && Date.now() - cached.at < 60000) req.userRole = cached.role; else { const { data: pr0 } = await admin().from('profiles').select('role').eq('id', u.id).maybeSingle(); req.userRole = (pr0 && pr0.role) || 'user'; _roleCache.set(u.id, { role: req.userRole, at: Date.now() }); if (_roleCache.size > 5000) _roleCache.clear(); } } catch (e) { req.userRole = req.userRole || 'user'; }
...
const perm = (p) => { const inner = requirePermission(p, admin); return async (req, res, next) => { try { const cfg = await siteSettings.getConfig(); if (cfg.security && cfg.security.totp_required && !/^\/api\/admin\/totp\//.test(req.path)) { const { data: pr } = await admin().from('profiles').select('totp_enabled').eq('id', req.userId).maybeSingle(); if (!(pr && pr.totp_enabled)) return res.status(403).json({ error: 'Two-factor authentication is required for staff. Set it up under Admin → Security.', code: 'TOTP_REQUIRED' }); } } catch (e) {} return _permInner(p, inner)(req, res, next); }; };
const _permInner = (p, inner) => { return async (req, res, next) => { try { const ok = await TOTP.sessionOk(req.userId, String(req.headers['x-ff-totp'] || req.query.totp || '')); if (!ok) return res.status(428).json({ error: 'Second factor required', totp_required: true }); } catch (e) {} return inner(req, res, next); }; };
...
app.post('/api/org/:id/clients/:cid/discover', auth, async (req, res) => {
  try {
    ...
    const kind = (req.body || {}).kind || (c.lane === 'work' ? 'work' : 'postdoc'); if (!(await searchGuard(req, res, kind, (req.body || {}).prefs || {}))) return;
...
app.get('/api/vault/checklist', auth, async (req, res) => {
  try { const extra = String(req.query.extra || '').split(',').map(x => x.trim()).filter(Boolean); const lane = String(req.query.for || 'study'); let purpose = lane; let ctx = {};
    /* PROGRESSIVE: before a case exists only the CV is asked; once a case is open the posting's own requirements decide; at visa stage the route's documents. Staff, and a caller who passes stage=full, get the complete list. */
    const { data: p } = await admin().from('profiles').select('journey_stage,lane_pref').eq('id', req.userId).maybeSingle(); const stage = String(req.query.stage || (p && p.journey_stage) || 'discover'); const isStaff = STAFF_ROLES.includes(req.userRole);
    if (stage !== 'full' && !isStaff) { if (['discover', 'qualify', 'match'].includes(stage)) purpose = 'discover'; else if (['visa', 'travel'].includes(stage)) purpose = lane === 'work' ? 'visa_work' : 'visa'; else if (['arrive', 'settle', 'pr'].includes(stage)) purpose = 'family'; else purpose = lane; }
    if (req.query.case) { try { const { data: a } = await admin().from('applications').select('id,opportunities(requirements,description,kind,category)').eq('id', String(req.query.case)).eq('user_id', req.userId).maybeSingle(); if (a && a.opportunities) { ctx.requirements_text = String(a.opportunities.requirements || '') + ' ' + String(a.opportunities.description || '').slice(0, 2000); if (purpose === 'discover') purpose = a.opportunities.kind === 'work' ? 'work' : 'study'; } } catch (e) {} }
    else if (purpose !== 'discover' && purpose !== 'family' && !/visa/.test(purpose)) { try { const { data: apps } = await admin().from('applications').select('opportunities(requirements,description)').eq('user_id', req.userId).in('status', ['draft', 'prepared', 'ready', 'sent', 'submitted', 'interview', 'offer']).order('updated_at', { ascending: false }).limit(3); ctx.requirements_text = (apps || []).map(a => a.opportunities ? String(a.opportunities.requirements || '') + ' ' + String(a.opportunities.description || '').slice(0, 800) : '').join(' '); } catch (e) {} }
    const ck = await VAULT.checklist(req.userId, purpose, extra, ctx); ck.stage = stage; ck.note = purpose === 'discover' ? 'Only your CV is needed now. Documents are asked for when a case needs them.' : purpose === 'visa' || purpose === 'visa_work' ? 'Your visa route asks for these; each one says why and who asks.' : purpose === 'family' ? 'Arrival and family documents for this stage.' : 'What your open case asks for; each one says why and who asks.'; res.json(ck); } catch (e) { res.status(400).json({ error: e.message }); }
});
...
app.post('/api/pay/checkout', auth, async (req, res) => {
  ...
  try { const cl = await consultancyClients(req.userId, 'id,org_id'); if (cl && cl.length && !STAFF_ROLES.includes(req.userRole)) return res.status(403).json({ error: 'Your cases are handled by your consultancy; there is nothing to buy here.', code: 'CONSULTANCY_CLIENT' }); } catch (e) {}
  try {
    ...
    const intentOpp = String((req.body || {}).opportunity_id || (req.body || {}).opportunityId || '').slice(0, 64) || null;
    const { data: pay, error } = await admin().from('payments').insert({ intent: intentOpp ? { opportunity_id: intentOpp } : null, user_id: req.userId, credits: q.credits, amount_pkr: q.amount_local && q.local_currency === 'PKR' ? q.amount_local : 0, status: 'pending', reference: 'CARD', discount_pkr: 0 }).select('id').single();
    if (error) return res.status(400).json({ error: error.message });
...
/* DISC-003 (Phase 7): a confirmed payment opens the case the person was buying it for — exactly once. Uses the same createApplication
   path as the button (duplicate guard, entitlement, preparation), so a repeated webhook or a manual confirm never opens a second case. */
async function settleIntoCase(p) { const opp = p && p.intent && p.intent.opportunity_id; if (!opp || p.case_id) return null; let out = null; const fakeRes = { status(c) { this._c = c; return this; }, json(b) { this._b = b; return this; } }; await createApplication({ userId: p.user_id, userRole: 'user', body: { opportunity_id: opp, from_payment: p.id }, headers: {}, ip: '', path: '/api/applications' }, fakeRes); const b = fakeRes._b || {}; const app = b.application || b.app || (b.applications && b.applications[0]); if (app && app.id) { out = app.id; await admin().from('payments').update({ case_id: app.id }).eq('id', p.id); try { await require('./lib/notify').push(p.user_id, 'case', 'Your case is open', 'Payment confirmed; we are preparing your application.', 'apps'); } catch (e) {} } return out; }
/* Pending card payments older than 7 days are marked ABANDONED (recoverable: a late webhook still settles; the person can start again). */
async function sweepAbandonedPayments() { const cutoff = new Date(Date.now() - 7 * 86400000).toISOString(); const { data } = await admin().from('payments').update({ status: 'abandoned', abandoned_at: new Date().toISOString() }).eq('status', 'pending').is('proof_path', null).lt('created_at', cutoff).select('id'); return { abandoned: (data || []).length }; }
async function expandBundle(p) {
  if (p.addon_key && (p.addon_key === 'o2s' || /_plus$/.test(p.addon_key))) { const { data: done } = await admin().from('user_addons').select('id').eq('payment_id', p.id).eq('bundle', p.addon_key).limit(1); if (done && done.length) return; try { const cfg = await siteSettings.getConfig(); const inc = ((cfg.plus || {}).includes) || ['offer_pack', 'visa_desk', 'interview_pack', 'arrival_pack', 'residence_year']; for (const k of inc) await admin().from('user_addons').insert({ user_id: p.user_id, addon_key: k, payment_id: p.id, bundle: p.addon_key, expires_at: k === 'residence_year' ? new Date(Date.now() + 365 * 86400000).toISOString() : null }).then(() => {}, () => {}); } catch (e) {} }
}
async function grantAddon(p) { const { data: ex } = await admin().from('user_addons').select('id').eq('payment_id', p.id).limit(1); if (ex && ex.length) return { already: true }; const days = p.addon_key === 'residence_year' ? 365 : p.addon_key === 'pathway_month' ? 31 : p.addon_key === 'pathway_year' ? 366 : null; await admin().from('user_addons').insert({ user_id: p.user_id, addon_key: p.addon_key, payment_id: p.id, expires_at: days ? new Date(Date.now() + days * 86400000).toISOString() : null }).then(() => {}, () => {}); if (/^pathway_/.test(p.addon_key)) { try { await require('./lib/pathway').detect(p.user_id, { notify: false }); await require('./lib/notify').push(p.user_id, 'pathway', 'Pathway Membership is on', 'Your pathway is now reassessed every week; alerts are immediate; ask a human any time from the card.', 'profile'); } catch (e) {} } return { ok: true }; }
async function settleCardPayment(session, source) {
  ...
  const { data: flipped } = await admin().from('payments').update({ status: 'confirmed', confirmed_at: new Date().toISOString() }).eq('id', p.id).in('status', ['pending', 'abandoned']).select('id');
  if (!flipped || !flipped.length) return { ok: true, already: true };
  /* Phase 7 BUG-011: a card-paid ADD-ON (O₂S, Pathway Membership, residence year) used to fall through the credit path (granting 1 credit and no add-on). Add-ons are granted here, exactly like the bank-transfer confirm path, and grant no credits. */
  if (p.addon_key) { await grantAddon(p); await expandBundle(p); try { await admin().from('audit_log').insert({ actor: p.user_id, event: 'PAYMENT_CONFIRMED', detail: p.id + ' addon ' + p.addon_key + ' card/' + source }); } catch (e) {} return { ok: true, addon: p.addon_key }; }
  ...
  /* DISC-003 (Phase 7): the package was bought to apply for a specific opportunity (payments.intent.opportunity_id, recorded at checkout).
     Open that case now, exactly once: an existing case for user × opportunity is reused; one credit is consumed through the ledger like any Start Case. */
  try { const oppId = p.intent && p.intent.opportunity_id; if (oppId && creditsN > 0) { const { data: ex } = await admin().from('applications').select('id').eq('user_id', p.user_id).eq('opportunity_id', oppId).limit(1); if (!(ex && ex.length)) { const { data: opp } = await admin().from('opportunities').select('id,institution,title').eq('id', oppId).maybeSingle(); if (opp) { const w = await ledgerWrite({ user_id: p.user_id, delta: -1, reason: 'consume', note: opp.institution, payment_id: p.id }); if (!w || !w.error) { const caseNo = 'FF-' + Date.now().toString(36).toUpperCase(); const { data: appRow, error: aErr } = await admin().from('applications').insert({ user_id: p.user_id, opportunity_id: opp.id, case_no: caseNo, stage: 'preparing', credits_consumed: 1, opened_by: 'payment' }).select('id').single(); if (aErr) { try { await ledgerWrite({ user_id: p.user_id, delta: 1, reason: 'refund', note: 'Case could not be opened after payment', payment_id: p.id }); } catch (e) {} } else { try { await admin().from('credit_ledger').update({ application_id: appRow.id }).eq('user_id', p.user_id).eq('reason', 'consume').eq('payment_id', p.id).is('application_id', null); } catch (e) {} try { await require('./lib/notify').push(p.user_id, 'case', 'Payment received; your case is being prepared', (opp.title || '') + ' · ' + (opp.institution || ''), 'apps'); } catch (e) {} try { JE.recompute(p.user_id); } catch (e) {} } } } } } } catch (e) {}
  if (led.error) { await admin().from('payments').update({ status: 'pending' }).eq('id', p.id); return { ok: false, reason: 'ledger' }; }
  try { await admin().from('payments').update({ settled_source: source || null }).eq('id', p.id); } catch (e) {}
  try { await settleIntoCase(p); } catch (e) { try { require('./lib/oblog').errlog('pay:case', e, { userId: p.user_id }); } catch (x) {} }
...
async function settleAgencySubscription(session) {
  ...
  try { await admin().from('org_subscriptions').update({ status: 'expired', updated_at: new Date().toISOString() }).eq('org_id', sub.org_id).eq('status', 'active').neq('id', sub.id); } catch (e) {}   /* Phase 7: one active subscription per organisation; the previous period is closed on renewal */
...
app.post('/api/pay/lemon/checkout', auth, async (req, res) => {
  try { if (!LEMON.enabled()) return res.status(400).json({ error: 'Lemon Squeezy is not configured.' }); const credits = Number((req.body || {}).credits); const q = await usdQuote(req.userId, credits); if (!q) return res.status(400).json({ error: 'Choose a package first.' });
    ...
    const { data: pay, error } = await admin().from('payments').insert({ user_id: req.userId, credits: q.credits, amount_pkr: 0, status: 'pending', reference: 'LEMON', provider: 'lemonsqueezy', intent: ((req.body || {}).opportunity_id || (req.body || {}).opportunityId) ? { opportunity_id: String((req.body || {}).opportunity_id || (req.body || {}).opportunityId).slice(0, 64) } : null }).select('id').single(); if (error) return res.status(400).json({ error: error.message });
    const origin = (req.headers.origin || ('https://' + req.headers.host));
...
app.get('/api/explore', auth, async (req, res) => {
  try { const q = Object.assign({}, req.query || {}); if (q.profession) { const prof = String(q.profession).slice(0, 60); const D = require('./lib/domains'); const fam = D.familyOfTitle(prof); const syn = fam && D.FAMILIES[fam] ? [fam, ...(D.FAMILIES[fam].syn || [])] : [prof]; q._profFilter = syn.map(x => String(x).toLowerCase()); q._profLabel = prof; }
    ...
    /* Phase 4 (DISC-002): legacy duplicates (same canonical URL or fingerprint) collapse to the freshest row. */
    try { const K = require('./lib/oppkey'); const seen = new Set(); r.rows = (r.rows || []).filter(o => { const keys = [o.url ? K.canonicalUrl(o.url) : null, o.fingerprint || (o.institution && o.title ? K.fingerprintOf(o) : null)].filter(Boolean); if (keys.some(k => seen.has(k))) return false; keys.forEach(k => seen.add(k)); return true; }); } catch (e) {}
    ...
    /* BUG-005 (Phase 2): a dismissed posting stayed visible in Explore. Hidden here, server-side, like the matches sheet. */
    try { const { data: dm } = await admin().from('app_settings').select('value').eq('key', 'dismissed:' + req.userId).maybeSingle(); const hide = new Set(((dm && dm.value && dm.value.ids) || []).map(String)); if (hide.size) r.rows = (r.rows || []).filter(o => !hide.has(String(o.id))); } catch (e) {}
...
app.post('/api/org/:id/clients/:cid/finance', auth, async (req, res) => { try { const c = await orgClient(req, res, 'finance.write'); const b = req.body || {}; if (!['fee_charged', 'payment_received', 'refund', 'cost', 'commission_in', 'commission_out', 'adjustment'].includes(b.kind) || !isFinite(Number(b.amount))) return res.status(400).json({ error: 'kind and amount required' }); const { data, error } = await admin().from('client_finance').insert({ org_id: c.org_id, client_id: c.id, kind: b.kind, amount: Number(b.amount), currency: String(b.currency || 'USD').toUpperCase().slice(0, 3), note: String(b.note || '').slice(0, 300) || null, reference: String(b.reference || '').slice(0, 120) || null, occurred_on: /^\d{4}-\d{2}-\d{2}$/.test(String(b.occurred_on || '')) ? b.occurred_on : new Date().toISOString().slice(0, 10),   /* Phase 8 BUG-013: undated money never reached the P&L */ created_by: req.userId }).select('*').single(); if (error) return res.status(400).json({ error: error.message }); await orgAudit(c.org_id, req.userId, 'FINANCE_' + b.kind.toUpperCase(), c.full_name + ' ' + b.amount + ' ' + (b.currency || 'USD')); res.json({ row: data }); } catch (e) { orgErr(res, e); } });
...
app.post('/api/payments/:id/refund', auth, perm('payments.write'), async (req, res) => {
  try { const { data: p } = await admin().from('payments').select('*').eq('id', req.params.id).maybeSingle(); if (!p) return res.status(404).json({ error: 'Not found' }); if (p.status !== 'confirmed') return res.status(400).json({ error: 'Only confirmed payments can be refunded' }); if (p.refunded_at) return res.status(400).json({ error: 'Already refunded' });
    ...
    await admin().from('payments').update({ status: 'refunded', refunded_at: new Date().toISOString(), refund_ref: ref || (String(p.reference || '').startsWith('CARD:') ? null : 'manual') }).eq('id', p.id);
...
app.post('/api/pay/pk/checkout', auth, async (req, res) => {
  try { if (!SAFEPAY.enabled()) return res.status(400).json({ error: 'Local PKR card payments are not switched on yet; use bank transfer with a screenshot.' });
    ...
    const { data: pay, error } = await admin().from('payments').insert({ user_id: req.userId, credits, amount_pkr: amount, status: 'pending', reference: 'SAFEPAY', provider: 'safepay', intent: ((req.body || {}).opportunity_id || (req.body || {}).opportunityId) ? { opportunity_id: String((req.body || {}).opportunity_id || (req.body || {}).opportunityId).slice(0, 64) } : null }).select('id').single(); if (error) return res.status(400).json({ error: error.message });
...
app.get('/api/org/:id/quota', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'clients.read'); const m = await ORGS.membership(req.params.id, req.userId); const L = await QUOTA.limitsFor(req.params.id, m); const u = await QUOTA.usage(req.params.id); res.json({ plan: L.ok ? { tier: L.sub.tier_name, cases_month: L.sub.cases_month, cases_used: L.sub.cases_used, searches_day: L.sub.searches_day, searches_month: L.sub.searches_month, period_end: L.sub.period_end } : null, my_limits: L.ok ? Object.fromEntries(Object.entries(L.lim).map(([k, v]) => [k, (typeof v === 'number' && v >= 1e8) ? null : v])) : null, reason: L.ok ? null : L.reason, usage: u }); } catch (e) { orgErr(res, e); } });
...
app.get('/api/intent', async (req, res) => { try { const ip = String(req.headers['x-forwarded-for'] || req.ip || '').split(',')[0].trim() || 'x'; if ((await LIMITER.hit('intent:' + ip, 60)) > 60) return res.status(429).json({ error: 'slow down' }); } catch (e) {} const v = String(req.query.v || '').slice(0, 120); if (v) res.setHeader('Set-Cookie', 'ff_intent=' + encodeURIComponent(v) + '; Max-Age=' + (30 * 86400) + '; Path=/; SameSite=Lax'); res.json({ ok: true }); });
...
app.post('/api/pay/paddle/webhook', express.raw({ type: 'application/json' }), async (req, res) => { try { if (!PADDLE.verify(req.body, req.headers['paddle-signature'])) return res.status(400).send('bad signature'); const ev = PADDLE.parse(req.body.toString('utf8')); if (/transaction\.(completed|paid)/.test(ev.event) && ev.paymentId) {
      /* Phase 7 BUG-015: this passed a bare payment id where a session object is expected, so paymentId resolved to undefined and NOTHING settled. A session is built; consultancy plans go to the agency settlement. */
      const { data: subRow } = await admin().from('org_subscriptions').select('id,gateway_ref').eq('payment_id', ev.paymentId).limit(1); const so = { id: (subRow && subRow[0] && subRow[0].gateway_ref) || ('paddle_' + ev.transactionId), payment_status: 'paid', client_reference_id: ev.paymentId, metadata: { payment_id: ev.paymentId, org_id: subRow && subRow[0] ? 'org' : '', credits: subRow && subRow[0] ? '0' : '' } };
      const md = so.metadata; await ((md.org_id && md.credits === '0') ? settleAgencySubscription(so) : settleCardPayment(so, 'paddle')); }
...
app.post('/api/admin/simulate/purge', auth, perm('settings.write'), async (req, res) => { try { if (!['admin', 'super_admin'].includes(req.userRole)) return res.status(403).json({ error: 'admin only' }); if (String((req.body || {}).confirm || '') !== 'PURGE') return res.status(400).json({ error: 'Type PURGE to confirm; this deletes every SIM record', code: 'CONFIRM_REQUIRED' }); await admin().from('audit_log').insert({ actor: req.userId, event: 'SIM_PURGE', detail: 'confirmed' }).then(() => {}, () => {}); res.json(await SIMU.purge()); } catch (e) { res.status(400).json({ error: e.message }); } });
/* ============================================================================================================================ */
...
async function orgWatchers(userId) { try { const cl = await consultancyClients(userId, 'id,org_id,owner_user_id,status'); const out = new Set(); for (const c of (cl || [])) { if (c.owner_user_id) out.add(c.owner_user_id); const { data: mem } = await admin().from('org_members').select('user_id').eq('org_id', c.org_id).eq('status', 'active').in('role', ['owner', 'manager']).limit(20); for (const mm of (mem || [])) out.add(mm.user_id); } out.delete(userId); return [...out]; } catch (e) { return []; } }
/* ======================================================================================================================================= */
...
app.post('/api/visa/desk/:id/manual', auth, async (req, res) => { try { const b = req.body || {}; const p = await VSTRAT.manualUpdate(req.params.id, req.userId, { status: b.status, decided_on: b.decided_on, note: b.note, by: 'you' }); res.json({ ok: true, patch: p }); } catch (e) { res.status(e.status || 400).json({ error: e.message }); } });
...
/* TEST-ONLY seed endpoint for the in-memory database (FF_MEMDB=1, never in production). */
if (process.env.FF_MEMDB === '1' && process.env.NODE_ENV !== 'production') { app.post('/__memdb/ingest', auth, async (req, res) => { try { const items = Array.isArray((req.body || {}).items) ? req.body.items : []; const r = await require('./lib/engine').ingestOpps(items, String((req.body || {}).kind || 'work'), req.userId); res.json({ ok: true, r }); } catch (e) { res.status(400).json({ error: e.message }); } }); app.get('/__memdb/rows', (req, res) => { const M = require('./lib/memdb'); res.json({ rows: M.tbl(String(req.query.table || '')) }); }); app.post('/__memdb/ai', auth, async (req, res) => { try { const K = require('./lib/ai_keys'); const b = req.body || {}; const purpose = String(b.purpose || 'main'); const orgId = await K.tenantOfUser(req.userId); const run = async () => ({ key: K.keyFor(/case_writing|profile_normalize/.test(purpose) ? 'anthropic' : 'gemini'), openai: K.keyFor('openai'), tenant: K.tenant() }); if (!orgId) return res.json(Object.assign({ tenant: null }, await run())); const keys = await K.loadKeys(orgId); const need = /case_writing|profile_normalize/.test(purpose) ? 'anthropic' : 'gemini'; if (!keys[need]) { await K.recordHealth(orgId, 'not_connected', 'no ' + need + ' key'); const e = new K.NotConnected(orgId, need); return res.status(e.status).json({ error: e.message, code: e.code }); } const out = await K.runAs(orgId, run); if (b.fail) { const e = new Error('provider said ' + b.fail); e.status = Number(b.fail); const h = K.classify(e); await K.recordHealth(orgId, h, e.message); } else await K.recordHealth(orgId, 'healthy', purpose); try { await admin().from('ai_cost_ledger').insert({ org_id: orgId, user_id: req.userId, purpose, provider: need, model: 'test', input_tokens: 10, output_tokens: 5, cost_usd: 0.001 }); } catch (e) {} res.json(out); } catch (e) { res.status(e.status || 400).json({ error: e.message, code: e.code }); } }); app.post('/__memdb/extract', auth, async (req, res) => { try { const r = await require('./lib/docs').extractProfile(req.userId, { pass: 0 }); res.json({ ok: true, r }); } catch (e) { res.status(400).json({ error: e.message }); } }); app.post('/__memdb/seed', express.json({ limit: '2mb' }), (req, res) => { const M = require('./lib/memdb'); if ((req.body || {}).reset) M.reset(); M.seed((req.body || {}).rows || {}); res.json({ ok: true, tables: Object.fromEntries(Object.entries(M.T).map(([k, v]) => [k, v.length])) }); }); }
/* Phase 4: postings whose official page could not be reached at discovery are 'pending'; re-checked nightly, shown only once reachable. Never a second attempt within the same day; after 5 failed attempts they are closed. */
async function verifyPending() { const { data: rows } = await admin().from('opportunities').select('id,url,verify_attempts').eq('status', 'pending').lt('verify_attempts', 5).limit(200); let ok = 0, closed = 0; for (const o of (rows || [])) { let good = false; try { const ac = new AbortController(); const tm = setTimeout(() => ac.abort(), 8000); let resp = await fetch(o.url, { method: 'HEAD', redirect: 'follow', signal: ac.signal, headers: { 'user-agent': 'ForiForeign verifier' } }); if (resp && (resp.status === 405 || resp.status === 501)) resp = await fetch(o.url, { method: 'GET', redirect: 'follow', signal: ac.signal, headers: { 'user-agent': 'ForiForeign verifier' } }); clearTimeout(tm); good = !!(resp && resp.status < 400); if (resp && (resp.status === 404 || resp.status === 410)) { await admin().from('opportunities').update({ status: 'closed', verify_note: 'official page returned ' + resp.status }).eq('id', o.id); closed++; continue; } } catch (e) {} if (good) { await admin().from('opportunities').update({ status: 'verified', verified_at: new Date().toISOString(), verify_note: null }).eq('id', o.id); ok++; } else { const n = (Number(o.verify_attempts) || 0) + 1; await admin().from('opportunities').update({ verify_attempts: n, status: n >= 5 ? 'closed' : 'pending', verify_note: n >= 5 ? 'unreachable on 5 nightly checks' : 'unreachable; attempt ' + n }).eq('id', o.id); if (n >= 5) closed++; } } return { verified: ok, closed, checked: (rows || []).length }; }
QUEUE.register('verify_pending', async () => verifyPending());
QUEUE.register('outbox_flush', async () => require('./lib/mailer').flushOutbox(100));
if (process.env.FF_MEMDB === '1' && process.env.NODE_ENV !== 'production') { app.post('/__memdb/mail', express.json(), (req, res) => { global.__ffFakeMail = String((req.body || {}).mode || 'ok'); res.json({ mode: global.__ffFakeMail, sent: require('./lib/mailer')._fake.sent.length }); }); app.post('/__memdb/enqueue', express.json(), async (req, res) => { try { const Q = require('./lib/queue'); if (req.body.kind === '__sleep') Q.register('__sleep', p => new Promise(r => setTimeout(() => r({ slept: p.ms }), Number(p.ms) || 1000))); res.json({ id: await Q.enqueue(req.body.kind, req.body.payload || {}, req.body.opts || {}) }); } catch (e) { res.status(400).json({ error: e.message }); } }); app.post('/__memdb/runone', async (req, res) => { try { res.json({ ran: await require('./lib/queue').runOne() }); } catch (e) { res.status(400).json({ error: e.message }); } }); app.post('/__memdb/reap', async (req, res) => { try { await require('./lib/queue').reapStale(); res.json({ ok: true }); } catch (e) { res.status(400).json({ error: e.message }); } }); app.post('/__memdb/housekeeping', async (req, res) => { try { res.json(await housekeeping()); } catch (e) { res.status(400).json({ error: e.message }); } }); app.post('/__memdb/patch', express.json(), (req, res) => { const M = require('./lib/memdb'); const t = M.tbl(String(req.body.table || '')); const r = t.find(x => x.id === req.body.id); if (r) Object.assign(r, req.body.patch || {}); res.json({ ok: !!r }); }); app.post('/__memdb/due', (req, res) => { const M = require('./lib/memdb'); let n = 0; for (const r of M.tbl('mail_outbox')) if (r.status === 'pending') { r.next_attempt_at = new Date(Date.now() - 1000).toISOString(); n++; } res.json({ made_due: n }); }); app.post('/__memdb/flush', async (req, res) => { res.json(await require('./lib/mailer').flushOutbox(100)); }); }
QUEUE.register('payments_abandon_sweep', async () => sweepAbandonedPayments());
if (process.env.FF_MEMDB === '1' && process.env.NODE_ENV !== 'production') { app.post('/__memdb/settle', express.json(), async (req, res) => { try { res.json(await settleCardPayment(req.body.session || {}, req.body.source || 'webhook')); } catch (e) { res.status(400).json({ error: e.message }); } }); app.post('/__memdb/abandon', async (req, res) => { res.json(await sweepAbandonedPayments()); }); }
/* ================= BYOC · the consultancy's own AI account (FF keeps models, prompts and routing) ================= */
async function validateProviderKey(provider, key) { const ctl = new AbortController(); const tm = setTimeout(() => ctl.abort(), 12000); try { let r; if (provider === 'gemini') r = await fetch('https://generativelanguage.googleapis.com/v1beta/models?pageSize=1', { headers: { 'x-goog-api-key': key }, signal: ctl.signal }); else if (provider === 'anthropic') r = await fetch('https://api.anthropic.com/v1/models?limit=1', { headers: { 'x-api-key': key, 'anthropic-version': '2023-06-01' }, signal: ctl.signal }); else if (provider === 'openai') r = await fetch('https://api.openai.com/v1/models?limit=1', { headers: { authorization: 'Bearer ' + key }, signal: ctl.signal }); else return { ok: false, reason: 'unknown provider' }; clearTimeout(tm); if (r.status === 401 || r.status === 403) return { ok: false, reason: 'auth_failed' }; if (r.status === 429) return { ok: false, reason: 'rate_limited' }; if (!r.ok) return { ok: false, reason: 'provider_unavailable' }; return { ok: true }; } catch (e) { clearTimeout(tm); return { ok: false, reason: 'provider_unavailable', detail: 'could not reach the provider' }; } }
app.get('/api/org/:id/ai', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'org.settings'); const { data: c } = await admin().from('org_ai_connections').select('status,health,health_note,last_ok_at,last_error_at,connected_at,gemini_last4,anthropic_last4,openai_last4').eq('org_id', req.params.id).maybeSingle(); const H = { healthy: 'Healthy', not_connected: 'Not connected', auth_failed: 'Authentication failed: the key was rejected. Reconnect with a valid key.', billing: 'Usage or billing issue on your provider account. Add credit or raise the limit, then Reconnect.', rate_limited: 'Temporarily rate limited by your provider. It usually clears in a minute.', provider_unavailable: 'Provider unavailable right now. Nothing to do; we retry automatically.' }; res.json({ connected: !!(c && c.status === 'connected' && (c.gemini_last4 || c.anthropic_last4)), providers: { gemini: !!(c && c.gemini_last4), anthropic: !!(c && c.anthropic_last4), openai: !!(c && c.openai_last4) }, masked: c ? { gemini: c.gemini_last4 ? '••••' + c.gemini_last4 : null, anthropic: c.anthropic_last4 ? '••••' + c.anthropic_last4 : null, openai: c.openai_last4 ? '••••' + c.openai_last4 : null } : null, health: c ? c.health : 'not_connected', health_text: H[c ? c.health : 'not_connected'] || c.health, health_note: c && c.health_note, last_ok_at: c && c.last_ok_at, last_error_at: c && c.last_error_at, connected_at: c && c.connected_at, required: ['gemini', 'anthropic'], optional: ['openai'], note: 'FF chooses and updates the models, prompts and routing; your account only carries the usage and is billed by your provider directly.' }); } catch (e) { orgErr(res, e); } });
app.post('/api/org/:id/ai', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'org.settings'); const C = require('./lib/crypto'); if (!C.enabled()) return res.status(503).json({ error: 'Encryption is not configured on the platform (FF_DATA_KEY). Keys cannot be stored safely.' }); const b = req.body || {}; const row = { org_id: req.params.id, status: 'connected', health: 'healthy', health_note: null, connected_by: req.userId, updated_at: new Date().toISOString() }; const results = {}; for (const p of ['gemini', 'anthropic', 'openai']) { const k = String(b[p] || '').trim(); if (!k) continue; if (k.length < 20 || /\s/.test(k)) return res.status(400).json({ error: 'That does not look like a valid ' + p + ' key' }); const v = (process.env.FF_MEMDB === '1' && process.env.NODE_ENV !== 'production') ? { ok: true } : await validateProviderKey(p, k); results[p] = v; if (!v.ok) return res.status(400).json({ error: 'The ' + p + ' key could not be validated: ' + (v.reason === 'auth_failed' ? 'the provider rejected it' : v.reason === 'rate_limited' ? 'the provider is rate limiting right now, try again in a minute' : 'the provider could not be reached'), provider: p, code: v.reason }); row[p + '_key_enc'] = C.encrypt(k); row[p + '_last4'] = k.slice(-4); } if (!row.gemini_key_enc && !row.anthropic_key_enc) { const { data: ex } = await admin().from('org_ai_connections').select('gemini_key_enc,anthropic_key_enc').eq('org_id', req.params.id).maybeSingle(); if (!ex || (!ex.gemini_key_enc && !ex.anthropic_key_enc)) return res.status(400).json({ error: 'Connect at least the Gemini key and the Anthropic key' }); } await admin().from('org_ai_connections').upsert(row, { onConflict: 'org_id' }); require('./lib/ai_keys').forget(req.params.id); await orgAudit(req.params.id, req.userId, 'AI_CONNECTED', Object.keys(results).join(',')); res.json({ connected: true, providers: Object.keys(results) }); } catch (e) { orgErr(res, e); } });
app.delete('/api/org/:id/ai', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'org.settings'); await admin().from('org_ai_connections').update({ status: 'disconnected', gemini_key_enc: null, anthropic_key_enc: null, openai_key_enc: null, gemini_last4: null, anthropic_last4: null, openai_last4: null, health: 'not_connected', updated_at: new Date().toISOString() }).eq('org_id', req.params.id); require('./lib/ai_keys').forget(req.params.id); await orgAudit(req.params.id, req.userId, 'AI_DISCONNECTED', ''); res.json({ ok: true }); } catch (e) { orgErr(res, e); } });
/* Private usage meter: counts by feature, staff, branch and day for the current billing period; the cost is an ESTIMATE from token counts, never the provider's invoice. */
app.get('/api/org/:id/ai/health', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'clients.read'); const { data: c } = await admin().from('org_ai_connections').select('status,health,health_note,last_ok_at').eq('org_id', req.params.id).maybeSingle(); const h = c && c.status === 'connected' ? c.health : 'not_connected'; const T = { healthy: null, not_connected: 'AI account not connected. Searches and preparation will not run until the consultancy admin connects it (Team & setup → AI connection).', auth_failed: 'AI account rejected the key. The consultancy admin needs to reconnect it.', billing: 'AI account has a usage or billing issue at the provider. The consultancy admin needs to add credit.', rate_limited: 'AI account is temporarily rate limited by the provider; try again shortly.', provider_unavailable: 'AI provider unavailable right now; we retry automatically.' }; res.json({ health: h, text: T[h] || null, last_ok_at: c && c.last_ok_at }); } catch (e) { orgErr(res, e); } });
app.get('/api/org/:id/ai/usage', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'finance.read'); const from = String(req.query.from || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString()); const { data: rows } = await admin().from('ai_cost_ledger').select('user_id,purpose,provider,model,input_tokens,output_tokens,cost_usd,est_cost_usd,created_at').eq('org_id', req.params.id).gte('created_at', from).limit(20000); const FEAT = p => /search|discover|verify|enrich/.test(p || '') ? 'Search & discovery' : /case_writing|prepare|profile_normalize/.test(p || '') ? 'Application preparation' : /doc_extract|extract|classify/.test(p || '') ? 'Document analysis' : /reply|brain|high_value/.test(p || '') ? 'AI replies' : 'Other AI operations'; const by = { feature: {}, staff: {}, branch: {}, day: {}, purpose: {} }; let ops = 0, cost = 0; const { data: mem } = await admin().from('org_members').select('user_id,name,branch').eq('org_id', req.params.id); const nameOf = Object.fromEntries((mem || []).map(m => [m.user_id, m.name || m.user_id.slice(0, 8)])); const branchOf = Object.fromEntries((mem || []).map(m => [m.user_id, m.branch || 'Head office'])); for (const r of (rows || [])) { ops++; const c = Number(r.cost_usd != null ? r.cost_usd : r.est_cost_usd) || 0; cost += c; const bump = (k, key) => { by[k][key] = by[k][key] || { ops: 0, est_usd: 0 }; by[k][key].ops++; by[k][key].est_usd += c; }; bump('feature', FEAT(r.purpose)); bump('purpose', r.purpose || 'other'); bump('staff', nameOf[r.user_id] || 'client / system'); bump('branch', branchOf[r.user_id] || 'Head office'); bump('day', String(r.created_at || '').slice(0, 10)); } for (const k of Object.keys(by)) for (const key of Object.keys(by[k])) by[k][key].est_usd = Math.round(by[k][key].est_usd * 100) / 100; res.json({ period_from: from.slice(0, 10), total_ops: ops, estimated_usd: Math.round(cost * 100) / 100, estimate_note: 'Estimated from token counts at list prices. Your provider\'s invoice is the authoritative amount.', by }); } catch (e) { orgErr(res, e); } });
/* TECHNICAL SEARCH PROTECTION (not a quota): identical requests within 60 s are deduplicated, and a per-user burst guard protects the queue and the database. Normal consultants never meet it. */
async function searchGuard(req, res, kind, opts) { try { const ip = String(req.headers['x-forwarded-for'] || req.ip || '').split(',')[0].trim() || 'x'; const sig = require('crypto').createHash('sha1').update(req.userId + '|' + kind + '|' + JSON.stringify(opts || {})).digest('hex'); const dupKey = 'searchdup:' + sig; const n = await LIMITER.hit(dupKey, 60); if (n > 1) { res.status(202).json({ ok: true, deduped: true, message: 'That search is already running; results appear in a moment.' }); return false; } const burst = await LIMITER.hit('searchburst:' + req.userId, 60); if (burst > 20) { res.status(429).json({ error: 'Please pause a moment; searches are being queued. You can try again in a minute.', code: 'SLOW_DOWN', retry_in: 60 }); return false; } const perIp = await LIMITER.hit('searchip:' + ip, 60); if (perIp > 60) { res.status(429).json({ error: 'Too many searches from this connection; try again in a minute.', code: 'SLOW_DOWN', retry_in: 60 }); return false; } return true; } catch (e) { return true; } }
/* ================================================================================================================ */
/* STORAGE PROTECTION (R14000 §20): temporary processing data expires; permanent CRM records stay. Nightly. */
async function housekeeping() { const ago = d => new Date(Date.now() - d * 86400000).toISOString(); const out = {}; const del = async (t, col, days, extra) => { try { let q = admin().from(t).delete().lt(col, ago(days)); if (extra) q = extra(q); const { data } = await q.select('id'); out[t] = (data || []).length; } catch (e) { out[t] = 'skip'; } }; await del('job_queue', 'updated_at', 14, q => q.in('status', ['done', 'dead'])); await del('mail_outbox', 'sent_at', 90, q => q.eq('status', 'sent')); await del('error_log', 'created_at', 90); await del('copilot_log', 'created_at', 60); await del('selfheal_log', 'created_at', 60); await del('portal_runs', 'created_at', 60); await del('ai_cost_ledger', 'created_at', 400); try { const { data } = await admin().from('opportunities').update({ status: 'closed' }).eq('status', 'pending').lt('created_at', ago(30)).select('id'); out.stale_pending_opportunities_closed = (data || []).length; } catch (e) {} return out; }
QUEUE.register('housekeeping', async () => housekeeping());
...
app.get('/api/profile/conflicts', auth, async (req, res) => {
  const det = [];
  try {
    const { data } = await admin().from('app_settings').select('value').eq('key', 'profilex:' + req.userId).single().then(x => x, () => ({ data: null }));
    /* Phase 6: deterministic cross-check of what each live document says against the profile (name, date of birth, passport number, nationality). No model needed; nothing is silently accepted. */
    try { const { data: me } = await admin().from('profiles').select('full_name,date_of_birth,passport_number,nationality').eq('id', req.userId).maybeSingle(); const { data: docs } = await admin().from('documents').select('id,name,doc_type,extracted').eq('user_id', req.userId).is('superseded_at', null).limit(100); const norm = x => String(x || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim(); for (const d of (docs || [])) { const x = d.extracted || {}; if (x.full_name && me && me.full_name && norm(x.full_name) !== norm(me.full_name)) det.push({ field: 'full_name', document: d.name, document_id: d.id, document_value: x.full_name, profile_value: me.full_name, text: 'Name on ' + (d.doc_type || 'document') + ' (' + x.full_name + ') differs from your profile (' + me.full_name + ')', severity: 'review' }); if (x.date_of_birth && me && me.date_of_birth && String(x.date_of_birth).slice(0, 10) !== String(me.date_of_birth).slice(0, 10)) det.push({ field: 'date_of_birth', document: d.name, document_id: d.id, document_value: x.date_of_birth, profile_value: me.date_of_birth, text: 'Date of birth on ' + (d.doc_type || 'document') + ' differs from your profile', severity: 'review' }); if (x.passport_number && me && me.passport_number && norm(x.passport_number) !== norm(me.passport_number)) det.push({ field: 'passport_number', document: d.name, document_id: d.id, document_value: x.passport_number, profile_value: me.passport_number, text: 'Passport number on ' + d.name + ' differs from your profile', severity: 'review' }); } } catch (e) {}
    const x = (data && data.value && data.value.x) || {};
    const list = (x._conflicts || []).filter(c => c && c.status !== 'resolved');
    res.json({ conflicts: [...list, ...det], completeness: (data && data.value && data.value.completeness) || null, sources: x._sources || [] });
...
app.get('/api/profile/assist', auth, async (req, res) => {
  try {
    ...
    let consultancy = ''; try { const cl = await consultancyClients(req.userId, 'id,org_id'); if (cl && cl.length) { const { data: og } = await admin().from('organisations').select('name').eq('id', cl[0].org_id).maybeSingle(); consultancy = og ? og.name : ''; } } catch (e) {}
...
app.post('/api/admin/users/:id/role', auth, perm('users.write'), async (req, res) => { try { _roleCache.delete(req.params.id); } catch (e) {}
  ...
  if (grantingAdmin && String((req.body || {}).confirm || '') !== 'GRANT') return res.status(400).json({ error: 'Type GRANT to confirm an admin-level role', code: 'CONFIRM_REQUIRED' });
...
app.get('/api/me', auth, async (req, res) => {
  let { data, error } = await admin().from('profiles').select('*').eq('id', req.userId).maybeSingle();
  if (!data) {   /* P2-001: first login, or a profile row missing for any reason: create it here, never 500 */
...
async function ledgerWrite(row) {
  ...
  let last = null;
  /* P2-004 (Phase 2): this loop called ledgerWrite recursively instead of inserting — infinite recursion on every credit
     consumption, grant and payment settlement ("Maximum call stack size exceeded"). Present in the deployed baseline. */
  for (const a of attempts) {
    let r; try { r = await admin().from('credit_ledger').insert(a).select('id').single(); } catch (e) { r = { error: { message: e.message } }; }
...
/* BUG-002 (Phase 2): every account also owns a PERSONAL workspace (lib/tenancy) with a self "client" row. Checks that meant "is this a
   consultancy's client?" matched that row, so every direct applicant was treated as a consultancy client: checkout refused (403
   CONSULTANCY_CLIENT), wrong consultancy banner, watchers notified. Consultancy clients are rows whose organisation is NOT personal. */
const _ccCache = new Map();
async function consultancyClients(userId, cols) { try { const hit = _ccCache.get(userId); if (hit && Date.now() - hit.at < 60000 && hit.cols === cols) return hit.rows; const { data: cl } = await admin().from('clients').select(cols || 'id,org_id,owner_user_id,status').eq('user_id', userId).limit(5); const rows = []; for (const c of (cl || [])) { const { data: o } = await admin().from('organisations').select('kind').eq('id', c.org_id).maybeSingle(); if (o && o.kind !== 'personal') rows.push(c); } _ccCache.set(userId, { rows, cols, at: Date.now() }); return rows; } catch (e) { return []; } }
const _orgCoverCache = new Map();
async function orgCovers(userId) { try { const hit = _orgCoverCache.get(userId); if (hit && Date.now() - hit.at < 60000) return hit.ok; const cl = await consultancyClients(userId, 'id,org_id'); let ok = false; if (cl && cl.length) { const Q = require('./lib/quota'); const L = await Q.limitsFor(cl[0].org_id, { user_id: userId }); ok = !!(L && L.ok); } _orgCoverCache.set(userId, { ok, at: Date.now() }); return ok; } catch (e) { return false; } }
async function entitled(userId, sim) {
  if (sim) return (sim.tier || 0) >= 1; // simulated package member or fresh visitor, exactly as chosen
  try {
    if (await orgCovers(userId)) return true;   // a consultancy's client: the consultancy's plan covers the desk work
...
app.post('/api/payments', auth, async (req, res) => {
  ...
  const full = {
    ...
    intent: ((req.body || {}).opportunity_id || (req.body || {}).opportunityId) ? { opportunity_id: String((req.body || {}).opportunity_id || (req.body || {}).opportunityId).slice(0, 64) } : null
  };
...
app.post('/api/payments/:id/confirm', auth, perm('payments.write'), async (req, res) => {
  ...
  try { await admin().from('payments').update({ settled_source: 'admin' }).eq('id', p.id); await settleIntoCase(Object.assign({}, p, { status: 'confirmed' })); } catch (e) {}   /* DISC-003: bank-transfer confirmations open the intended case too */
  if (led.error) {
  ...
  if (p.addon_key) { await grantAddon(p); await expandBundle(p); }
...
async function createApplication(req, res) {
  const opportunityId = (req.body || {}).opportunityId || (req.body || {}).opportunity_id;   /* accept both spellings (app uses opportunityId; API consumers snake_case) */
  const { data: prof } = await admin().from('profiles').select('role').eq('id', req.userId).single();
  ...
  /* P2-005 (Phase 2): the same opportunity started twice created two cases and consumed two credits. One open case per opportunity. */
  try { const { data: dup } = await admin().from('applications').select('*, opportunities(title,institution,country_code,deadline,url)').eq('user_id', req.userId).eq('opportunity_id', opp.id).order('created_at', { ascending: false }).limit(5); const open = (dup || []).find(d => !/withdrawn|rejected|closed/.test(String(d.status || d.stage || ''))); if (open) return res.json({ application: open, existing: true, note: 'You already have a case for this opening; it is open below.' }); } catch (e) {}
...
app.post('/api/documents', auth, up.array('files', 20), enforceUploadLimits, async (req, res) => {
  try {
    ...
    try { await JE.compute(req.userId); } catch (e) {}
...
app.get('/api/documents', auth, async (req, res) => {
  const { data } = await admin().from('documents').select('id,kind,name,mime,size_bytes,created_at').eq('user_id', req.userId).is('superseded_at', null).eq('generated', false).order('created_at', { ascending: false });
...
async function hasEverPaid(userId) {
  if (await orgCovers(userId)) return true;   // covered by the consultancy's plan: no free-tier limits, no paywall prompts
...
app.use((err, req, res, next) => {
  ...
  if (err && err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: 'That file is larger than 15 MB. Please compress it or split it and try again.', code: 'FILE_TOO_LARGE', max_mb: 15 });
  if (err && err.code === 'UNSUPPORTED_TYPE') return res.status(400).json({ error: err.message, code: 'UNSUPPORTED_TYPE' });
  if (err && err.code === 'LIMIT_FILE_COUNT') return res.status(400).json({ error: 'Up to 20 files at a time.', code: 'TOO_MANY_FILES' });
