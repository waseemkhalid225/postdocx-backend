...
async function connect() {
  ...
  const doc2 = new GoogleSpreadsheet(sheetId, auth);
  ...
  for (const [title, headers] of Object.entries(SCHEMA)) {
    let sheet = doc2.sheetsByTitle[title];

    if (!sheet) {
      sheet = await doc2.addSheet({
        title,
    ...
    const have = sheet.headerValues || [];
    const missing = headers.filter(h => !have.includes(h));
    if (missing.length) {
      const merged = [...have.filter(Boolean), ...missing];
      if ((sheet.columnCount || 0) < merged.length) {
        await sheet.resize({ rowCount: sheet.rowCount || 1000, columnCount: merged.length + 2 });
      }
      await sheet.setHeaderRow(merged);
    }
  }

  doc = doc2; // cache ONLY after the full schema sync succeeded
  return doc;
}
...
async function rows(tab) {
  ...
  if (!sheet) {
    // Self-heal: create the missing tab with its schema headers right now
    if (SCHEMA[tab]) sheet = await doc.addSheet({ title: tab, headerValues: SCHEMA[tab] });
    else throw new Error(`Google Sheet tab "${tab}" does not exist`);
  }
