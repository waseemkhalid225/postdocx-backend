...
const checks = [
  ...
  ['R7200: minimal landing (one line, one action, four steps, two doors, live trust), sign-in behind a link, button hierarchy, nav hidden when signed out on phones, bottom sheets on phones, wider desktop canvas, deferred secondary dashboard calls, consultancy sheet', (() => {
    ...
    return land.includes('class="land-hero"') && land.includes('Your CV in.<br>Your visa out.') && land.includes('Start free, upload your CV') && land.includes('I run a consultancy') && land.includes('class="land-steps"') && !land.includes('mkt-stats') && !land.includes('We Prepare Everything') && land.includes("mode==='landing'")
  ...
  ['R9300: the killer line, free search shows real details and the pathway (locked cards and guest preview), profession autocomplete that auto-selects, /api/professions, never-zero matches sheet (catalogue fallback with a note)', (() => {
    ...
    return L.includes('Your future abroad.<br><em>Your way.</em>') && L.includes('without handing your entire case to a traditional consultant') && L.includes("Pathway: ") && sv.includes('async function pathwayFor(') && sv.includes("app.get('/api/professions'") && sv.includes("details: String(o.description || '')") && f.includes('<span>Pathway</span>') && f.includes('list="profList"') && f.includes('function profAuto(') && f.includes('NEVER ZERO') && f.includes("Object.assign(o,{fallback:true})") && ex.includes('description,requirements,visa_sponsorship');
  })()],
  ['R9400: landing without duplicate buttons, single-line explainer, Ask us instead of open FAQs, gold hairlines and gradient accents; CRM hero killer lines with three stat tiles and twelve uniform check-boxed modules; split sign-in with the story; dedicated ForiForeign address wording', (() => {
    ...
    return !L.includes('<a class="cta" href="/app#signup">Start free</a>') && (L.match(/href="\/app#signup">Start free/g) || []).length === 2 && L.includes('class="lines"') && L.includes('>Ask us<') && !L.includes('<details') && L.includes('border-top:3px solid var(--gold)') && L.includes('<em>Your way.</em>') && C.includes('does the work.</em>') && C.includes('class="kill"') && (C.match(/<div class="card( hi)?"><h3>/g) || []).length >= 12 && C.includes(".card li::before{content:'✓'") && f.includes('class="auth-wrap"') && f.includes('class="auth-story"') && f.includes('own ForiForeign address');
  })()],
