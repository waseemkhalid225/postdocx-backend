...
const checks = [
  ...
  ['R10400: readability final (every inline colour becomes ink unless it carries meaning; dark inline surfaces become white), scrollbars truly black and 14 px (the old white scrollbar-color rules removed), CV upload opens the profile, step 1 renamed, profile tabs About/Documents/Settings/Options (journey cards moved to Options), currency by profile origin then IP geolocation (cached) then browser, prices shown as $19 (Rs 5,300)', (() => {
    ...
    return f.includes('body.light [style*="color:#"],body.light [style*="color: #"]') && !f.includes('scrollbar-color:rgba(255,255,255,.85)') && !f.includes('scrollbar-color:#FFFFFF transparent') && f.includes('*{scrollbar-color:#0B1B3A #E3E9F6;scrollbar-width:auto}') && f.includes('::-webkit-scrollbar{width:14px;height:14px}') && f.includes("['Profile (upload your CV)'") && f.includes("window._ptab='about';if(TAB!=='profile'){go('profile')}") && f.includes("['options','Options',[") && !f.includes("['journey','Journey',[") && sv.includes('async function geoCountry(') && sv.includes('async function visitorCountryAsync(') && sv.includes("app.get('/api/me/visitor'") && f.includes("('+esc(loc.display)+')");
  })()],
