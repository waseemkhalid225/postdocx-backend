...
const checks = [
  ...
  ['premium Claude lane exists with Gemini fallback', (() => { const r = fs.readFileSync(__dirname + '/../lib/router.js', 'utf8'); return r.includes("case_writing:      { provider: 'anthropic'") && r.includes('profile_normalize') && r.includes('modelOverrides') && r.includes("provider === 'anthropic' && require('./ai_keys').keyFor('anthropic')"); })()],
  ...
  ['journey steps never block: every step stays clickable after completion', (() => {
    ...
    return fe.includes("'Update CV'") && fe.includes("'Search'") &&
      fe.includes("'Review & Apply'") && fe.includes('class="card cvcard"') &&
  ...
  ['R4750: plans page cannot throw on HM, admin checks read the real ME, payment details are live', (() => {
    ...
    return !f.includes('window.ME&&') && f.includes('var HM=null;')
      ...
      && st.includes("easypaisa_number: '03455216903'") && st.includes("account_title: 'ForiForeign (Private) Limited'")
  ...
  ['R4760: every ledger write survives the reason CHECK constraint', (() => {
    ...
    return sv.includes('async function ledgerWrite(row)')
      && (sv.match(/admin\(\)\.from\('credit_ledger'\)\.insert\(/g) || []).length === 1 /* only inside ledgerWrite (P2-004) */
  ...
  ['R4810 Phase 1: document intelligence, checklist engine and mobility profile', (() => {
    ...
    return V.DOC_TYPES.includes('passport') && V.SENSITIVE.has('bank_statement') && !V.SENSITIVE.has('cv')
      ...
      && Array.isArray(M.REQUIRED_FOR_MATCH) && M.ALL_FIELDS.includes('visa_refusals') && M.ALL_FIELDS.length > 50
  ...
  ['R4830: USD is the currency of record; international card checkout with a signed webhook', (() => {
    ...
    return G.verifySignature(body, sig, secret) === true && G.verifySignature(body, sig, 'other') === false && G.verifySignature(body, stale, secret) === false
      ...
      && sv.includes('async function settleCardPayment(session, source)') && sv.includes(".eq('id', p.id).in('status', ['pending', 'abandoned']).select('id');")
      && sv.includes("if ((s.metadata || {}).user_id && s.metadata.user_id !== req.userId) return res.status(403)")
      && st.includes("usd: 25, promo_usd: 19") && st.includes("usd_month: 179") && st.includes("pricing: { currency: 'USD', show_local: true }") && st.includes("gateway: { card: !!(process.env.STRIPE_SECRET_KEY || (process.env.LEMON_API_KEY && process.env.LEMON_STORE_ID))")
  ...
  ['R5000 Days 14-15: runbook, backup/restore drill, pricing page, launch build feature flags', (() => {
    ...
    return fs.existsSync(__dirname + '/../docs/RUNBOOK.md') && fs.existsSync(__dirname + '/../tools/backup.js') && fs.existsSync(__dirname + '/../tools/restore.js') && fs.existsSync(__dirname + '/../public/pricing.html')
      ...
      && sv.includes("res.set('x-ff-features'") && /const FF_BUILD = '2026-\d{2}-\d{2}-R\d{4,5}';/.test(sv);
  })()],
  ...
  ['R5500 fix pass: attachments to vault, journey engine, confidence gate, change detection, consent confirm, licences, refunds, metering, retention, sources, tracks, alerts, session revocation, legal pages', (() => {
    ...
    return JE.ORDER[0] === 'lead' && JE.ORDER.includes('pr') && Object.keys(S.ADAPTERS).length === 5
      ...
      && fs.existsSync(__dirname + '/../public/legal.html') && f.includes('class="next-card"') && f.includes('function profileTabs(') && f.includes("Your consultant asks to watch") && f.includes('async function licencesLoad(') && f.includes("Is this a ${esc((m.classification||'reply')")
  ...
  ['R6000 (R14000: fixed plans, no FF caps): agency plans with quota allocation down the tree, resale locks, leads scope, translation plugin, simple finder, boards, profession filter', (() => {
    ...
    return S.length === 3 && S.every(t => t.cases_month == null) && S.every(t => t.searches_day == null && t.searches_month == null && t.usd_year) && !st.includes('pkr_month')
  ...
  ['R6300: revision of the low scores - discovery quality score + ESCO synonyms + freshness sweep, design pass 3, legal DPA/sub-processors/retention/cookies/age/agent-law, security headers + CSP report-only, shared limiter (Redis-ready), verify-assist, bounces, warm-up cap, policy pages, self-probe, FAQPage data', (() => {
    ...
    return hi.quality >= 90 && hi.label === 'high' && lo.quality <= 10 && L.backend() === 'memory'
      && sv.includes("LIMITER.hit('ip:' + ip, 60)") && sv.includes("res.set('Content-Security-Policy', csp)") && sv.includes("app.post('/api/csp-report'") && sv.includes("quality: (typeof DQ !== 'undefined' && DQ) ? DQ.score(o) : null") && sv.includes("app.post('/api/admin/visa/rules/:rid/assist'") && sv.includes("app.post('/api/mail/events'") && sv.includes("async function outreachCap()") && sv.includes("app.get('/updates/:cc?'") && sv.includes("async function selfProbe()")
      ...
      && f.includes('Design pass 3 (R6300)') && f.includes('const qChip=') && lg.includes('12. Data processing addendum') && lg.includes('13. Sub-processors') && lg.includes('14. Retention schedule') && lg.includes('17. Where a licensed adviser') && hp.includes('FAQPage');
  })()],
  ['R6400: 10,259 universities shipped (6,457 in the 54 destinations), 85 international scholarships, requirements brief per opportunity x person, forimail-only policy, portal fill plan with presence notifications', (() => {
    ...
    return UW.count() > 10000 && UW.count('DE') > 300 && UW.count('US') > 2000 && P.filter(c => UW.count(c) > 0).length === 54 && SC.length >= 80 && SC.every(s => /^https?:/.test(s.url) && s.levels.length) && P.filter(c => ccs.has(c)).length >= 45
      ...
      && f.includes('async function reqBrief(') && f.includes('What this case needs from me') && f.includes('SCHOLARSHIPS OPEN TO INTERNATIONAL APPLICANTS') && fs.readFileSync(__dirname + '/../tools/browser-worker.js.example', 'utf8').includes('FILL PLAN (R6400)');
  })()],
  ...
  ['R7200: minimal landing (one line, one action, four steps, two doors, live trust), sign-in behind a link, button hierarchy, nav hidden when signed out on phones, bottom sheets on phones, wider desktop canvas, deferred secondary dashboard calls, consultancy sheet', (() => {
    ...
    return land.includes('class="land-hero"') && land.includes('Your CV in.<br>Your visa out.') && land.includes('Start free, upload your CV') && land.includes('I run a consultancy') && !land.includes('mkt-stats') && !land.includes('We Prepare Everything') && land.includes("mode==='landing'")
  ...
  ['R7400: confidence layer on the landing - sample case timeline, how we make money plainly, security strip, founder, first-questions FAQ, sticky mobile CTA, accessible who-selector, deferred globe', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    return f.includes('What a case looks like') && f.includes('How we make money, plainly') && f.includes('class="land-sec"')  && f.includes('id="landFaq"') && f.includes('id="landSticky"') && f.includes('role="tab" aria-selected=') && f.includes('(fn=>setTimeout(fn,600)))(()=>bootGlobe())') && f.includes('Questions people ask first');
  })()],
  ['R7500: standalone 27 KB landing at /, app at /app, hash-driven sign-in/sign-up with intent, preview falls back to sourced seeds, who-selector with keyboard, compare table, sources strip, illustrated case', (() => {
    ...
    return L.length < 60000 && L.includes('CASE FF-C-2026-000118') && L.includes('href="/app#signup"') && !L.includes('supabase')
  ...
  ['R8500: light only (no toggle, no dark world), journey-named navigation, next-best-step hero with the journey line, study/work lane chooser stored on the profile and driving Explore, calmer cards', (() => {
    ...
    return !f.includes('id="themeBtn"') && f.includes("document.body.classList.add('light');") && f.includes('#worldBg,#worldOverlay{display:none!important}') && f.includes('>My journey<') && f.includes('>Opportunities<') && f.includes('data-t="apps">Applications<') && f.includes('You are here · next step') && f.includes('class="tracker" role="list"') && f.includes('function laneChooser(') && f.includes("laneSet('study')") && f.includes("localStorage.getItem('ffLane')||''") && sv.includes("'lane_pref']") && mg.includes('lane_pref');
  })()],
  ...
  ['R9000: contrast (no shadows, ink on white, darker secondary text), humanised next step, applicant-first landing with one line and one action and no audience pills, partners page with one email address and confidential terms, one pricing page in the visitor\'s currency with natural rounding, native prices in the app, no employer/university pricing, "email address" wording', (() => {
    ...
    return f.includes('body.light *{text-shadow:none!important}') && f.includes('body.light .sub,body.light .muted,body.light .hint{color:#4A5A78!important}') && je.includes("highest_level: 'your highest degree'") && i18n.includes("nav_dashboard: { en: 'My journey'") && L.includes('Global opportunities.<br><em>Your way.</em>') && !L.includes('class="who"') && !L.includes('Plain prices') && L.includes('href="/partners.html">Partners</a>') && L.includes('own ForiForeign address') && !L.includes('/trust.html') && PA.includes('admin@foriforeign.com') && PA.includes('confidential') && !P.includes('$null') && P.includes("FF-CRM for consultancies") && P.includes('local_currency') && !P.includes('Employer') && sv.includes("app.get('/api/local-price'") && sv.includes('function visitorCountry(') && W.localPrice(19, 'PK', { PKR: 280 }).display === 'Rs 5,300' && W.niceRound(5314) === 5300 && f.includes('function localMoney(') && f.includes('function niceRound(') && !sv.includes("employers: { receive_candidates: 'free'");
  })()],
  ['R9100: FF-CRM - the consultancy module named and described as a complete CRM (ten modules, what no other CRM does, per-consultancy pricing, set-up in a day), linked from landing, pricing and partners', (() => {
    ...
    return f.includes('data-i18n="nav_workspace">FF-CRM<') && f.includes('Open your FF-CRM') && C.includes('Sixteen modules, one desk') && C.includes('Lead capture') && C.includes('Visa management') && C.includes('Branches and sub-agents') && C.includes('The work itself') && C.includes('Per consultancy, not per user') && C.includes('admin@foriforeign.com') && P.includes('FF-CRM for consultancies') && L.includes('href="/crm.html">FF-CRM</a>') && seo.includes('/crm.html');
  })()],
  ...
  ['R9300: the killer line, free search shows real details and the pathway (locked cards and guest preview), profession autocomplete that auto-selects, /api/professions, never-zero matches sheet (catalogue fallback with a note)', (() => {
    ...
    return L.includes('Global opportunities.<br><em>Your way.</em>') && L.includes('find the right path to study or work abroad') && L.includes("Pathway: ") && sv.includes('async function pathwayFor(') && sv.includes("app.get('/api/professions'") && sv.includes("details: String(o.description || '')") && f.includes('<span>Pathway</span>') && f.includes('list="profList"') && f.includes('function profAuto(') && f.includes('NEVER ZERO') && f.includes("Object.assign(o,{fallback:true})") && ex.includes('description,requirements,visa_sponsorship');
  })()],
  ['R9400: landing without duplicate buttons, single-line explainer, Ask us instead of open FAQs, gold hairlines and gradient accents; CRM hero killer lines with three stat tiles and twelve uniform check-boxed modules; split sign-in with the story; dedicated ForiForeign address wording', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8');
    return !L.includes('<a class="cta" href="/app#signup">Start free</a>') && (L.match(/href="\/app#signup">Start (free|your journey)/g) || []).length === 2 && L.includes('class="lines"') && L.includes('>Ask us<') && !L.includes('<details') && L.includes('border-top:3px solid var(--gold)') && L.includes('<em>Your way.</em>') && C.includes('does the work.</em>') && C.includes('class="kill"') && (C.match(/<div class="card( hi)?"><h3>/g) || []).length >= 12 && C.includes(".card li::before{content:'✓'") && f.includes('class="auth-wrap"') && f.includes('class="auth-story"') && f.includes('own ForiForeign address');
  })()],
  ['R9500: real two-week trial on Starter limits (quota), plan-state endpoint, workspace banner with the end date, API keys gated to paid Growth/Scale, no 15-day wording on public pages, comparison table with lanes, client mailbox, live updates and privacy', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const P = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const Q = require('../lib/quota.js');
    return typeof Q.trialSub === 'function' && Q.TRIAL_DAYS === 14 && fs.readFileSync(__dirname + '/../lib/quota.js', 'utf8').includes("if (!sub) sub = await trialSub(orgId);") && sv.includes("app.get('/api/org/:id/plan-state'") && sv.includes("code: 'PLAN_API'") && f.includes('async function planStateLoad(') && f.includes('Your free trial runs for two weeks') && !C.includes('15-day') && !P.includes('15-day') && !f.includes('15-day') && C.includes('<th>Study lane</th><th>Work lane</th>') && C.includes('Client mailbox') && C.includes('Live updates') && C.includes('Clients masked from vendor') && C.includes('free two-week trial');
  })()],
  ['R9530 audit fixes: fx rates map no longer overwritten by the pay sheet, profession autocomplete keeps focus, landing sentences never truncated, CRM subtitle corrected, trial pseudo-subscription carries counters, lane chooser respects the saved profile lane, staff see no credit pill', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const q = fs.readFileSync(__dirname + '/../lib/quota.js', 'utf8');
    return f.includes('window._fxQuote={currency:u.local_currency') && !f.includes('window._fx={currency:u.local_currency') && f.includes("el.setSelectionRange(el.value.length,el.value.length)") && !L.includes('white-space:nowrap;overflow:hidden;text-overflow:ellipsis}') && !C.includes('Nepal and Australia') && q.includes('cases_used: 0, trial: true') && f.includes("function laneChooser(){return false;") && f.includes("const staff=ME&&isAdminRole(ME.role);$('creditPill').style.display=staff?'none':'inline-block'");
  })()],
  ['R9600: subscription lifecycle - 3-day grace in quota, days-left/grace state, reminders at 7/3/0 days by app and email with audit lines (daily sweep), automatic numbered invoice on letterhead emailed on every activation and renewal, renewal starts at the old period end, invoices endpoint and Billing list, workspace banners', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const q = fs.readFileSync(__dirname + '/../lib/quota.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const Q = require('../lib/quota.js');
    return Q.GRACE_DAYS === 3 && q.includes('in_grace: true') && q.includes('days_left:') && sv.includes('async function subscriptionSweep(') && sv.includes("reminded_7") && sv.includes("reminded_3") && sv.includes("reminded_0") && sv.includes('async function issueOrgInvoice(') && sv.includes("from('org_invoices').insert") && sv.includes("renewed_from: cur ? cur.id : null") && sv.includes("if (sub.renewed_from) {") && sv.includes("app.get('/api/org/:id/invoices'") && sv.includes("QUEUE.register('subscription_sweep'") && ag.includes("'subscription_sweep'") && mg.includes('org_invoices') && f.includes('Renew now') && f.includes('async function orgInvoicesLoad(') && f.includes('id="orgInvoices"') && f.includes('day${d.days_left===1?') ;
  })()],
  ['R9700 deep audit: consultancy clients masked from platform staff in users, applications, global search and the case view (grant lifts it instantly); consultancy staff can search their own clients; live updates fan out to the consultancy watching a client; pathway cache expires', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8');
    return sv.includes('async function maskedClientMap(') && sv.includes('async function maskRows(') && sv.includes('async function staffMayOpen(') && sv.includes("code: 'MASKED'") && sv.includes("res.json({ users: await maskRows(") && sv.includes("res.json({ applications: await maskRows(") && sv.includes("title: 'Consultancy client (masked)'") && sv.includes("kind: 'client', id: c.id") && sv.includes('async function orgWatchers(') && sv.includes("kind: 'client_' + kind") && sv.includes("Date.now() - hit.at < 3600000") && sv.includes("_maskCache.at = 0; await ORGS.requireOrg(req, req.params.id, 'org.write')") && fs.readFileSync(__dirname + '/../lib/checkins.js', 'utf8').includes("c.at <= today && !c.sent") && sv.includes("if (patch._schedule) { try { await CHECKINS.schedule(c.id); } catch (e) {} try { await VSTRAT.planAfterSubmit(c.id); } catch (e) {} }");
  })()],
  ['R9700 deep audit: admin payments and support masked, free previews and locked cards redacted (institution words, links, emails, phones), ForiForeign pages redirected on white-label hosts, consultancy clients see no FF packages and cannot buy them server-side, check-ins reach the consultancy, typo-tolerant countries, never-zero for consultancy clients with a rendered fallback', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const E = require('../lib/explore.js'); const ck = fs.readFileSync(__dirname + '/../lib/checkins.js', 'utf8');
    const red = E.redactFree({ institution: 'Sunrise Care Group', description: 'Sunrise Care Group hires. https://sunrise.example hr@sunrise.example +44 20 7946 0000' }).description;
    return sv.includes("payments: await maskRows(list, 'user_id'") && sv.includes("tickets: await maskRows(data || [], 'user_id'") && !red.includes('Sunrise') && !red.includes('http') && !red.includes('@') && !red.includes('7946') && sv.includes('EXPLORE.redactFree(o)') && sv.includes("require('./lib/explore').redactFree(Object.assign(o, {") && sv.includes('crm\\.html|commitments\\.html|legal\\.html|api-docs\\.html') && f.includes("document.body.classList.add('cc-client')") && f.includes('body.cc-client .stage-offers') && f.includes('Your cases are handled by') && sv.includes("code: 'CONSULTANCY_CLIENT'") && ck.includes('async function tellOrg(') && E.understand({ text: 'nurse germny' }).cc === 'DE' && sv.includes('NEVER ZERO for a consultancy') && f.includes('o.matches&&o.matches.fallback');
  })()],
  ['R9800 work lane end to end: regions and multi-word countries, 60+ work words for lane detection, labour vocabulary (maid, farm, picker, rigger...), richer labour discovery queries for 18 destinations, GB Health and Care and Seasonal as two routes, a real salary parser (currency, period, perks, monthly USD), pay/language/clearance on every work card, category-aware pathway and visa-route suggestion with preselect, six more fraud patterns, care language mapping, labour-only preview', (() => {
    const E = require('../lib/explore.js'); const L = require('../lib/labour.js'); const P = require('../lib/payscale.js'); const S6 = require('../lib/visa_seed6.js'); const seed = S6.seed || S6; const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const ov = fs.readFileSync(__dirname + '/../lib/offer_verify.js', 'utf8'); const lg = fs.readFileSync(__dirname + '/../lib/language_guide.js', 'utf8');
    const u1 = E.understand({ text: 'truck driver saudi arabia' }), u2 = E.understand({ text: 'labour in europe' }), u3 = E.understand({ text: 'housemaid kuwait' }), u4 = E.understand({ text: 'driver job cn' });
    const pay = P.parseSalary('SAR 1800/month + food + accommodation', 'SA'), pay2 = P.parseSalary('£12.50 per hour', 'GB');
    return u1.cc === 'SA' && u1.text === 'truck driver' && u2.cc.split(',').length > 10 && u3.kind === 'work' && u4.cc === 'CN' && L.classify({ title: 'housemaid kuwait' }) === 'labour' && L.classify({ title: 'fruit picker' }) === 'labour' && L.queriesFor('QA').length >= 6 && L.queriesFor('KR').some(q => /eps/.test(q)) && seed.some(r => r.route_key === 'gb_health_care') && seed.some(r => r.route_key === 'gb_seasonal') && !seed.some(r => r.route_key === 'gb_care_seasonal') && pay.currency === 'SAR' && pay.monthly_usd === 480 && pay.perks.includes('accommodation') && pay2.period === 'hour' && pay2.monthly_usd > 2000 && sv.includes('lang_need:') && sv.includes("clearance: (o.category === 'labour' || o.category === 'care')") && sv.includes('async function pathwayFor(cc, lane, category)') && sv.includes("app.get('/api/visa/suggest'") && f.includes("add('Before you fly',o.clearance+' clearance')") && f.includes("api('/api/visa/suggest?cc='+cc") && ov.includes('asks for the original passport') && ov.includes('recruitment fee charged to the worker') && lg.includes("lane === 'care') ? 'labour'") && sv.includes("if (lane === 'labour') q = q.in('category', ['labour', 'care'])");
  })()],
  ['R9900 brand: the approved F + airplane mark is the one identity - transparent master and 8 sizes generated from the supplied artwork, favicon.ico, apple-touch-icon, PWA icons, OG image; mark + wordmark on every public page and the app header, all clickable to /, aria-labelled; compact mark on phones; white label swaps to the consultancy logo', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const mf = fs.readFileSync(__dirname + '/../public/manifest.json', 'utf8');
    const files = ['brand/ff-mark.png', 'brand/ff-mark-16.png', 'brand/ff-mark-32.png', 'brand/ff-mark-64.png', 'brand/ff-mark-180.png', 'brand/ff-mark-512.png', 'favicon.ico', 'apple-touch-icon.png', 'icon-192.png', 'icon-512.png', 'share.png'].every(x => fs.existsSync(__dirname + '/../public/' + x));
    const pages = ['landing', 'pricing', 'crm', 'partners', 'help', 'legal', 'commitments', 'trust', 'api-docs'].every(p => { const h = fs.readFileSync(__dirname + '/../public/' + p + '.html', 'utf8'); return h.includes('aria-label="Go to ForiForeign Home"') && h.includes('/brand/ff-mark-64.png') && h.includes('href="/favicon.ico"') && h.includes('apple-touch-icon.png'); });
    return files && pages && f.includes('id="brandLink" href="/" aria-label="Go to ForiForeign Home"') && f.includes('id="brandMark" src="/brand/ff-mark-64.png"') && f.includes('@media (max-width:640px){.brand span{display:none}') && f.includes("b.setAttribute('href','/app');b.setAttribute('aria-label','Go to '+(w.name||'home'))") && f.includes('/share.png') && L.includes('class="brand" href="/"') && mf.includes('#F7F9FF');
  })()],
  ['R10100 STRICT SEPARATION: FF-CRM clients are never matched, linked or notified against ForiForeign accounts (no email lookup, no consent flow, no pending link); an account is attached to a consultancy only if it was created on that consultancy\'s domain (sign-up origin recorded); platform accounts refused with a plain message; consultancy clients cannot use ForiForeign checkout; commitments, legal and the comparison state the wall both ways; every org route tenancy-guarded', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const o = fs.readFileSync(__dirname + '/../lib/orgs.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const lg = fs.readFileSync(__dirname + '/../public/legal.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8');
    const routes = [...sv.matchAll(/app\.(get|post|put|delete|patch)\('(\/api\/org\/:id[^']*)'/g)]; const unguarded = routes.filter(m => !/requireOrg|orgClient\(/.test(sv.slice(m.index, m.index + 900)));
    return !o.includes("from('profiles').select('id,role').ilike('email'") && !o.includes('pending_user_id') && !sv.includes('consultancy/requests') && !f.includes('ccRequests') && sv.includes("signup_org_id: o.id") && sv.includes('cannot be attached to a consultancy') && sv.includes("code: 'CONSULTANCY_CLIENT'") && mg.includes('signup_org_id') && mg.includes('drop column if exists pending_user_id') && sv.includes('Can we add a person who already has a ForiForeign account as our client?') && sv.includes("Who collects the client\\'s money?") && lg.includes('strict separation') && C.includes("Vendor's own applicants walled off") && f.includes("$('bestRail')") && f.includes("$('glMatches')") && unguarded.length === 0;
  })()],
  ['R10200 applicant polish: black thick scrollbars, jet-black forms, one document vault (must-have box with instructions, strengthens box, drop zone opens the picker, highest degree first, optional files labelled), verified-info and approve-your-way cards removed, offers read from mail, 71 portal presets incl. PK/IN/BD/Gulf/EU/US origins and destinations, CV extraction of 30+ facts with computed totals (never the CV email), plus packages (Basic+/Smart+/Premium+) replacing step-by-step add-ons, on dashboard, checkout, settlement and pricing page', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const rd = fs.readFileSync(__dirname + '/../lib/reader.js', 'utf8'); const B = require('../lib/browserbot.js'); const S = require('../lib/settings.js'); const P = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return f.includes('*::-webkit-scrollbar{width:14px') && f.includes('scrollbar-color:#0B1B3A') && f.includes('class="vbox must"') && f.includes('class="vbox can"') && f.includes('aria-label="Drop files here or tap to choose"') && f.includes('not required now · may be asked later') && !f.includes('id="verifyCard"') && !f.includes('Approve &amp; Apply, your way') && f.includes('read from your mail') && Object.keys(B.PORTALS).length >= 70 && B.PORTALS.pk_beoe && B.PORTALS.in_emigrate && B.PORTALS.sa_mofa && B.PORTALS.us_ceac && rd.includes('"total_experience_years":0') && rd.includes('Never use the email address in the CV') && rd.includes('highest_degree_year') && S.DEFAULTS.plus && S.DEFAULTS.plus.tiers.length === 1 && S.DEFAULTS.plus.tiers[0].key === 'o2s' && S.DEFAULTS.plus.tiers[0].usd === 100 && sv.includes("/_plus$/.test(key)") && sv.includes("bundle: p.addon_key") && !sv.includes("items.push({ key: 'offer_pack'") && sv.includes('plusItem') && P.includes('one payment, ever') && mg.includes('add column if not exists bundle');
  })()],
  ['R10300 admin cockpit: admins land on Admin with a one-paragraph summary, needs-a-decision buttons and twelve section numbers; a header switch to the applicant view with rights intact; admin starts a case for any applicant (their vault, address, reference; no credit; audited; applicant told; preparation runs); every country for consultancies; contrast pass (blue labels, black values, slate nav, white on the identity card)', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    return sv.includes("app.get('/api/admin/cockpit'") && sv.includes("app.post('/api/admin/cases/start'") && sv.includes("event: 'ADMIN_STARTED_CASE'") && sv.includes('credits_consumed: 0, prep_status') && f.includes('async function adminCockpit(') && f.includes('Today, in one paragraph') && f.includes('id="viewSwitch"') && f.includes("localStorage.getItem('ffAdminView')") && f.includes('async function adminStartCase(') && f.includes('Start a case for them') && f.includes("window._i18n.origins)||{PK:{name:'Pasktan'}}".replace('Pasktan','Pakistan')) && f.includes('body.light #nav button{color:#2F3E5C!important') && f.includes('body.light .pid-card h2');
  })()],
  ['R10400: readability final (every inline colour becomes ink unless it carries meaning; dark inline surfaces become white), scrollbars truly black and 14 px (the old white scrollbar-color rules removed), CV upload opens the profile, step 1 renamed, profile tabs About/Documents/Settings/Options (journey cards moved to Options), currency by profile origin then IP geolocation (cached) then browser, prices shown as $19 (Rs 5,300)', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8');
    return f.includes('body.light [style*="color:#"],body.light [style*="color: #"]') && !f.includes('scrollbar-color:rgba(255,255,255,.85)') && !f.includes('scrollbar-color:#FFFFFF transparent') && f.includes('*{scrollbar-color:#0B1B3A #E3E9F6;scrollbar-width:auto}') && f.includes('::-webkit-scrollbar{width:14px;height:14px}') && f.includes("setTimeout(()=>openFinder(true),700)") && !f.includes("['journey','Journey',[") && sv.includes('async function geoCountry(') && sv.includes('async function visitorCountryAsync(') && sv.includes("app.get('/api/me/visitor'") && f.includes("('+esc(loc.display)+')");
  })()],
  ['R10500 missed-instruction sweep: profession is a real filter (family/words, not full text); portal login with the ForiForeign address recorded as consent; portal watch offered from the visa desk; profile card shows every CV fact (DOB, nationality, address, computed experience, education, jobs, skills, languages, tests, certifications, licences, awards...); Mail has a Drafts group; loading state carries the mark; sticky primary action on phones; MOU confidentiality clause; consultancy creates the client account + forimail address (born in its workspace, never for an existing account); Opportunities heading matches the nav', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const pr = fs.readFileSync(__dirname + '/../lib/partnerships.js', 'utf8');
    return sv.includes('q._profFilter = syn.map') && !sv.includes("if (q.profession) q.text = String(q.profession).slice(0, 60); const r") && sv.includes("'portal_login_with_ff_address'") && f.includes('id="pfUseFF"') && f.includes('_vzWatchBtn') && f.includes("['Experience (computed)'") && f.includes("['Language tests'") && f.includes("['drafts','Drafts']") && f.includes('id="mailDrafts"') && f.includes('/brand/ff-mark-64.png" alt="" width="44"') && f.includes('.ff-appview>div>.row:last-child') && pr.includes('confidentiality: the commercial terms') && sv.includes("app.post('/api/org/:id/clients/:cid/account'") && sv.includes("signup_host: 'ff-crm'") && f.includes('Create account &amp; mailbox for this client') && f.includes('<h2 style="margin:0">Opportunities</h2>');
  })()],
  ['R10500 catch-up of missed instructions: visa step inside the case (suggested route, open file, portal connect with FF-address consent); mobility profile covers both lanes; free preview link kept on the landing; cockpit shows SEO pages, briefs, enrolled, CRM clients; every FF-CRM client gets a ForiForeign address at creation, inbound replies are filed to the client and the consultancy is told, mail listed on the client screen', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const o = fs.readFileSync(__dirname + '/../lib/orgs.js', 'utf8'); const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return f.includes('Visa step for this case') && f.includes('Open the visa file') && !L.includes('See 3 verified matches') && sv.includes("seo: { pages: seoPages, briefs, visits30: seoHits }") && f.includes("['SEO pages live'") && o.includes('row.apply_email = cand') && sv.includes("eq('apply_email', mine).maybeSingle()") && sv.includes("app.get('/api/org/:id/clients/:cid/mail'") && f.includes('async function clientMailLoad(') && f.includes('Client mailbox') && mg.includes('clients add column if not exists apply_email');
  })()],
  ['R10600: no pre-signup preview anywhere (free = no charge after sign-up until a case is processed); company only, never a person, in pages, letters, signers and defaults', (() => {
    const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const st = fs.readFileSync(__dirname + '/../lib/settings.js', 'utf8'); const pr = fs.readFileSync(__dirname + '/../lib/partnerships.js', 'utf8'); const po = fs.readFileSync(__dirname + '/../lib/prospecting.js', 'utf8');
    const pages = fs.readdirSync(__dirname + '/../public').filter(x => x.endsWith('.html') && x !== 'index.min.html').every(x => !/Waseem|Khalid|Downtown/i.test(fs.readFileSync(__dirname + '/../public/' + x, 'utf8')));
    return !L.includes('id="pv"') && !L.includes('pvRun') && !L.includes('3 verified matches') && !L.includes('See 3 matches') && pages && !/Waseem|Khalid/.test(st) && !/Waseem|Khalid/.test(pr) && !/Waseem|Khalid/.test(po) && !f.includes('land-founder"><div');
  })()],
  ['R10600: no pre-signup preview and no personal story anywhere; the LIVE extraction pipeline (docs.js) now asks for and writes the full profile fact set through a shared mapper (30+ fields, totals computed with month precision)', (() => {
    const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8'); const R = require('../lib/reader.js'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    const r = R.applyExtractedFacts({ education: [{ degree: 'PhD Pharmacology', year: '2026' }, { degree: 'Pharm-D', year: '2018' }], experience: [{ role: 'Pharmacist', org: 'X', start: '2019-01', end: 'present' }, { role: 'Y', org: 'Z', start: '2018-03', end: '2022-01' }], skills: ['a'], date_of_birth: '1996-03-02' }, {});
    return !L.includes('id="pv"') && !L.includes('pvRun') && !L.includes('3 verified matches') && !/waseem|founder/i.test(L) && !f.includes('<div class="land-founder">') && d.includes('"language_tests":[{"test"') && d.includes("require('./reader').applyExtractedFacts(v, p)") && r.patch.experience_years > 8 && r.patch.experience_years < 10 && r.patch.degree_level === 'phd' && r.patch.highest_degree_year === '2026' && r.patch.date_of_birth === '1996-03-02';
  })()],
  ['R10700 FF-CRM full solution: own partner universities (CRUD, CSV import, priority-first for the consultancy\'s clients, protected from platform outreach), finance (bank accounts as records, expenses, disputes, commissions received, P&L by branch and month, lead sources), lead capture (lead email address, own WhatsApp number + own AI key with webhook verify, public API POST /api/v1/leads), screens in FF-CRM, CRM page updated', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const o = fs.readFileSync(__dirname + '/../lib/orgs.js', 'utf8');
    return sv.includes("app.post('/api/org/:id/partners'") && sv.includes("app.post('/api/org/:id/partners/import'") && sv.includes('async function orgPartnerNames(') && sv.includes("org_partner: true") && sv.includes("app.get('/api/org/:id/finance/report'") && sv.includes("app.post('/api/org/:id/expenses'") && sv.includes("app.post('/api/org/:id/disputes'") && sv.includes("app.post('/api/org/:id/bank-accounts'") && sv.includes("app.get('/api/org/:id/lead-channels'") && sv.includes("app.post('/api/hooks/whatsapp/:id'") && sv.includes("app.post('/api/v1/leads'") && sv.includes("/^leads\\./.test(mine)") && fs.existsSync(__dirname + '/../lib/byoc.js') && f.includes('async function orgPartnersPanel(') && f.includes('async function orgFinancePanel(') && f.includes('async function orgLeadChannelsPanel(') && f.includes('Your partner · P') && mg.includes('org_partners') && mg.includes('org_expenses') && mg.includes('org_disputes') && o.includes("'finance.write'") && C.includes('Sixteen modules, one desk') && C.includes('Own partner priority');
  })()],
  ['R10800 Portal Assist + CRM gaps: extension v1.7 (FF_PORTAL_ASSIST, visible panel, human-completed security, status + screenshot back), one-time handoff tokens for the applicant and for the consultancy on behalf with consent, status words → visa file update, screenshots stored; follow-up sequences (1/3/7 days, stop on stage change), My day tasks, broadcasts by stage/branch (email or own WhatsApp), duplicate detection on client creation, lost reasons, by-consultant report, sub-agent payables', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    const zip = fs.statSync(__dirname + '/../public/foriforeign-apply-assistant.zip').size;
    return zip > 15000 && sv.includes("app.post('/api/me/portals/:id/handoff'") && sv.includes("app.post('/api/org/:id/clients/:cid/portals/:pid/handoff'") && sv.includes("app.post('/api/me/portals/:id/status'") && sv.includes('function portalStatusFromText(') && f.includes('async function portalAssist(') && f.includes("type:'FF_PORTAL_ASSIST'") && f.includes('Connect &amp; fetch status') && sv.includes('async function leadFollowups(') && sv.includes("QUEUE.register('lead_followups'") && ag.includes("'lead_followups'") && sv.includes("app.get('/api/org/:id/my-tasks'") && sv.includes("app.post('/api/org/:id/broadcast'") && sv.includes("duplicate_of: dup[0]") && sv.includes("app.post('/api/org/:id/clients/:cid/lost'") && sv.includes('by_consultant: byConsultant') && sv.includes('sub_agent_payable_usd') && f.includes('async function orgTodayPanel(') && f.includes('async function orgFollowupsPanel(') && f.includes('Mark lost · reason') && mg.includes('lost_reason') && mg.includes('org_broadcasts');
  })()],
  ['R10900 smart status + smart filling: decision from mail first, manual update second, one automated check after the usual window, never a second attempt, humans told; manual status route; filler v2 with 60+ profile fields, origin-aware nationality, smart selects/dates/radios; full assist packet with the ForiForeign address; no per-action consent friction', (() => {
    const ck = fs.readFileSync(__dirname + '/../lib/checkins.js', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return ck.includes('async function smartStatus(') && ck.includes('if (f.manual_status_at || f.auto_checked_at) continue;') && ck.includes("ForiForeign will not try again") && sv.includes("app.post('/api/visa/desk/:id/manual'") && f.includes('Update status myself') && sv.includes("email: pr.apply_email || ''") && sv.includes('ielts_listening') && !sv.includes("'portal_assist_handoff'") && sv.includes('exp: Date.now() + 7 * 86400000') && mg.includes('auto_checked_at') && fs.statSync(__dirname + '/../public/foriforeign-apply-assistant.zip').size > 15000;
  })()],
  ['R11000 FF-CRM door + guides + support: /crm and /crm/<slug> open the staff sign-in (never the applicant landing) and land on FF-CRM; Your door panel with copy link; two generated guides (applicant help.html, crm-guide.html) from docs/guides.json rebuilt on every start; in-app Help & support with FAQs and a ticket routed to the consultancy owner (to ForiForeign only when asked)', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const pk = JSON.parse(fs.readFileSync(__dirname + '/../package.json', 'utf8')); const H = fs.readFileSync(__dirname + '/../public/help.html', 'utf8'); const CG = fs.readFileSync(__dirname + '/../public/crm-guide.html', 'utf8'); const G = JSON.parse(fs.readFileSync(__dirname + '/../docs/guides.json', 'utf8'));
    return sv.includes("app.get(['/crm', '/crm/:slug']") && sv.includes("app.get('/api/org/:id/door'") && f.includes("renderAuth('signin','crm')") && f.includes('FF-CRM · Staff sign-in') && f.includes("localStorage.getItem('ffCrmDoor')==='1'") && f.includes('async function orgDoorPanel(') && f.includes('async function crmHelp(') && f.includes('Help &amp; support · FF-CRM') && sv.includes("handled_by: orgId && !toPlatform ? 'consultancy' : null") && pk.scripts.prestart.includes('build-guides') && H.includes('Applicant Guide') && CG.includes('Staff and Owner Guide') && G.crm.sections.length >= 9 && fs.existsSync(__dirname + '/../public/help-data.json');
  })()],
  ['R10900 visa status strategy: email first (source marked), manual update by applicant or consultant, ONE portal check after the country\'s processing window (never daily, never a retry; on failure both are told to check by hand); handoff simplified (no extra consent step, 24 h token); assistant v1.8 fills work-lane and profile fields and the intended course; CRM page stats strip and WhatsApp support button on public pages', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const V = require('../lib/visa_strategy.js'); const cb = fs.readFileSync(__dirname + '/../lib/casebrain.js', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const L = fs.readFileSync(__dirname + '/../public/landing.html', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return typeof V.planAfterSubmit === 'function' && typeof V.manualUpdate === 'function' && typeof V.sweep === 'function' && fs.readFileSync(__dirname + '/../lib/visa_strategy.js', 'utf8').includes("eq('check_attempts', 0)") && sv.includes("QUEUE.register('visa_status_sweep'") && sv.includes("app.post('/api/visa/desk/:id/manual'") && sv.includes("app.post('/api/org/:id/clients/:cid/visa/:vid/manual'") && sv.includes('VSTRAT.planAfterSubmit(c.id)') && cb.includes("decision_source: 'email'") && ag.includes("'visa_status_sweep'") && !sv.includes("'portal_assist_handoff'") && sv.includes('exp: Date.now() + 7 * 86400000') && f.includes('Update status by hand') && f.includes('p.profile._case=') && mg.includes('check_attempts') && C.includes('class="stats"') && C.includes('wa-support') && L.includes('wa.me/923455216903');
  })()],
  ['R11100 PR pathways: 54 structured, sourced rows (42 verified, 12 marked re-verify), pr_pathways table seeded on boot, public and personal APIs (years, target date, progress, needs, citizenship target), nightly policy watch on PR sources with change flags and notifications, admin reseed/edit, PR card in the app, CRM page line', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const pw = fs.readFileSync(__dirname + '/../lib/policywatch.js', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const seed = require('../lib/pr_seed.js').seed; const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8');
    return seed.length === 54 && seed.every(r => r.source_url && r.pr_route && r.confidence) && new Set(seed.map(r => r.country_code)).size === 54 && sv.includes("app.get('/api/pr/:cc'") && sv.includes("app.get('/api/me/pr-plan'") && sv.includes('async function prSeedIfEmpty(') && sv.includes("app.put('/api/admin/pr/:cc'") && pw.includes('async function sweepPR(') && ag.includes('sweepPR()') && mg.includes('pr_pathways') && f.includes('async function pathwayLoad(') && f.includes('Your pathway manager') && C.includes('through visa tracking to the PR pathway') && C.includes('PR pathways, 54 countries');
  })()],
  ['R11200 Pathway Membership: free discovery + readiness band + monthly pulse for everyone; membership ($9/$79) opens breakdown, gaps with how-to-close, document readiness, action plan, weekly reassessment, immediate alerts, human review; upsell only after 7 days AND a meaningful moment; events detected on profile/document changes and sweeps; pricing page block', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const P = require('../lib/pathway.js'); const S = require('../lib/settings.js'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const pr = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8');
    const s1 = P.score({ degree_level: 'bachelors', experience_years: 1, language_tests: [] }, { language: 'B2 English', years_to_pr: 5 }, {}); const s2 = P.score({ degree_level: 'masters', experience_years: 4, language_tests: [{ test: 'IELTS', score: '7' }], date_of_birth: '1995-01-01' }, { language: 'B2 English', years_to_pr: 5 }, { visa_status: 'granted', days_lived: 1900 });
    return s1.score < 40 && s1.band === 'Not yet' && s2.score >= 90 && s2.band === 'Ready to plan' && s1.gaps[0] && s1.gaps[0].how && S.DEFAULTS.pathway.month_usd === 9 && sv.includes("app.get('/api/me/pathway'") && sv.includes("const upsell = !member && ageDays >= (pc.free_days_before_upsell || 7) && meaningful") && sv.includes("app.post('/api/me/pathway/review'") && sv.includes("kind: 'pathway_review'") && sv.includes("key === 'pathway_month'") && sv.includes("QUEUE.register('pathway_sweep_members'") && sv.includes("require('./lib/pathway').detect(p.userId, { notify: true })") && ag.includes("'pathway_sweep_members'") && ag.includes("'pathway_sweep_free'") && mg.includes('pathway_events') && f.includes('async function pathwayLoad(') && f.includes('Your pathway manager') && f.includes('Pathway Membership · $') && f.includes('Ask a human') && pr.includes('Pathway Membership');
  })()],
  ['R11300 O₂S: one package from the offer letter to settlement and PR ($100, local currency in brackets) replaces Basic+/Smart+/Premium+ on the dashboard offer, checkout, settlement and pricing page', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const S = require('../lib/settings.js'); const pr = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8');
    return S.DEFAULTS.plus.tiers.length === 1 && S.DEFAULTS.plus.tiers[0].key === 'o2s' && S.DEFAULTS.plus.tiers[0].usd === 100 && sv.includes("key === 'o2s' || /_plus$/.test(key)") && sv.includes("p.addon_key === 'o2s' ||") && sv.includes("Offer letter to settle and get PR abroad") === false && sv.includes("pt.name + ' · ' + pt.long") && sv.includes("' (' + loc.display + ')'") && pr.includes('one payment, ever') && pr.includes("o2.long") && !pr.includes('Basic +');
  })()],
  ['R11400 profile as one section: no tabs; identity card with profile score, origin and language selects at the top; facts in a uniform two-column grid; questions as dropdowns, trimmed to what no document tells; journey tools (offers, visa readiness, portal watch, family, pathway manager, after the visa) moved to the dashboard; extraction reads DOCX/TXT and any language; runtime contrast guard that repairs any dark surface or pale text it finds', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8');
    return f.includes("const PTABS=[['about','Profile',[") && f.includes('function journeyToolsHtml(){') && f.includes('id="journeyTools"') && f.includes('Visa, family &amp; residence tools') && f.includes('id="profScore"') && f.includes('id="pidOrigin"') && f.includes('id="pidLang"') && f.includes('class="pf-grid"') && f.includes("['budget_pkr_per_year','Budget per year','select'") && f.includes("['dependants','Dependants travelling','select'") && f.includes('Profile questions <span') && f.includes('_ffContrastFix') && d.includes('wordprocessingml') && d.includes('any language');
  })()],
  ['R11500 dashboard redesign: status tracker (not buttons) mapped from every engine stage incl. qualify/travel/pr, one next-step card with human CTA (Complete Profile / View Matches / Review & Apply...), state boxes with real counts and self-explaining empty states (matches, applications by state, documents ready/missing/review, attention items), CV card with file/date/Update, duplicate bottom next card removed, developer tools row removed, tools collapsed under Helpful tools, pricing offer only from the offer stage, engine humanises every missing field and links to the right screen, staff status control per case with audit + recompute + client notification', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const je = fs.readFileSync(__dirname + '/../lib/journey_engine.js', 'utf8');
    return f.includes("const M={discover:0,qualify:0,match:1,prepare:2,apply:3,reply:4,offer:5,visa:6,travel:7,arrival:7,settle:8,pr:8}") && f.includes('class="tracker" role="list"') && f.includes('class="next-card"') && f.includes("'Complete Profile'") && f.includes("'Review & Apply'") && f.includes('No matches yet') && f.includes('No applications yet') && f.includes('Nothing needs your attention') && f.includes('class="card cvcard"') && !f.includes('id="nextCard"') && !f.includes('Tools: finder, offer check, Ask ForiForeign') && f.includes('Helpful tools') && f.includes("${STG>=5?'<div id=\"offersForMe\"></div>':''}") && sv.includes('out.dash = { matches,') && sv.includes("app.post('/api/admin/applications/:id/status'") && je.includes("const H = { highest_level: 'your highest degree'") && je.includes("stage = 'reply'") && f.includes("Set status</span>");
  })()],
  ['R11510: notifications panel is a readable dialog (white, 560px, full text with line breaks, date, Open link, visible Close button, Escape closes); a stale or raw-keyed next action is recomputed synchronously before the dashboard answers', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8');
    return f.includes('role="dialog" aria-label="Notifications"') && f.includes('✕ Close') && f.includes('white-space:pre-wrap;line-height:1.5') && f.includes("if(e.key==='Escape')") && sv.includes("const stale = !pn || !pn.next_action || /[a-z]+_[a-z]+/.test(String((pn.next_action || {}).text || ''))") && sv.includes('await JE.compute(uid)');
  })()],
  ['R11600 FF-CRM operations audit: work queues (14, computed from tasks, documents, applications, mail, payments, stage timing) that open exactly those clients; role-gated workspace header (owner/manager only for Money, Partners, Reports, Team, Import); filters by staff, country, priority; board cards show assignee, priority, days in stage, idle; client screen assign/reassign, priority, request-from-client; requests appear on the client\'s dashboard; stage changes stamped; My day at the top of the workspace; sections for Money and Partners', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const o = fs.readFileSync(__dirname + '/../lib/orgs.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return sv.includes("app.get('/api/org/:id/queues'") && sv.includes("add('stuck', c.id)") && sv.includes("app.post('/api/org/:id/clients/:cid/request'") && sv.includes('extra.stage_changed_at = new Date().toISOString()') && sv.includes('c.days_in_stage = Math.floor') && sv.includes('out.consultant_requests = reqs') && o.includes("'assigned_to', 'priority'") && o.includes('stage_changed_at,last_activity_at') && f.includes("['owner','manager'].includes(org.my_role)?") && f.includes('async function wsQueuesLoad(') && f.includes('function wsSection(') && f.includes('id="wsStaff"') && f.includes("cl.days_in_stage!=null?cl.days_in_stage+'d in stage'") && f.includes('Request from client') && f.includes('Your consultant needs from you') && mg.includes('for_client') && mg.includes('stage_changed_at');
  })()],
  ['R11700 FF-CRM gaps and blunders: P&L now reads the real finance kinds; broadcasts owner/manager only; consultants cannot reassign/move clients; member emails hidden from non-managers; invitations land on the consultancy\'s own domain and can be resent (invited_at); archive/restore; owner deletes a record with audit; owner exports everything; WhatsApp webhook verified by Meta signature when the app secret is set; inbound WhatsApp matches phone or whatsapp and never duplicates; follow-ups skip imported/walk-in leads unless enabled; archived clients leave lists, board and queues', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const o = fs.readFileSync(__dirname + '/../lib/orgs.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return sv.includes("if (f.kind === 'payment_received') bump(b, m, 'income'") && !sv.includes("f.kind === 'fee_received'") && sv.includes("app.post('/api/org/:id/broadcast', auth, async (req, res) => { try { await ORGS.requireOrg(req, req.params.id, 'org.settings')") && sv.includes("for (const k of ['owner_user_id', 'assigned_to', 'branch', 'sub_agent_user_id', 'sub_agent_share_pct']) delete (req.body || {})[k]") && sv.includes("email: undefined, invite_email: undefined") && sv.includes("app.post('/api/org/:id/clients/:cid/invite'") && sv.includes('async function orgHomeUrl(') && sv.includes("app.post('/api/org/:id/clients/:cid/archive'") && sv.includes("app.delete('/api/org/:id/clients/:cid'") && sv.includes("app.get('/api/org/:id/export'") && sv.includes("x-hub-signature-256") && sv.includes("req.rawBody = buf.toString('utf8')") && sv.includes("or('whatsapp.ilike.%' + num.slice(-9)") && sv.includes("['csv', 'walk-in', 'referral'].includes(String(c.source || ''))") && o.includes("query = query.is('archived_at', null)") && f.includes('Resend invitation') && f.includes('async function orgExportPanel(') && f.includes('id="waSecret"') && mg.includes('archived_at') && mg.includes('invited_at');
  })()],
  ['R11700 gaps + blunders: (1) BLUNDER the profile extractor wrote non-existent columns so no CV fact ever reached the profile; columns added and the writer filters to known columns; (2) WhatsApp hook accepts only the org\'s own phone_number_id and is rate-limited; (3) lead mailbox ignores auto-replies/no-reply senders and floods; (4) API leads endpoint rate-limited per key; (5) caps default present; (6) ENV.md documents every variable', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const R = require('../lib/reader.js'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const st = fs.readFileSync(__dirname + '/../lib/settings.js', 'utf8'); const env = fs.readFileSync(__dirname + '/../ENV.md', 'utf8');
    const r = R.applyExtractedFacts({ methods: 'x', languages: [{ language: 'Urdu', level: 'native' }], bogus_field: 'y' }, {});
    return mg.includes('add column if not exists methods text') && mg.includes('add column if not exists languages jsonb') && mg.includes('add column if not exists target_countries jsonb') && R.PROFILE_COLS && r.patch.methods === 'x' && Array.isArray(r.patch.languages) && r.patch.bogus_field === undefined && sv.includes("LIMITER.hit('wahook:'") && sv.includes("pid !== String(wa.phone_id)") && sv.includes("LIMITER.hit('leadmail:'") && sv.includes("LIMITER.hit('apileads:'") && st.includes('caps: {}') && env.includes('RESEND_API_KEY') && env.includes('INTAKE_SECRET') && env.includes('FF_DATA_KEY');
  })()],
  ['R11800 reach-99 set: outcome calibration of the chance estimate (nightly, blended by evidence); AI reading of ambiguous portal status; PR verify-now against the source; per-org queue thresholds; cohort conversion; marketing opt-out honoured by broadcasts/follow-ups; BYOC replies with conversation memory; lead scores; cockpit 7-day trends; key rotation tool; security posture; tax on plan invoices; sitemap ping; per-portal field maps in the assistant (v1.9); renewals at 90 days', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const su = fs.readFileSync(__dirname + '/../lib/success.js', 'utf8'); const by = fs.readFileSync(__dirname + '/../lib/byoc.js', 'utf8'); const cr = fs.readFileSync(__dirname + '/../lib/crypto.js', 'utf8'); const st = fs.readFileSync(__dirname + '/../lib/settings.js', 'utf8'); const pm = require('../lib/portal_maps.js'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const pe = fs.readFileSync(__dirname + '/../lib/partners_engine.js', 'utf8');
    return su.includes('success_calibration') && typeof require('../lib/success.js').calibrate === 'function' && ag.includes("'success_calibrate'") && sv.includes('async function portalStatusSmart(') && sv.includes("app.post('/api/admin/pr/:cc/verify'") && sv.includes('TH.stuck_days') && sv.includes('cohorts: Object.entries(cohorts)') && sv.includes('filter(x => !x.no_marketing)') && by.includes('Conversation so far') && sv.includes('function leadScore(') && sv.includes('trends.signups') && sv.includes("app.post('/api/admin/security/rotate-key'") && cr.includes('async function reencryptAll(') && st.includes('security: { totp_required') && st.includes('tax: { default_pct') && sv.includes('tax_pct: taxPct') && sv.includes('async function pingSitemaps(') && pm.forUrl('https://apply.ukvi.homeoffice.gov.uk/x') && sv.includes('pkg.portal_map') && mg.includes('no_marketing') && !pe.includes('60 * 86400000');
  })()],
  ['R11900 acquisition + intelligence: programmatic pages for 54 countries × 40 professions and × 25 fields (3,510 URLs, FAQ JSON-LD, sourced), public verified-job pages with JobPosting schema, llms.txt, intent capture from public pages into sign-up (lane, country, profession, opportunity), sitemap carries all of it; Ask ForiForeign is a context-aware assistant (own state + sourced rules, cited, actions, human escalation) available on every screen; opportunity-by-id route', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const S = require('../lib/seo.js');
    const h = S.professionPage('GB', 'nurse', 'work'); const j = S.jobPage({ id: 'x', kind: 'work', title: 'Nurse', institution: 'Trust', country_code: 'GB', description: 'd', verified_at: '2026-09-01' });
    return h && h.includes('"@type":"FAQPage"') && S.programmaticUrls().length > 3000 && j && j.includes('"@type":"JobPosting"') && sv.includes("app.get(['/study-in/:cc/:prof', '/work-in/:cc/:prof']") && sv.includes("app.get('/jobs/:id'") && sv.includes("app.get('/llms.txt'") && sv.includes("app.get('/api/intent'") && sv.includes("app.post('/api/me/intent'") && sv.includes('SEO.programmaticUrls()') && sv.includes('INSTANT LAYER') && sv.includes('"actions":[{"label"') && f.includes('ff-ask-fab') && f.includes('async function openOpp(') && sv.includes("app.get('/api/opportunities/:id'");
  })()],
  ['R12000 progressive profiling: nothing blocks matching (REQUIRED_FOR_MATCH empty); checklist is stage- and case-aware (discover = CV only; case = posting requirements + lane; visa = route documents) with why/who per document; the vault shows one calm line before a case exists, no red banner, the CV row never marked optional; profile questions appear by stage (3 early, ~12 mid, all late); CRM client form wording under strict separation', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const V = require('../lib/vault.js'); const M = require('../lib/mobility.js');
    return Array.isArray(M.REQUIRED_FOR_MATCH || []) && fs.readFileSync(__dirname + '/../lib/mobility.js', 'utf8').includes('const REQUIRED_FOR_MATCH = [];') && V.CHECKLISTS.discover && V.CHECKLISTS.discover.required.length === 1 && V.docsFromRequirements('IELTS 6.5 and references').includes('lor') && V.WHY.passport && sv.includes("if (['discover', 'qualify', 'match'].includes(stage)) purpose = 'discover'") && sv.includes('ctx.requirements_text') && f.includes('All you need now') && f.includes("reqSet.add('cv')") && f.includes("nothing else is needed yet") && f.includes("const EARLY=['lane','target_countries','field']") && f.includes('<label>Email (optional)</label>') && f.includes("asked by '+r.who");
  })()],
  ['R12100: after the CV is read the finder opens pre-filled from it (lane from experience/headline, profession → field, target countries, experience band); the lane popup is retired; the dashboard next step opens the finder', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    return f.includes('PREFILL FROM THE CV') && f.includes("setTimeout(()=>openFinder(true),700)") && f.includes('function laneChooser(){return false;') && f.includes("'Find my matches'") && f.includes('onclick="openFinder()">${nextCta.label}');
  })()],
  ['R12150: the same finder, with every lane filter, serves the consultant for a client; the chosen filters travel as prefs to the client\'s search', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8');
    return f.includes('async function clientDiscover(cid,filters)') && f.includes('window._finderClient=cid') && f.includes('if(window._finderClient){') && sv.includes("prefs: (req.body || {}).prefs || {}") && f.includes("sel('visaSel'") && f.includes("sel('tuition'") && f.includes("sel('salaryBand'") && f.includes("sel('instruction'");
  })()],
  ['R12200 two payment stages verified and the CRM gap closed: direct applicants - Basic/Smart/Premium per case until the offer, then O₂S once; consultancy clients - covered by the consultancy\'s plan (entitled, add-on gates, free-tier limits) and never asked to pay ForiForeign', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const S = require('../lib/settings.js');
    return sv.includes('async function orgCovers(') && sv.includes("if (await orgCovers(userId)) return true;   // a consultancy") && sv.includes("if (await orgCovers(userId)) return { ok: true, org: true }") && sv.includes("if (await orgCovers(userId)) return true;   // covered by the consultancy") && sv.includes("code: 'CONSULTANCY_CLIENT'") && (S.DEFAULTS.packages.tiers || []).length === 3 && S.DEFAULTS.plus.tiers[0].key === 'o2s';
  })()],
  ['R12250 local currency in brackets on consultancy plans (offering carries local_month/local_year; pricing and CRM pages show them); applicant packages already show $ (local); CRM records cash received per client in one tap (fees, receipts, refunds, costs, commissions in/out) for its own cash management and P&L', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const pr = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    return sv.includes("local_month: lp(Number(t.usd_month) || 0)") && pr.includes('p.local_month.display') && C.includes('crmLocalPrices') && f.includes('async function recordCash(') && f.includes('Record cash received') && f.includes("('+esc(loc.display)+')") && f.includes('<option value="payment_received">');
  })()],
  ['R12300 final de-duplication: opportunity cards carry one Details toggle, one primary action and Save (the two deep tools live inside the fold); public pages carry one CTA per place (pricing: one Start free; FF-CRM: hero + bottom Try free, header Sign in); rendered dashboard, profile, workspace, client and team screens have no duplicate buttons or headings', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const P = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8');
    return (f.match(/>Full intelligence<\/button>/g) || []).length === 0 && f.includes('Full intelligence report</a>') && f.includes('.card .card-more{display:none}.card.open .card-more{display:block}') && (P.match(/>Start free</g) || []).length === 1 && (C.match(/>Try FF-CRM free</g) || []).length === 2 && (C.match(/>Request a demo</g) || []).length === 0;
  })()],
  ['PHASE 2 regressions: BUG-002 personal workspace no longer counts as a consultancy (consultancyClients helper, 7 call sites); BUG-003 doc_type set at upload so the CV is seen at once; BUG-004 journey recomputed synchronously after an upload; BUG-005 Explore hides dismissed postings; journey suite (28 checks) in npm test', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8');
    return sv.includes('async function consultancyClients(') && (sv.match(/consultancyClients\(/g) || []).length >= 6 && !sv.includes("from('clients').select('org_id').eq('user_id', req.userId).limit(1)") && d.includes("doc_type: kind === 'cv' ||") && sv.includes("try { await JE.compute(req.userId); } catch (e) {}\n    if (ok) QUEUE.enqueue('profile_extract'") && sv.includes("'dismissed:' + req.userId).maybeSingle(); const hide = new Set(") && pk.includes('journey.test.js') && !sv.includes('/__memdb/dump');
  })()],
  ['PHASE 3 regressions: BUG-006 degree-level regex matched "diploma" as masters (fixed, word boundaries); BUG-007 overlapping jobs were summed (union of intervals now); DISC-001 document versions (version, supersedes_id, superseded_at; lists hide superseded); DATA-002/003 mobility mirrors and writes lane_pref/target_countries; profile_provenance per extracted field; extraction suite (21 checks) in npm test with a canned model answer', (() => {
    const R = require('../lib/reader.js'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8'); const mb = fs.readFileSync(__dirname + '/../lib/mobility.js', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8'); const rt = fs.readFileSync(__dirname + '/../lib/router.js', 'utf8');
    const lv = R.applyExtractedFacts({ education: [{ degree: 'Post-RN Diploma Critical Care' }, { degree: 'BSc Nursing' }] }, {}).patch.degree_level;
    const yrs = R.applyExtractedFacts({ experience: [{ start: '2016-01', end: '2018-12' }, { start: '2018-01', end: '2020-12' }] }, {}).patch.total_experience_years;
    return lv === 'bachelors' && Math.abs(yrs - 5) < 0.01 && mg.includes('profile_provenance') && mg.includes('superseded_at') && d.includes('supersedes_id = prev[0].id') && d.includes("patch.profile_provenance = prov") && mb.includes('upd.lane_pref = m.lane') && mb.includes("m.lane = p.lane_pref") && pk.includes('extraction.test.js') && rt.includes('FF_FAKE_AI_FILE') && rt.includes("process.env.NODE_ENV !== 'production'");
  })()],
  ['PHASE 4 regressions: DISC-002 one identity per opportunity (lib/oppkey canonicalUrl + fingerprint without deadline/source; ingest dedupes by url_key, fingerprint, url with separate queries; Explore collapses legacy duplicates before redaction); unreachable official pages are PENDING with a note, re-verified nightly (verify_pending), closed after 5 failures or 404/410; BUG-008 work never below level; BUG-009 experience and test score compared; no percentage when nothing is known; discovery suite (21) in npm test', (() => {
    const K = require('../lib/oppkey.js'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const en = fs.readFileSync(__dirname + '/../lib/engine.js', 'utf8'); const mt = fs.readFileSync(__dirname + '/../lib/match.js', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8');
    return K.canonicalUrl('http://www.a.org/x/?utm_source=1#f') === 'https://a.org/x' && en.includes("K.canonicalUrl(it.url)") && en.includes("status: it._reachable === false ? 'pending' : 'verified'") && !en.includes("or('url.eq.' + it.url") && sv.includes('async function verifyPending(') && ag.includes("'verify_pending'") && sv.includes('legacy duplicates (same canonical URL or fingerprint)') && mt.includes("String(opp.kind || '') !== 'work') overqualified = true") && mt.includes("status = 'needs_profile'") && mt.includes("Requires ' + reqYrs + '+ years experience") && mg.includes('url_key') && pk.includes('discovery.test.js');
  })()],
  ['PHASE 5 regressions: DISC-006 durable outbox (mail_outbox, idempotency key, backoff, outbox_flush every 10 min, failed after 12 tries with a notification); case sends are written first, deduplicated by key, never thrown away when the provider is down; case_messages carry send_status/idempotency_key/provider id; inbound attaches by alias or by the recipient\'s own address only; mail suite (23) in npm test with a simulated provider', (() => {
    const ml = fs.readFileSync(__dirname + '/../lib/mailer.js', 'utf8'); const cb = fs.readFileSync(__dirname + '/../lib/casebrain.js', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const ag = fs.readFileSync(__dirname + '/../lib/agents.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8');
    return ml.includes('async function sendDurable(') && ml.includes('async function flushOutbox(') && ml.includes("onConflict: 'idempotency_key'") && cb.includes('M.sendDurable({ idempotencyKey: idem') && cb.includes("send_status: 'queued', idempotency_key: idem") && !cb.includes("throw new Error('Sending is not configured yet") && sv.includes("QUEUE.register('outbox_flush'") && ag.includes("'outbox_flush'") && mg.includes('mail_outbox') && mg.includes('idempotency_key') && pk.includes('mail.test.js') && ml.includes("process.env.NODE_ENV !== 'production' && process.env.FF_FAKE_MAIL");
  })()],
  ['PHASE 6 regressions: DISC-004 every visa document requirement carries rule text + official source, a requirement class (required/conditional/recommended) with the condition, a normalised state (missing/awaiting verification/needs review/verified/expired), legalisation flags only when the configured rule says so (else an honest note), origin attestation with source; BUG-010 unread uploads are awaiting verification, never verified (doc_status uploaded at upload); manual visa update 404 when not the caller\'s file; deterministic document-vs-profile cross-check; visa suite (20) in npm test', (() => {
    const vi = fs.readFileSync(__dirname + '/../lib/visa.js', 'utf8'); const va = fs.readFileSync(__dirname + '/../lib/vault.js', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8'); const vs = fs.readFileSync(__dirname + '/../lib/visa_strategy.js', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8');
    return vi.includes("requirement: cond ? 'conditional'") && vi.includes('legalisation_note') && vi.includes('origin_attestation: att') && vi.includes("state_label: STATE_LABEL[base.state]") && va.includes("if (!d.doc_status || d.doc_status === 'uploaded' || d.doc_status === 'reading') return") && d.includes("doc_status: 'uploaded'") && vs.includes("e.status = 404; throw e") && sv.includes('deterministic cross-check') && pk.includes('visa.test.js');
  })()],
  ['PHASE 7+8 regressions: DISC-003 a confirmed payment opens the intended case once (opened_by payment, one consume row linked to the payment); BUG-011 card-paid add-ons grant the add-on (grantAddon/expandBundle, idempotent by payment id) and no credits; BUG-012 recording money needs finance.write; BUG-013 undated money reached the P&L as NULL and was dropped (dated at insert); payments (24) and crm (52) suites in npm test', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8');
    return sv.includes("opened_by: 'payment'") && sv.includes('async function grantAddon(') && sv.includes('async function expandBundle(') && sv.includes("if (p.addon_key) { await grantAddon(p); await expandBundle(p); try { await admin().from('audit_log')") && sv.includes("app.post('/api/org/:id/clients/:cid/finance', auth, async (req, res) => { try { const c = await orgClient(req, res, 'finance.write')") && sv.includes("Phase 8 BUG-013") && mg.includes('opened_by') && pk.includes('payments.test.js') && pk.includes('crm.test.js');
  })()],
  ['PHASE 7 closure: refunds carry status refunded (admin filter knows refunded/abandoned); abandoned payments settle on a late webhook; BUG-014 Lemon and Paddle webhooks route consultancy plan purchases to the agency settlement (was: settled as a credit purchase, plan never activated); one active subscription per organisation on renewal; no trial fallback after a paid plan; payments suite 29', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const q = fs.readFileSync(__dirname + '/../lib/quota.js', 'utf8'); const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    return sv.includes("update({ status: 'refunded', refunded_at:") && sv.includes("Phase 7 BUG-014") && sv.includes("(md.org_id && md.credits === '0') ? await settleAgencySubscription(so) : await settleCardPayment(so, 'lemon')") && sv.includes("settleCardPayment(so, 'paddle')") && sv.includes('Phase 7 BUG-015') && sv.includes("one active subscription per organisation") && q.includes('never fall') === false && q.includes("does not fall back to a trial") && f.includes("['','pending','confirmed','failed','refunded','abandoned']");
  })()],
  ['PHASE 9 regressions: jobs carry an idempotency key (no duplicate queued/running work) and a per-job timeout, dead after max attempts, stale running jobs re-queued (reapStale); dangerous admin actions need a typed confirmation and the right role (PURGE, ROTATE, GRANT); DISC-005 national IDs, card numbers and IBANs masked in text sent for extraction with a documented settings flag; admin suite (26) in npm test', (() => {
    const q = fs.readFileSync(__dirname + '/../lib/queue.js', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8'); const st = fs.readFileSync(__dirname + '/../lib/settings.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8');
    return q.includes("idem_key: idem, status: 'queued'") && q.includes('job timeout after') && q.includes('async function reapStale(') && sv.includes("!== 'PURGE'") && sv.includes("!== 'ROTATE'") && sv.includes("!== 'GRANT'") && d.includes('function minimisePII(') && st.includes('pii_to_model: true') && mg.includes('idem_key') && mg.includes('timeout_ms') && pk.includes('admin.test.js');
  })()],
  ['PHASE 10 regressions: accessibility enhancer (onclick anchors keyboard-operable with role=button, inputs labelled from their <label>/title/option, images get alt), visible focus ring, UX-003 dead in-app landing branch removed (auth split layout kept, it is live), public pages have lang/viewport/h1 and no dead internal links, tools/ui-audit.js kept', (() => {
    const f = fs.readFileSync(__dirname + '/../public/index.html', 'utf8');
    const pages = fs.readdirSync(__dirname + '/../public').filter(x => x.endsWith('.html') && x !== 'index.min.html' && x !== 'index.html');
    const basics = pages.every(p => { const x = fs.readFileSync(__dirname + '/../public/' + p, 'utf8'); return /<html[^>]*lang=/.test(x) && /viewport/.test(x) && /<h1/.test(x); });
    return f.includes('ACCESSIBILITY ENHANCER') && f.includes(":focus-visible{outline:3px solid #1683FF") && !f.includes("${mode==='landing'?`<div class=\"auth-card glass\" id=\"authCard\" style=\"text-align:center\">") && f.includes('aria-label="Promotion length in hours"') && basics && fs.existsSync(__dirname + '/../tools/ui-audit.js');
  })()],
  ['PHASE 11: OPS-001 migration runner (tools/migrate.js applies the assembled script once per checksum, ledger in schema_migrations, proven twice on Postgres); OPS-002 CI workflow runs the full suite and build on every push; release runbook (deploy, rollback, backup/restore drill, required env, monitoring, never-in-production flags)', (() => {
    const m = fs.readFileSync(__dirname + '/../tools/migrate.js', 'utf8'); const ci = fs.existsSync(__dirname + '/../.github/workflows/ci.yml') && fs.readFileSync(__dirname + '/../.github/workflows/ci.yml', 'utf8'); const rb = fs.readFileSync(__dirname + '/../RUNBOOK_RELEASE.md', 'utf8');
    return m.includes('schema_migrations') && m.includes('ALL_MIGRATIONS_run_in_order.sql') && ci && ci.includes('npm test') && ci.includes('tools/build-sql.js') && rb.includes('tools/migrate.js') && rb.includes('Rollback') && rb.includes('Never in production');
  })()],
  ['FINAL AUDIT: RLS covers tables created after 0086 (0092 loop, ledger too); extraction reads live documents only; test-mode flags refuse to start on a production URL; public DB-backed GETs rate-limited; build label current', (() => {
    const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const d = fs.readFileSync(__dirname + '/../lib/docs.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const mr = fs.readFileSync(__dirname + '/../tools/migrate.js', 'utf8');
    return mg.includes('0092_rls_late_tables') && d.includes(".eq('generated', false).is('superseded_at', null).order('created_at'") && sv.includes('Refusing to start') && sv.includes("LIMITER.hit('pubjob:'") && sv.includes("LIMITER.hit('intent:'") && mr.includes('alter table public.schema_migrations enable row level security') && (sv.match(/const FF_BUILD = /g) || []).length === 1;
  })()],
  ['R14000 BYOC AI: per-consultancy AI connection (org_ai_connections, encrypted, last4 only), router resolves the tenant and runs inside its key context (never the platform key, never another tenant); AI_NOT_CONNECTED is explicit; health classified and recorded; usage meter scoped by org with an estimate note; fixed platform plans $79/$179/$399 with no application/search caps; technical search guard (dedupe + burst); housekeeping job (temporary data expiry); flood simulation; public copy updated everywhere (index, pricing, crm, guides, SEO, FAQ seed, quota defaults); legacy tier keys growth/scale mapped to professional/business with caps lifted for existing subscriptions; plan-state returns null for uncapped; byoc suite (31) in npm test', (() => {
    const K = require('../lib/ai_keys.js'); const rt = fs.readFileSync(__dirname + '/../lib/router.js', 'utf8'); const g = fs.readFileSync(__dirname + '/../lib/gemini.js', 'utf8'); const a = fs.readFileSync(__dirname + '/../lib/anthropic.js', 'utf8'); const sv = fs.readFileSync(__dirname + '/../server.js', 'utf8'); const S = require('../lib/settings.js'); const q = fs.readFileSync(__dirname + '/../lib/quota.js', 'utf8'); const mg = fs.readFileSync(__dirname + '/../ALL_MIGRATIONS_run_in_order.sql', 'utf8'); const pk = fs.readFileSync(__dirname + '/../package.json', 'utf8'); const C = fs.readFileSync(__dirname + '/../public/crm.html', 'utf8'); const pr = fs.readFileSync(__dirname + '/../public/pricing.html', 'utf8');
    return typeof K.runAs === 'function' && typeof K.keyFor === 'function' && !g.includes('process.env.GEMINI_API_KEY') && !a.includes('process.env.ANTHROPIC_API_KEY') && !rt.includes('process.env.OPENAI_API_KEY') && rt.includes('throw new K.NotConnected(orgId, need)') && sv.includes("app.post('/api/org/:id/ai'") && sv.includes("app.get('/api/org/:id/ai/usage'") && sv.includes('async function searchGuard(') && sv.includes("QUEUE.register('housekeeping'") && S.DEFAULTS.agency.tiers.map(t => t.usd_month).join(',') === '79,179,399' && S.DEFAULTS.agency.tiers.every(t => t.cases_month == null && t.searches_day == null) && q.includes('UNCAPPED') && mg.includes('org_ai_connections') && pk.includes('byoc.test.js') && C.includes('Your account') && pr.includes('own AI account') && q.includes("LEGACY_TIER = { growth: 'professional', scale: 'business' }") && !fs.readFileSync(__dirname + '/../lib/seo.js', 'utf8').includes('Starter 100') && !fs.readFileSync(__dirname + '/../lib/faq_seed.js', 'utf8').includes('Growth 500') && !fs.readFileSync(__dirname + '/../public/index.html', 'utf8').includes('applications/month · ${t.searches_day');
  })()],
