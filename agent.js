...
function userGmail(u) {
  try {
    if (u && u.gmailRefresh) {
      const tok = decrypt(u.gmailRefresh);
      if (tok) return { refresh: tok, addr: u.gmailAddr || u.email || '' };
    }
  } catch (e) {}
  return undefined;
}
...
function stripPlaceholders(t) {
  return String(t || '').replace(/\[(?:insert|your|add|name|date|university|position|supervisor|lab|amount|deadline)[^\]]*\]/gi, '').replace(/\[\s*\]/g, '');
}
function noDashes(t) {
...
function cfg() {
  return {
    ...
    minEngage: parseInt(process.env.MIN_ENGAGE_SCORE || '60'),
    regions: process.env.REGIONS || 'all of Europe with Scandinavia emphasized, plus AUSTRALIA and NEW ZEALAND strongly emphasized (among the easiest and highest-chance destinations for Pakistani researchers, excellent salaries, family friendly visas and perks), UK, Ireland, USA, Canada, Gulf states',
...
async function draftOutreachBatch(researchers, report) {
  ...
  await Promise.allSettled(targets.slice(0, 4).map(async o => {
    ...
    if (!r) return;
    const toEmail = extractEmail(o.note) || o.contact || '';
    try {
      const txt = await claude(
...
async function sendBatch(researchers, report) {
  ...
  for (const m of ready) {
    ...
    try {
      if ((r.role || 'user') !== 'admin' && !userGmail(r)) { await db.log('SKIP_NO_GMAIL', (r.name || m.resId) + ': connect Gmail in Profile'); continue; }
      let attachments = [];
      if (m.type === 'outreach') attachments = await generatedAttachments(m.resId, m.oppId);
      ...
      await sendMail({
        ...
        bcc: r.email || undefined, gmail: userGmail(r)
      });
...
async function sendOne(outboxId, opts) {
  ...
  // Every member sends from their OWN Gmail. Only the owner account may use the shared server token.
  if ((r.role || 'user') !== 'admin' && !userGmail(r)) {
    return { ok: false, error: (r.name || 'This member') + ' has not connected their own Gmail yet. Open Profile and settings, section Sending: your own Gmail, and paste the refresh token. Every member sends from their own address, never anyone else’s.' };
  }
  const creds = userCreds(r);
  let attachments = [];
  if (m.type === 'outreach' || m.type === 'application' || m.type === 'followup') {
    attachments = await generatedAttachments(m.resId, m.oppId);
  }
  const sig = `\n\nBest regards,\n${r.name || ''}\n${r.title || ''}${r.email ? '\n' + r.email : ''}${r.orcid ? '\nORCID: ' + (r.orcid.startsWith('http') ? r.orcid : 'https://orcid.org/' + r.orcid) : ''}${r.schedLink ? '\nBook a call: ' + r.schedLink : ''}`;
  try {
    await sendMail({ to: m.toEmail, subject: m.subject, text: m.body + sig, fromName: r.name, attachments, creds, replyTo: creds ? undefined : (r.email || undefined), bcc: r.email || undefined, gmail: userGmail(r) });
...
async function buildReminders() {
  ...
  // FEATURE: funding cases fully prepared but not yet forwarded -> remind to forward
  const outbox2 = await db.all('Outbox');
  for (const o of opps) {
    if (o.archived === 'yes' || o.status === 'EXPIRED') continue;
    const isFunding = /^Funding: /.test(o.title || '') || o.parentOppId;
    if (isFunding && o.prepDone === 'yes') {
      const sent = outbox2.some(m => m.oppId === o.id && m.status === 'SENT');
      if (!sent && !existing.has(o.id + '|forward_funding')) {
        add.push({ id: uid(), resId: o.resId, oppId: o.id, kind: 'forward_funding', dueOn: o.deadline || today(), note: 'Funding case ready to forward: ' + (o.institution || o.title), status: 'open', createdOn: today() });
      }
    }
  }
...
/* ---------- attachments for a case: agent-written PDFs + ORIGINAL credentials unmodified ---------- */
async function generatedAttachments(resId, oppId) {
  ...
  const gen0 = docs.filter(d => d.resId === resId && d.driveId && !excluded.has(d.id) && /generated from/i.test(d.note || '') &&
    (!oppId || (d.note || '').includes('opp:[' + oppId + ']')));
  const byKind = {};
  for (const d of gen0) { const k = KIND(d); if (!byKind[k] || String(d.updatedOn || '') >= String(byKind[k].updatedOn || '')) byKind[k] = d; }
  const gen = Object.values(byKind);
  const seen = new Set();
  const cred = docs.filter(d => d.resId === resId && d.driveId && !excluded.has(d.id) && !/generated from/i.test(d.note || '') &&
    /(degree|certificat|transcript|publication|reference letter)/i.test(d.type || ''))
    .filter(d => { const n = (d.name || '').toLowerCase(); if (seen.has(n)) return false; seen.add(n); return true; });
  const out = [];
  for (const d of gen.slice(0, 5)) {
    try { out.push({ filename: d.name || (d.type + '.pdf'), content: await fileBuffer(d.driveId), contentType: d.mime || 'application/pdf' }); } catch (e) {}
  }
  for (const d of cred.slice(0, 4)) {
    try { out.push({ filename: d.name || (d.type + '.pdf'), content: await fileBuffer(d.driveId), contentType: d.mime || 'application/pdf' }); } catch (e) {}
  }
  return out;
}
...
async function followups(researchers, report) {
  ...
  for (const m of outbox) {
    ...
    // deadline-aware: if the position deadline is inside 7 days, follow up after just 3 days instead of the normal wait
    let deadlineSoon = false, deadlineStr = '';
    try {
      const oppRow = (await db.all('Opportunities')).find(o => o.id === m.oppId);
      if (oppRow && isDate(oppRow.deadline)) {
        const dl = Math.ceil((new Date(oppRow.deadline) - Date.now()) / 86400000);
        if (dl >= 0 && dl <= 7) { deadlineSoon = true; deadlineStr = oppRow.deadline + ' (' + dl + ' day' + (dl === 1 ? '' : 's') + ' away)'; }
      }
    } catch (e) {}
    if (daysSince < (deadlineSoon ? 3 : c.followupDays)) continue;
    const r = researchers.find(x => x.id === m.resId) || {};
    try {
      const txt = await claude(
`${STYLE}\n\nWrite a very short (under 70 words), courteous, professional follow-up email from ${r.name} to ${m.toName} regarding the earlier message "${m.subject}". Do not guilt-trip, do not repeat the whole pitch; add one small new element of value (e.g. availability for a brief call or a relevant recent result from the researcher's real profile: ${r.pubs}).${deadlineSoon ? " The application deadline is " + deadlineStr + ", so mention gently and respectfully that the applicant wanted to follow up before the deadline passes, without any pressure on the professor." : ""} Respond ONLY with JSON: {"subject":"","body":""}`,
        { search: false, maxTokens: 500 });
      const d = parseJSON(txt);
      d.subject = noDashes(d.subject); d.body = noDashes(d.body);
      if (cfg().autoSend) {
        const fuAtt = deadlineSoon ? await generatedAttachments(m.resId, m.oppId) : [];
        await sendMail({ to: m.toEmail, subject: d.subject, text: d.body + `\n\nBest regards,\n${r.name || ''}`, fromName: r.name, attachments: fuAtt, bcc: r.email || undefined, gmail: userGmail(r) });
...
async function jobDiscover(r, report) {
  ...
  if (wantRemote) scopes.push('fully remote roles hiring internationally that a Pakistan-based PhD pharmacist can do remotely and earn in USD or EUR: clinical trials and clinical research associate, pharmacovigilance and drug safety, medical writing and scientific writing, regulatory affairs, medical affairs, pharmaceutical or medical data roles, health NGO and global health roles, scientific consulting. Match strictly to the applicant degree and CV domain, never suggest roles outside pharmacy, medical, clinical, pharmacology, regulatory, or health-data fields.');
  let arr = [];
  try {
    const txt = await claude(
`${RULES}\n\n${profileBlock(r)}\n\nTASK: Search the web NOW (today: ${today()}) for currently-open JOB postings matching this PhD-level professional. Scope: ${scopes.join('; and ')}.\nQUALITY BAR: real, currently-open, paid positions from the employer's own site or a credible job board, strictly within the applicant qualification domain, a PhD pharmacist and researcher: pharmacovigilance, drug safety, regulatory affairs, medical affairs, clinical research and trials, medical and scientific writing, pharmaceutical or medical data, health NGO and global health, hospital pharmacy leadership, academia. NEVER suggest roles beyond this domain or beyond the CV experience (no unrelated IT, finance, sales, engineering, teaching outside the field). Include mid and senior roles. EXCLUDE only: unpaid, internships, obvious scams, expired, or roles with no application route. Aim for the MAXIMUM number of genuinely relevant positions up to the cap.${r.minSalary ? ' Minimum acceptable compensation: ' + r.minSalary + '.' : ''}\nFind up to ${cap} items. Respond with ONLY a JSON array. Each element:\n{"title":"","institution":"","country":"","city":"","url":"","deadline":"YYYY-MM-DD or empty","category":"national_job|remote_job","compensation":"if stated","contactEmail":"official application email if publicly listed else empty","summary":"1 sentence","fit":"1-2 sentences","matchScore":0-100}`,
...
async function renderProposalPdf(proposalId) {
  ...
  pdf.font('Helvetica-Bold').fontSize(13.5).text(pRow.title.replace(/^(CV|Cover letter[^:]*|Concept note|Funding application|Interview brief|Two-body dossier): ?/i, ''));
  ...
  for (const para of String(pRow.content).split(/\n\n+/)) {
    ...
    if (isHeading) {
      pdf.moveDown(0.6).font('Helvetica-Bold').fontSize(12.5).fillColor('#000000').text(line.replace(/:$/, ''));
      const hy = pdf.y + 1.5;
      pdf.moveTo(pdf.page.margins.left, hy).lineTo(pdf.page.width - pdf.page.margins.right, hy).lineWidth(0.7).strokeColor('#222222').stroke();
      pdf.moveDown(0.3).font('Helvetica').fontSize(10.5).fillColor('#1a1a1a');
    }
  ...
  const kind = /^CV: /.test(pRow.title) ? 'CV' : /Cover letter/i.test(pRow.title) ? 'Cover letter' : /Concept note/i.test(pRow.title) ? 'Concept note' : /Funding/i.test(pRow.title) ? 'Funding application' : 'Document';
  // Human, professional filenames: no underscores, no dates, nothing that smells automated.
  const instShort = (pRow.title.match(/\(([^)]+)\)\s*$/) || [])[1] || '';
  const clean = t => String(t).replace(/[^A-Za-z0-9 .,-]/g, '').replace(/\s+/g, ' ').trim().slice(0, 40);
  let base;
  if (/^CV: /.test(pRow.title)) base = 'CV - ' + clean(u.name || 'Applicant');
  else if (/Cover letter/i.test(pRow.title)) base = 'Cover Letter' + (instShort ? ' - ' + clean(instShort) : '');
  else if (/Concept note/i.test(pRow.title)) base = 'Research Concept Note' + (instShort ? ' - ' + clean(instShort) : '');
  else if (/Funding/i.test(pRow.title)) base = 'Funding Application' + (instShort ? ' - ' + clean(instShort) : '');
  else base = clean(pRow.title);
...
`${RULES}\n\n${STYLE}\n\n${profileBlock(u)}\n\nCONTEXT: ${u.name} previously wrote to ${m.toName} (${opp ? opp.institution : ''}) — subject "${m.subject}":\n---\n${m.body}\n---\n\nTHE SUPERVISOR HAS NOW REPLIED — subject "${rep.subject}":\n---\n${rep.text || '(body could not be read)'}\n---\n\nTasks:\n1. Classify the reply intent as exactly one of: positive_interest | info_request | proposal_request | no_funding | rejection | interview | offer | administrative | unclear.\n2. Draft the appropriate next email from ${u.name} following the reply's actual content. Rules per intent: info_request, answer precisely and offer the specific documents; proposal_request, say a tailored concept note follows within 2 days and summarize the core idea in 3 sentences; positive_interest, suggest a brief call${'${'}u.schedLink ? ' and share the researcher scheduling link ' + u.schedLink + ' so they pick any slot' : ' with 2-3 concrete time windows (Pakistan time, offer to adapt)'} and one sharp scientific question; no_funding, propose the most credible fellowship route for this host country; rejection, one warm gracious sentence thanking them and keeping the door open; interview or offer, draft enthusiastic confirmation ASKING NOTHING BINDING, the researcher will finalize personally; administrative, respond helpfully; unclear, draft a polite clarification.\n3. Say whether the CV should be attached (only if they asked for documents).\nRespond ONLY with JSON: {"intent":"","subject":"","body":"","attachCV":true|false}`,
      ...
      if (canAuto) {
        try {
          let attachments = [];
          if (d.attachCV) {
            attachments = await generatedAttachments(u.id, m.oppId);
          }
          await sendMail({ to: rep.from, subject: d.subject,
            text: d.body + `\n\nBest regards,\n${u.name}\n${u.title || ''}`,
            fromName: u.name, creds, replyTo: creds ? undefined : (u.email || undefined), attachments, bcc: u.email || undefined, gmail: userGmail(u) });
...
`${RULES}\n\n${STYLE}\n\nPROFILE:\n${profileBlock(r)}\n\nTARGET: ${opp.title} at ${opp.institution}${opp.pi ? ', PI ' + opp.pi : ''}. Notes: ${opp.note}\n\nUsing the master CV above${cv ? '' : ' (no CV file found, use the profile)'} and ONLY facts it actually contains, write an opportunity-specific CV as clean plain text. Reorder so the most relevant research comes first. IMPORTANT: include EVERY publication, thesis and research output the applicant genuinely has, with real detail, emphasizing the ones this position cares about most but never dropping a real publication. Emphasize the techniques this laboratory cares about. Rewrite the opening research summary for this specific position. Never add, inflate or invent anything. Keep standard academic CV sections with clear bold headings.` });
  ...
  try {
    ...
    if (rev && rev.trim()) {
      const revised = await claude(`${STYLE}\n\nHere is a tailored CV:\n\n${cvOut}\n\nPrivate reviewer notes:\n${rev}\n\nReturn the improved CV using the same facts only, clean plain text with bold section headings. Return ONLY the CV, no commentary.`, { maxTokens: 3000, premium: true });
      if (revised && revised.trim().length > 200) cvOut = noDashes(revised);
    }
  } catch (e) {}
  const row = { id: uid(), resId: r.id, oppId, title: 'CV: ' + opp.title + ' (' + opp.institution + ')', status: 'DRAFT', content: cvOut.slice(0, 45000), createdOn: today() };
  await db.add('Proposals', row);
...
/* ---------- guarantee every core document of a case exists as an attachable PDF ---------- */
async function ensureCasePdfs(oppId) {
  const [props, docs] = await Promise.all([db.all('Proposals'), db.all('Documents')]);
  const CORE = ['CV: ', 'Cover letter: ', 'Concept note: ', 'Funding application: '];
  const newest = {};
  for (const p of props.filter(x => x.oppId === oppId)) {
    const k = CORE.find(c => (p.title || '').startsWith(c));
    if (!k) continue;
    if (!newest[k] || String(p.createdOn || '') >= String(newest[k].createdOn || '')) newest[k] = p;
  }
  let made = 0;
  for (const k of Object.keys(newest)) {
    const p = newest[k];
    const has = docs.some(d => /generated from/i.test(d.note || '') && (d.note || '').includes('opp:[' + oppId + ']') && (d.note || '').includes((p.title || '').slice(0, 30)));
    if (!has) { try { await renderProposalPdf(p.id); made++; } catch (e) {} }
  }
  if (made) await db.log('PDFS_ENSURED', oppId + ': ' + made + ' rendered');
  return made;
}
/* ---------- backfill: apply the new rules to ALL previously prepared cases ---------- */
async function backfillPreparedCases(limit) {
  limit = limit || 8;
  await db.connect();
  const [opps, outbox, props] = await Promise.all([db.all('Opportunities'), db.all('Outbox'), db.all('Proposals')]);
  const CORE = ['CV: ', 'Cover letter: ', 'Concept note: ', 'Funding application: ', 'Tailored CV: '];
  const hasDraft = new Set(outbox.filter(m => ['PENDING', 'APPROVED', 'SENT'].includes(m.status)).map(m => m.oppId));
  let done = 0;
  for (const o of opps) {
    if (done >= limit) break;
    if (o.archived === 'yes' || o.status === 'EXPIRED') continue;
    const hasCoreDoc = props.some(p => p.oppId === o.id && CORE.some(k => (p.title || '').startsWith(k)));
    const wasPrepared = o.prepDone === 'yes' || hasCoreDoc;
    if (!wasPrepared) continue;
    // Mark old cases as prepared so they surface in Action needed
    if (o.prepDone !== 'yes') {
      o._row.set('prepDone', 'yes');
      if (!o.prepStartedAt) o._row.set('prepStartedAt', new Date().toISOString());
      await o._row.save();
    }
    // Missing draft -> full new treatment: extract, hunt, guaranteed draft
    if (!hasDraft.has(o.id)) {
      try { await ensureCasePdfs(o.id); await draftEmailForCase(o.id); done++; } catch (e) { await db.log('BACKFILL_FAIL', (o.institution || '') + ': ' + e.message); }
    }
  }
  if (done) await db.log('BACKFILL', done + ' previously prepared case(s) upgraded');
  return done;
}
/* ---------- one-time data migration: clean old titles across ALL previous cases ---------- */
async function migrateNaming() {
  try {
    const props = await db.all('Proposals');
    for (const p of props) {
      const t = p.title || '';
      let nt = t.replace(/^Tailored CV: /, 'CV: ').replace(/^Cover letter \((Claude|GPT)\): /, 'Cover letter: ')
        .replace(/\u2014|\u2013/g, '-');
      if (nt !== t) { p._row.set('title', nt); await p._row.save(); }
    }
    const opps = await db.all('Opportunities');
    for (const o of opps) {
      const t = o.title || '';
      const nt = t.replace(/\u2014|\u2013/g, '-');
      if (nt !== t) { o._row.set('title', nt); await o._row.save(); }
    }
    const docs = await db.all('Documents');
    const users = await db.all('Users');
    const props2 = await db.all('Proposals');
    for (const d of docs) {
      const n = d.name || '';
      if (!/generated from/i.test(d.note || '')) continue;
      // old note formats: 'opp:xyz' without brackets, or no opp tag at all -> normalize to opp:[xyz]
      let note = d.note || '';
      const mOld = note.match(/opp:(?!\[)([A-Za-z0-9_-]+)/);
      if (mOld) { note = note.replace(/opp:(?!\[)[A-Za-z0-9_-]+/, 'opp:[' + mOld[1] + ']'); }
      else if (!/opp:\[/.test(note)) {
        // no tag: recover oppId by matching the source proposal title inside the note
        const srcTitle = (note.match(/Generated from:\s*([^|]+)/) || [])[1];
        const src = srcTitle ? props2.find(pp => srcTitle.trim().startsWith((pp.title || '').slice(0, 40))) : null;
        if (src && src.oppId) note = note.trim() + ' | opp:[' + src.oppId + ']';
      }
      if (note !== d.note) { d._row.set('note', note); await d._row.save(); }
      if (!/_\d{8}\.pdf$/i.test(n) && !/_/.test(n)) continue; // already human
      const u = users.find(x => x.id === d.resId) || {};
      const inst = ((d.note || '').match(/\(([^)]+)\)\s*\|/) || [])[1] || '';
      const clean = t => String(t).replace(/[^A-Za-z0-9 .,-]/g, '').replace(/\s+/g, ' ').trim().slice(0, 40);
      let base = '';
      if (/cv/i.test(d.type)) base = 'CV - ' + clean(u.name || 'Applicant');
      else if (/cover/i.test(d.type)) base = 'Cover Letter' + (inst ? ' - ' + clean(inst) : '');
      else if (/concept|proposal/i.test(d.type)) base = 'Research Concept Note' + (inst ? ' - ' + clean(inst) : '');
      else if (/funding/i.test(d.type)) base = 'Funding Application' + (inst ? ' - ' + clean(inst) : '');
      if (base) { d._row.set('name', base + '.pdf'); await d._row.save(); }
    }
    await db.log('MIGRATED', 'naming cleaned');
  } catch (e) {}
}
/* ---------- draft the outreach email for a case, recipient = hunted official address ---------- */
async function draftEmailForCase(oppId) {
  const opp = (await db.all('Opportunities')).find(o => o.id === oppId);
  if (!opp) return;
  const r = (await db.all('Users')).find(x => x.id === opp.resId) || {};
  // STAGE 0: extract any address already sitting in our scraped data (posting text, requirements, next steps)
  if (!opp.contact) {
    try {
      const hay = [opp.note, opp.requirements, opp.nextSteps, opp.funding].map(x => String(x || '')).join(' ');
      const found0 = [...new Set((hay.match(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g) || [])
        .map(e => e.toLowerCase().replace(/[.,;:]+$/, ''))
        .filter(e => !/(example|test|domain|email)\.(com|org)$/.test(e.split('@')[1]) && !/noreply|no-reply|donotreply/.test(e)))];
      if (found0.length) {
        opp._row.set('contact', found0.slice(0, 3).join(', ').slice(0, 300)); opp._row.set('noEmail', '');
        await opp._row.save(); opp.contact = found0.join(', ');
        await db.log('EMAIL_FOUND', opp.institution + ' (from posting): ' + found0.join(', '));
      }
    } catch (e) {}
  }
  // AGGRESSIVE CONTACT HUNT: if no address yet, search the supervisor full name at the university NOW
  if (!opp.contact) {
    try {
      const FREE = /(gmail|yahoo|hotmail|outlook|live|icloud|proton)\./i;
      const FAKE = /(example|test|domain|email)\.(com|org)$/i;
      const cleanE = arr => [...new Set((arr || [])
        .map(e => String(e || '').replace(/^mailto:/i, '').trim().replace(/[.,;:]+$/, '').toLowerCase())
        .filter(e => /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(e) && !FAKE.test(e.split('@')[1])))]
        .sort((x, y) => (FREE.test(x) ? 1 : 0) - (FREE.test(y) ? 1 : 0));
      const huntTxt = await claude(
`Find a REAL contact email for this postdoc position: ${opp.title} at ${opp.institution}${opp.pi ? ', supervisor ' + opp.pi : ''}. Search in this order until you find addresses: 1) the supervisor's full name with the university name (staff directory, faculty profile, lab website), 2) the position posting itself (postings often state 'applications to' or 'informal enquiries to' an address), 3) the institute or department contact page (secretariat, office, HR, admissions addresses on the university domain are all acceptable). Respond ONLY with a JSON array of addresses you literally SAW on official pages, best first (personal supervisor address > lab or secretariat > department office), up to 3, NEVER guessed or constructed: ["..."] or []`,
        { searchUses: 4, maxTokens: 300 });
      const found = cleanE(parseJSON(huntTxt));
      if (found.length) { opp._row.set('contact', found.slice(0, 3).join(', ').slice(0, 300)); opp._row.set('noEmail', ''); await opp._row.save(); opp.contact = found.join(', '); await db.log('EMAIL_FOUND', opp.institution + ': ' + found.join(', ')); }
      else await db.log('EMAIL_HUNT_EMPTY', opp.institution + ': no address on official pages yet');
    } catch (e) { await db.log('EMAIL_HUNT_FAIL', (opp.institution || '') + ': ' + e.message); }
  }
  const outbox = await db.all('Outbox');
  const existing = outbox.find(m => m.oppId === oppId && (m.status === 'PENDING' || m.status === 'APPROVED'));
  let subject = '', body = '';
  try {
    const txt = await claude(
`${RULES}\n\n${profileBlock(r)}\n\nVERIFIED OPPORTUNITY: ${opp.title} at ${opp.institution}${opp.pi ? ', PI ' + opp.pi : ''}. Notes: ${(opp.note || '').slice(0, 400)}\n\n${STYLE}\n\nBEFORE writing, search the web briefly for this supervisor's most recent relevant work. Write a warm, respectful, confident first-contact email from the applicant to the supervisor, 180 to 240 words, flowing prose paragraphs, no bullets. Open with genuine specific familiarity with the supervisor's recent work. Introduce the applicant confidently with directly relevant real strengths. Offer one concrete idea where the applicant's methods contribute to the supervisor's current line. State clearly that a CV and a short research concept note are ATTACHED for their consideration (never ask permission to send). Close courteously thanking them for their time. SUBJECT: name the supervisor's research topic or the position, never generic.\nRespond ONLY with JSON: {"subject":"","body":""}`,
      { premium: true, search: true, searchUses: 2, maxTokens: 900 });
    const d = parseJSON(txt) || {};
    subject = noDashes(d.subject || '').slice(0, 200);
    body = noDashes(d.body || '').slice(0, 7000);
  } catch (e) {}
  if (!body) {
    // AI unavailable or malformed output: build a solid professional draft locally so the case is NEVER empty
    const piName = (opp.pi || '').replace(/^(Prof\.?|Dr\.?|Professor)\s*/i, '');
    const greet = piName ? 'Dear Professor ' + piName.split(' ').slice(-1)[0] + ',' : 'Dear Professor,';
    subject = 'Postdoctoral application, ' + (opp.title || 'your research group').slice(0, 80) + ' at ' + (opp.institution || '').slice(0, 60);
    body = greet + '\n\n' +
'I am writing to express my strong interest in the ' + (opp.title || 'postdoctoral position') + ' at ' + opp.institution + '. My research background aligns closely with the goals of this project, and I would be glad to contribute from day one.\n\n' +
'I hold a ' + (r.title || 'PhD') + (r.field ? ' with a focus on ' + r.field : '') + '. ' + (r.methods ? 'My core methods include ' + String(r.methods).slice(0, 180) + '. ' : '') + (r.pubs ? 'My published work includes ' + String(r.pubs).slice(0, 220) + '. ' : '') + '\n\n' +
'My curriculum vitae and a short research concept note are attached for your consideration. I would welcome the opportunity to discuss how my experience can support your current research directions.\n\n' +
'Thank you for your time and consideration.\n\nKind regards,\n' + (r.name || '');
    body = noDashes(body);
    subject = noDashes(subject);
    await db.log('DRAFT_FALLBACK', opp.institution + ': local draft used, AI draft unavailable');
  }
  const toEmail = (opp.contact || '').slice(0, 300);
  if (existing) {
    existing._row.set('subject', subject || existing.subject);
    existing._row.set('body', body);
    if (!existing.toEmail && toEmail) existing._row.set('toEmail', toEmail);
    await existing._row.save();
  } else {
    await db.add('Outbox', { id: uid(), resId: opp.resId, oppId, toEmail, toName: opp.pi || opp.institution,
      subject, body, type: 'outreach', status: 'PENDING', createdOn: today(), sentOn: '', followups: '0', lastFollowupOn: '', replied: '' });
  }
  await ensureCase(opp, toEmail ? 'Email prepared' : 'Verified');
  await db.log('EMAIL_DRAFTED', opp.institution + (toEmail ? ' -> ' + toEmail : ' (no address yet)'));
}
...
async function analyzeCase(oppId) {
  ...
  try {
    const txt = await claude(
`${RULES}\n\n${profileBlock(r)}\n\nRead the FULL requirements of this position by searching its official page and the supervisor or lab pages.\nPOSITION: ${opp.title} at ${opp.institution}${opp.pi ? ', PI ' + opp.pi : ''}. Country: ${opp.country}. URL: ${opp.url || 'search for it'}. Notes: ${opp.note || ''}\nThe applicant already has these document types on file: ${haveTypes.join(', ') || 'none yet'}.\n\nReturn a complete analysis. Respond ONLY with JSON:\n{"requiredDocuments":["exact documents this application requires"],"eligibility":["key eligibility criteria and whether the applicant plausibly meets each, one line each"],"missingFromUser":["documents or specific information the applicant must still provide, empty if none"],"stipend":"salary or stipend with currency and period if stated, else best public estimate labelled as estimate","duration":"contract length if stated","deadline":"YYYY-MM-DD or empty","chance":"Strong|Moderate|Long shot with one sentence why, grounded in real fit","perks":"brief real perks: salary level, housing or family benefits, relocation support, visa ease, healthcare, pension, one short line","supervisorFocus":"2-3 sentence summary of the PI current research focus from their recent work","nextSteps":["what the agent will do next for this case, concrete"],"whatToPrepare":["CV","Cover letter","Concept note","Funding narrative"],"officialEmails":["THE MOST IMPORTANT FIELD. Search the supervisor FULL NAME together with the university name. Check, in order: the university staff directory, the PI official faculty profile page, the lab website, the position posting, the department contact page. Return up to 3 REAL addresses you actually SAW on those pages, university domain addresses FIRST. NEVER guess, construct, or invent an address, only ones literally present on an official page. If the PI has both a university address and another address, include both, university first. Empty array ONLY if genuinely nothing exists after thorough searching."],"fundingRoute":{"needed":false,"scheme":"name of the external fellowship or sponsor scheme if this position needs separate funding, else empty","url":"official scheme page if known","deadline":"YYYY-MM-DD or empty"}}`,
      { premium: true, search: true, searchUses: 5, maxTokens: 1800 });
    a = parseJSON(txt) || {};
  ...
  set('stipend', a.stipend || ''); set('duration', a.duration || ''); set('chance', a.chance || ''); set('perks', a.perks || '');
  set('nextSteps', a.nextSteps || []);
  if (a.deadline && isDate(a.deadline) && !opp.deadline) opp._row.set('deadline', a.deadline);
  await opp._row.save();

  // FEATURE: funding on a separate forum -> auto-start a LINKED funding sub-case, no waiting
  try {
    const fr = a.fundingRoute || {};
    if (fr && (fr.needed || (fr.scheme && fr.scheme.trim()))) {
      const opps2 = await db.all('Opportunities');
      const exists = opps2.some(x => x.parentOppId === oppId);
      if (!exists) {
        const fRow = {
          id: uid(), resId: r.id, title: 'Funding: ' + (fr.scheme || 'External fellowship') + ' (for ' + opp.institution + ')',
          institution: fr.scheme || 'External funding scheme', country: opp.country || '', pi: opp.pi || '',
          url: fr.url || '', deadline: (fr.deadline && isDate(fr.deadline)) ? fr.deadline : '',
          funding: 'Funding application', status: 'VERIFIED', matchScore: opp.matchScore || '70',
          note: 'Linked funding route for ' + opp.title + ' at ' + opp.institution + '. ' + (fr.scheme || ''),
          dupKey: 'fund|' + oppId, addedOn: today(), verifiedOn: today(),
          category: 'international_job', section: 'postdoc', parentOppId: oppId
        };
        await db.add('Opportunities', fRow);
        await ensureCase(fRow, 'Discovered');
        fundingNarrative(fRow.id).catch(() => {});
        await db.log('FUNDING_SUBCASE', fRow.title);
      }
    }
  } catch (e) {}
  // Missing items are shown inside the case view as optional info only. No tasks, no nagging.
  ...
  const cleanEmails = arr => [...new Set((arr || [])
    .map(e => String(e || '').replace(/^mailto:/i, '').trim().replace(/[.,;:]+$/, '').toLowerCase())
    .filter(e => /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(e) && !FAKE.test(e.split('@')[1])))]
    .sort((x, y) => (FREE.test(x) ? 1 : 0) - (FREE.test(y) ? 1 : 0)); // university/official domains first
  let emails = cleanEmails(Array.isArray(a.officialEmails) ? a.officialEmails : [a.officialEmail]);
  if (!emails.length) {
    // Dedicated hunt: search the supervisor full name at the university directly
    try {
      const huntTxt = await claude(
`Search the web for the official email address of ${opp.pi || 'the supervising professor'} at ${opp.institution}. Search their full name with the university name, check the university staff directory and their official faculty page. Respond ONLY with a JSON array of addresses you literally SAW on official pages, university domain first, up to 3, never guessed or constructed: ["..."] or []`,
        { searchUses: 3, maxTokens: 300 });
      emails = cleanEmails(parseJSON(huntTxt));
    } catch (e) {}
  }
  if (emails.length) { opp._row.set('contact', emails.slice(0, 3).join(', ').slice(0, 300)); opp._row.set('noEmail', ''); }
  else if (!opp.contact) { opp._row.set('noEmail', 'yes'); }
  ...
  try { if (opp.status === 'NEEDS_VERIFICATION') { opp._row.set('status', 'VERIFIED'); opp._row.set('verifiedOn', today()); await opp._row.save(); } } catch (e) {}
...
async function coverLetter(oppId) {
  ...
  const row = { id: uid(), resId: r.id, oppId, title: 'Cover letter: ' + opp.title + ' (' + opp.institution + ')', status: 'DRAFT', content: content.slice(0, 45000), createdOn: today() };
  await db.add('Proposals', row);
  const owner = r.email || process.env.OWNER_EMAIL;
  // (document is saved in the app; not emailed to the user)
...
async function runCycle(opts = {}) {
  ...
  try {
    ...
    if (!light) {
      ...
      if ((process.env.JOB_ENGINE || 'off') === 'on') researchers.forEach(r => jobs.push(jobDiscover(r, report)));
...
module.exports = { runCycle, cfg, draftProposal, interviewBrief, coupleDossier, weeklyReview, draftRefereeRequests, tailoredCV, coverLetter, setRuntimeMode, loadRuntimeMode, targetLabMap, fundingNarrative, sendOne, analyzeCase, piInsight, computeReadiness, buildReminders, migrateNaming, draftEmailForCase, backfillPreparedCases, ensureCasePdfs };
